...
| Idempotent? | Yes — balanced/converted trees are no-ops; see Step 5 |
| Modifies memory files? | Yes — only after explicit confirmation; see Step 5 |
| FKF-aware moves? | Yes — bundle-relative rewrites preserve FKF frontmatter; see Step 5 Verify |
| Requires config/constitution? | No |
| Is the memory rebalancer? | Yes — folder/file split, merge, flatten, and move; see Steps 3–5 |
| Splits over-size files? | Yes — per `split-file`; restyling stays in `/docs-distill-memory` |
| Detects duplicate coverage? | Yes — `## Duplicate Coverage`; see Steps 3–4 |
| Triages `_unsorted/`? | Yes — per-file move/delete with its exemption and approval rules; see Steps 1, 3, 5 |
