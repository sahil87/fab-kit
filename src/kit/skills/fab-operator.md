...
**Same-repo resolution (`stacked-prs` mode).** Steps 1–3 are skipped for same-repo dependencies — the dependent's branch is created off its nearest same-repo predecessor's *branch* at the §6 spawn sequence's worktree/branch step instead of off `origin/{default_branch}` (the probe-and-route per `_cli-external.md` § wt: existing dep branch → `wt create --checkout <dep-branch>` route). Right after that branch step, the operator records the dependency branch as the dependent's base — `fab status set-base-branch <change> <dep-branch>`, run in the target worktree — so `/git-pr` creates the dependent's PR against `<dep-branch>` directly and every base-relative surface measures only the dependent's own delta; no post-create retarget. The squashed `"operator: cherry-pick"` commit does not exist for same-repo deps in this mode. The merge-all choreography for the stack lives under Ordered Merge below. Dependency-branch drift after a dependent PR exists (a dep's review-pr rework moving its branch) is out of scope — the same exposure exists in the cherry-pick model; conflicts surface at merge-all and escalate.
