...
| Principle | Rule |
|-----------|------|
| Coordinate, don't execute | Every task, bug report, or idea the user hands the operator **is a work request** and goes to an agent; a request to start an agent **with no task is a bare spawn** and also goes to an agent — the operator **never refuses to start an agent**. Routing is §6 Working a Change's two axes (does the target repo have `fab/`; was a task given). A report naming a live tracked item is a send to that item's agent. Reading code to reproduce or diagnose, or editing files, in the operator pane **is** executing and is prohibited. The operator reads whatever plan, roadmap, intake, or task document the tracked work needs, and keeps handing the next unit of work to an agent until the plan is done. Direct actions are exactly this maintenance allowlist: merge PR, archive, worktree deletion, rebase/cherry-pick for dependency resolution **and § Review-PR Recovery** (§6), `fab operator track` verbs (§4), and pane sends/answers/nudges (§5). The only thing the operator asks about a work request **or a spawn** is **which repo** — never whether to spawn, and **never for a task: a spawn request with no task is a bare spawn, not a question**; that ask-set is scoped to the request and does not relax the §3 pre-send and destructive-action confirmations or the §6 queue confirmation, which still apply. |
...
`wt` gates the **agent spawn path only** (§6 step 3 — every cell of §6 Working a Change, bare spawns included) — tracking a GitHub PR, a Linear query, or a shell probe needs no worktree, so startup does not probe it. When a pane item is about to spawn, probe `command -v wt >/dev/null 2>&1` once per operator session; on failure, STOP the spawn:
...
4. Sync and read the clock — run `fab operator clock sync` (`_cli-fab-operator.md` § fab operator clock sync): the binary seeds the operator-tick entry when this server has none, reconciles the mute against the tracked set (muting an untracked set; clearing a standing mute or lease while work is tracked — any item whose state is not `done`), applies the derived schedule, and prints the resolved row as YAML — `id`, `schedule_summary`, `deliver`, `muted`, `muted_until` (`muted` is the effective state — an indefinite mute or a live lease; `muted_until`, unix seconds, is non-null only while a lease is live). The operator never composes an `rk cron add` or hand-parses `rk cron list --json` here. A non-zero exit STOPs, quoting the binary's own line verbatim: `ERROR: could not resolve or seed the operator-tick cron entry` (rk broken, an ambiguous `role:operator` tie, or a failed `rk cron add`)
5. Output the ready line **with the clock status** — exactly one template, rendered from step 4's fields:
   ...
   (`{schedule_summary}` and `{deliver}` are step 4's YAML values verbatim; append ` · muted` when `muted` is true; append ` until HH:MM` — local time — when `muted_until` is present. Renders today as `Operator ready. Clock: rk cron "operator tick" · backoff 3m→24m · skip-if-busy`, or `… · skip-if-busy · muted until 14:30`. A missing entry is seeded by step 4, and an entry that can neither be resolved nor seeded STOPs there — there is no `Clock: none` form.)
...
3. **Single match, action**: create a worktree and proceed — probe-and-route the branch per `_cli-external.md` § wt (existing branch → `wt create --non-interactive --name <name> --checkout <branch>`; missing → the positional form)
...
| Review-PR Recovery (§6) | 1 per reason | per its table |

---

## 4. The Clock

The operator's cadence is **not owned by this skill** — it is HexoKit substrate the skill documents and verifies, exactly like `@rk_pane_agent_state`. The clock is the **operator-tick cron entry** (one per tmux server), seeded by fab's clock reconcile whenever the server has no `role:operator` row — `rk operator`'s launcher also seeds it, and both key on that row, so there is never a duplicate; HexoKit's operator-cron spec is the entry's schema authority, fab's reconcile its lifecycle owner. Ticks arrive as the bare text `operator tick` delivered into the operator pane, for **every** provider (Claude, codex, gemini, …) — no provider-specific in-session clock is the primary cadence.

The entry's steady-state shape (reference summary — schema and semantics are owned by the cron spec, not restated here). Every line below is what fab's seed writes and its reconcile keeps (§4 Ownership); `respawn` is rk's launcher argv, carried verbatim:
...
**Ownership.** fab's clock reconcile owns the entry end to end. When a reconcile finds no `role:operator` row it seeds one — exactly one `rk cron add` with the full steady-state shape above, issued by the binary (the skill never composes an `add`) — and every `fab operator track` verb, `tick-start --diff`, and `fab operator clock sync` manages it as a side effect: the **tracked predicate** — any item whose state is not `done` — flips the entry between muted and live via `rk cron mute <id>` / `--off`, and the **schedule reconcile** derives the cadence from the tracked set (backoff `3m`→`24m` while only pane/none items are tracked; `--idle-every`/`--every <min check_every>` once any shell/agent item exists — deliver is `skip-if-busy` on every branch) and applies it with exactly one `rk cron edit` when — and only when — the live entry differs. The seed rule, derive table, epoch detection, and fail-silent posture are owned by `_cli-fab-operator.md` § fab operator (the shared **Clock side effect** paragraph); `rk cron rm` stays the user's, and a removed entry is re-seeded by the next reconcile.
...
| Element | Rule |
|---|---|
| Header | `🛰️ **Operator** · {HH:MM} · tick #{N} · **{tracked} tracked** · {schedule_summary} · {deliver}` — the cadence cells are copied verbatim from the per-tick `fab operator clock sync` output (tick step 1), never from memory |
| Compact frame (quiet tick) | `🛰️ **Operator** · {HH:MM} · tick #{N} · **{tracked} tracked** · {schedule_summary} · {deliver} · no change[ · {W} waiting]` — rendered from `fleet_summary:` plus the same `clock sync` output |
...
- **Pipeline-first** — new work **in a fab project** (the target repo has a `fab/` directory) MUST enter through `/fab-new`, then `/fab-fff`, `/fab-ff`, or `/fab-continue`; never send raw implementation instructions to a fab-project agent or use `/fab-continue` to skip intake — the single exception is § Review-PR Recovery's CI fix request, which amends a shipped change. A repo with **no** `fab/` is exempt only from the pipeline — there is none there — never from delegation: its work goes to a plain agent in a fresh worktree (§6 Working a Change → no-`fab/` + task cell). A spawn with no task is exempt for a different reason: nothing is sent, so nothing bypasses `/fab-new` (→ bare agent). The operator **never runs `fab init`** to manufacture a pipeline — bootstrapping fab into a repo is the user's repo-structure decision; the operator MAY mention it once in a report as an option, never picks it, never offers it as a menu. Direct actions are exactly the §1 maintenance allowlist.
- **Spawn in a worktree** — reserve the operator pane for orchestration. Every pipeline command, including a one-line change — and every plain-agent or bare-agent spawn — starts with `wt create --non-interactive` (§2 wt Gate) and runs in a fresh agent tab.

A spawn that deliberately parks early — e.g. a `/fab-ff` run, which stops after hydrate — MUST be tracked with `--stop-stage hydrate`; otherwise the item never completes and sits in the list until the user stops it. Plain-agent and bare-agent spawns have no stages and take no `--stop-stage`; the plain agent completes through its chained `github-pr` item, the bare agent through pane death / agent exit / `track rm` (§6 Working a Change).
...
3. **Create worktree** — run the repo-targeted, probe-and-route procedure in `_cli-external.md` § wt (gated by the §2 wt Gate on the first agent spawn of the session); never rely on the operator's CWD. **Fetch first, pin the base**: before any **new-branch** `wt create` form (the positional `<change-folder-name>`, or the branch-less bare/no-`fab/` form), run `git fetch origin` in the target repo's main-worktree root, resolve `{default_branch}` per Dependency Resolution step 0's chain, and pass the fetched tip as the start-point:

   ```sh
   # In the TARGET repo's main-worktree root, immediately before a new-branch wt create:
   git fetch origin || exit 1                                  # a failed fetch is a hard spawn STOP — see below
   base_sha=$(git rev-parse "origin/${default_branch}")     # {default_branch}: Dependency Resolution step 0
   wt create --non-interactive --name <name> --base "$base_sha" [<change-folder-name>]
   ```

   **A failed fetch is a hard spawn STOP**: report the git error, run no `wt create`, leave the item `pending` for the next tick (a transient network failure retries itself; a persistent one is escalated per §3). Never fall through to `rev-parse` after a failed fetch — it would silently return the *old* `origin/{default_branch}` and cut exactly the stale-base worktree this step exists to prevent. Without the pin, `wt create` cuts the branch from the main checkout's **local HEAD** — whatever commit that checkout last pulled — so every queue item after the first inherits accumulated staleness and the conflict surfaces at merge time. **Pass the commit SHA, never the ref name `origin/{default_branch}`**: a remote-tracking start-point makes git set upstream tracking on the new branch (`branch.<name>.remote=origin`), which flips `/fab-new` Step 11 row 5's rename guard (it requires an empty upstream) to the "created, leaving intact" row and leaves the worktree on the wrong branch, and mis-targets `push.default=simple`; a SHA start-point sets no upstream. The `--checkout <branch>` form (an existing change branch; a `stacked-prs` dependent off its dependency branch) is **untouched** — wt rejects `--base` alongside `--checkout`, and that route already checks out the intended base. A `--reuse` respawn may carry `--base` harmlessly (the existing worktree is reused). Dependency Resolution step 0's own fetch stays: it refreshes the cherry-pick range; this fetch fixes the worktree base.
4. **Activate the change pointer (existence-guarded)** — in the **just-created worktree's directory**, set that worktree's own `.fab-status.yaml` so the worktree is self-describing after the pipeline completes (a bare `fab`/`/fab-*` later resolves the change without naming it). Run the switch **only when the change folder already exists** — `fab resolve --folder <change>` succeeds iff a non-archived change folder matches:
   ...
   **Guard:** switch only an already-existing change, from the just-created worktree CWD, and fail soft. Raw/backlog spawns wait for `/fab-new` Step 10; the no-`fab/` and bare cells have no change and skip the switch entirely (the guard's `fab resolve` fails in a repo with no `fab/`, which is the intended fail-soft); the dedicated worktree owns its own pointer, and the embedded transient override preserves correctness if activation fails.
...
**Same-repo resolution (`stacked-prs` mode).** Steps 1–3 are skipped for same-repo dependencies — the dependent's branch is created off its nearest same-repo predecessor's *branch* at the §6 spawn sequence's worktree/branch step instead of off `origin/{default_branch}` (the probe-and-route per `_cli-external.md` § wt: existing dep branch → `wt create --checkout <dep-branch>` route). The operator then records the dependency branch as the dependent's base — `fab status set-base-branch <change> <dep-branch>`, run in the target worktree — so `/git-pr` creates the dependent's PR against `<dep-branch>` directly and every base-relative surface measures only the dependent's own delta; no post-create retarget. The write's **timing splits by spawn kind**: an existing-change entry's `.status.yaml` already exists, so the write runs right after the branch step; a raw-text, backlog, or Linear entry's change does not exist yet — the pane's `/fab-new` creates it later, and its Step 11 records the chain-resolved default first — so the write instead rides the rendered `fab-new` skill prompt as a trailing instruction ("after `/fab-new` creates the change, run `fab status set-base-branch <new-change> <dep-branch>`"), overwriting that default with the dependency branch. The squashed `"operator: cherry-pick"` commit does not exist for same-repo deps in this mode. The merge-all choreography for the stack lives under Ordered Merge below. Dependency-branch drift after a dependent PR exists (a dep's review-pr rework moving its branch) is out of scope — the same exposure exists in the cherry-pick model; conflicts surface at merge-all and escalate.
...
| | task given | no task |
|---|---|---|
| **`fab/` present** | **Pipeline.** Existing change → skill `fab-fff` with argument `<change>` = the item's `scope.change` (its id for known-change spawns); the target repo is the item's `scope.repo` or its `branch_map` entry — a path, never the argument. The transient override targets the pipeline and step 4 activates the pointer. Raw text (for example, "fix login after password reset") → skill `fab-new` with the raw description as its argument string. Backlog ID or Linear issue → resolve it first (optional `idea` lookup per `_cli-external.md` § Delegation and binary gate), then skill `fab-new` with argument `<id>`. Render per `_cli-agents.md` § Skill Prompts; the existence guard skips activation until `/fab-new` creates and activates the change at Step 10. Pipeline-first (§6 The pane Kind and Spawn Rules) binds this cell. Completion: the built-in predicate — PR ready, optionally archive. | **Bare agent** (below) |
| **no `fab/`** | **Plain agent.** There is no pipeline to enter, so the prompt is the **raw task text followed by the closing instruction `Commit on this branch, push, and open a draft PR against the default branch.`** — not a skill invocation: no `skill_prefix`, nothing to render per `_cli-agents.md` § Skill Prompts. Step 3 is the fetch-first base pin, then `wt create --non-interactive --name <name> --base "$base_sha"` with **no branch argument** (there is no change branch; wt puts the worktree on a new branch of the same name, cut from the fetched default-branch tip, on any git repo); step 4's existence guard trivially skips; step 6's `fab agent default -o yaml --repo <target-repo>` degrades to the project-free config cascade (`_cli-fab-operator.md` § fab agent) and its `command` is still the session command. Step 8 tracks it unconditionally as a change-less `pane` item, id = the worktree name, passing the branch wt created — also `<wt>` — so the `{branch, repo}` pair lands in `branch_map[<wt>]` and survives the item's removal: `fab operator track add <wt> --kind pane --pane <pane-id> --session <session> --repo <repo> --branch <wt>`; no stages, so no `--stop-stage`. **Completion is a chained `github-pr` item**: once the PR URL is known (the pane shows it, or the user relays it), run `fab operator track add <wt>-pr --kind github-pr --scope '{"repo":"<repo>","pr":<n>}' --check-every 2m` and `fab operator track rm <wt>`; the user may `track rm` the pane item at any time. | **Bare agent** (below) |

**Bare agent — no task, either repo type.** The same eight steps run: step 3 `wt create --non-interactive --base "$base_sha"` with no branch argument (the fetch-first base pin from step 3 applies — it is a new-branch form; wt picks a random name, or `--name <name>` when the user gave one — that name is the worktree, the branch, the tab name, and the item id); step 4's existence guard trivially skips (no change); step 5 dependencies as usual — a bare spawn MAY depend on other items, but **nothing may `--depends-on` a bare item**: with no change it has no built-in completion and never reaches `done`, so a dependent would be held forever (`track rm` of a not-done item leaves dependents held); step 6 as usual (`skill_prefix` unused); **step 7 passes no prompt token** — the argv after `--` ends at the session command: no skill prefix, no closing PR instruction; step 8 `fab operator track add <wt> --kind pane --pane <pane-id> --session <session> --repo <repo> --branch <wt>`, no `--stop-stage`, **no chained `github-pr` item**. The user drives that pane — including typing `/fab-new` in it; a change that later appears is an ordinary `changed` delta. **Completion is pane death, agent exit, or the user's `track rm`**: the operator reports and acks per §4 Tick Behavior step 4 and never respawns (respawn is queue-driven only, § Queues). **Pipeline-first still holds in the `fab/` cell**: the operator sends nothing, so nothing enters the repo outside `/fab-new` — "MUST enter through `/fab-new`" binds *work the operator sends*, not the act of opening an agent.

The table and the bare-agent paragraph above are the **only** place the cells are defined; every other site names a cell (`fab/` + task, no-`fab/` + task, bare agent), never a numbered form. **Never `fab init` the repo** — the rule and its rationale are owned by §6 The pane Kind and Spawn Rules → Pipeline-first.
...
- **`merge-auto`** — merge-as-you-go: **arm** each PR on completion (§6 Auto-Merge Choreography — a one-PR merge sequence; all five rules apply) instead of merging and foreground CI-waiting; once the merge is verified on a later tick (the `github-pr` item's `done` delta runs its `then`), spawn the next change through §6 — its step 3 fetches and pins the new branch to the just-advanced `origin/{default_branch}` tip (the default branch resolved per Dependency Resolution step 0 — never a hardcoded `origin/main`), so the next change starts from the advanced line. Implicit chaining is disabled in this mode — each change is based on `origin/{default_branch}` independently, and nothing is rebased before spawn (a backlog or raw-text item has no branch yet; the base pin is the mechanism). Natural language equivalents: "merge as you go", "merge on complete", "merge each when done".
...
- **`stacked-prs`** — `cherry-pick-ladder` merge timing (PRs created up front, merged only on explicit user request) with true stacked-PR topology for same-repo chains: the dependent's branch is created off its dependency's *branch* (no cherry-pick commit), the operator records that branch via `fab status set-base-branch`, and `/git-pr` targets the PR at the dependency's branch through the recorded base, so each PR diff shows only its own delta. Mechanics: same-repo resolution in Dependency Resolution above; merge-all choreography in Ordered Merge. Natural language equivalents: "stacked PRs", "stack the PRs".
...
In `merge-auto` mode, steps 5–8 arm the just-shipped PR (§6 Auto-Merge Choreography — one `github-pr` item whose `then` spawns the next change) and **defer the next change's spawn to the tick that verifies the merge** — spawning earlier would base the next agent on an `origin/{default_branch}` that does not yet contain the merge. On the verified-merge tick: report the merge and spawn per §6 — step 3's `git fetch origin` + `--base <sha>` is what puts the new branch on the advanced tip; there is no separate pre-spawn rebase step.
...
**Failures**: review exhausted → skip (`track rm`). Ship-stage rebase conflict (`/git-pr` rebases onto the recorded base before pushing; an unclear conflict aborts and STOPs in the worker pane, any mode) → flag to the user; in `merge-auto` the queue continues with the next item (nothing depends on the stopped one), in chained modes its dependents stay held. Rebase conflict during a `stacked-prs` merge-all → escalate (never skip). Cherry-pick conflict → escalate (do not skip). Pane dies → 1 respawn (`--reuse`), then skip. Stage timeout (>30m) → flag. Total timeout (>2h) → flag.
...
3. **Stall rule.** The tick's mechanical probe of a still-unmerged armed `github-pr` item feeds two inspections. A **failed required check** (`gh pr checks`) is Ordered Merge's CI failure → § Review-PR Recovery `ci-failed` (a failed check never makes the PR `CONFLICTING`; auto-merge just silently never fires). `unchanged ≥ 3` on the item (the binary-owned consecutive-no-delta counter) with no failed check → check `gh pr view --json mergeable` (the field is `mergeable` — `gh pr view --json` exposes no `mergeableState` field); `CONFLICTING` → § Review-PR Recovery `conflicting`. Both shapes are event-less — the tick's probe IS the poll.
4. **The sequence is a chain of `github-pr` items.** Starting a merge sequence MUST add one item per PR: `fab operator track add pr-<n> --kind github-pr --scope '{"repo":…,"pr":n,"change":"<id>"}' --then "arm next: gh pr merge --auto --squash <next>" --depends-on pr-<n-1>` — the first item takes no `--depends-on` and is armed immediately (the `"change"` key is the PR → change link § Review-PR Recovery uses to locate the authoring pane). An armed PR **outlives the operator** (it survives compaction, `/clear`, crash, and abandonment), and the items ARE the sequence state: a restarted operator re-orients from `fab operator track list` and resumes verifying/arming. Per tick, a `done` delta on `pr-<n>` runs its `then` verbatim — arming PR_{n+1} (readying a draft per rule 2) — and each armed item's `unchanged` counter feeds rule 3's stall threshold; on halt, rule 5's disarm runs over the remaining armed items, which are then removed via `track rm`.
5. **Disarm on halt.** Any halt or escalation — CI failure, stall, conflict — **after § Review-PR Recovery's bounded action is exhausted** — MUST run `gh pr merge --disable-auto` on the remaining armed PRs of the **halted sequences** (one `github-pr` item each): the failing repo's sub-sequence plus its transitive cross-repo dependent cone, matching the halt-dependents-only policy (which assumes unstarted merges stay unstarted — armed auto-merge violates that without the disarm). An armed PR is NOT disarmed while a recovery round is in flight. Independent sub-sequences keep their armed items and continue. A user "stop" is global and disarms every armed PR.
...
A `review-pr` stage that cannot finish is ONE condition with a **reason**; each reason has ONE bounded recovery action, after which the stage is terminal or the item escalates with evidence. Recovery is **per item** — independent stuck items are not a queue and are never sequenced against each other; a *failed* recovery still applies Ordered Merge's halt-dependents-only policy and rule 5's disarm. Detection uses only signals that already exist: `/git-pr-review`'s `timeout` outcome, the `gh pr checks` failed-required-check probe, and `gh pr view --json mergeable` == `CONFLICTING` (never a GraphQL timeline / `reviewRequests` read — GraphQL omits bot reviewers — and never a new drop counter).

| Reason | Detected by | One bounded action | Then |
|--------|-------------|--------------------|------|
| `no-reviewer` | A tracked `pane` item observed at stage `review-pr` (`scope.stage`), agent `idle` past the §8 stuck threshold (15 m default), with no `recovery` key in the item's `scope` — the pipeline stopped on `/git-pr-review`'s first `timeout` and is waiting for a re-run that nothing else will send | **Re-send once**: record the send FIRST — `fab operator track update <id> --scope '{"recovery":"resend"}'` (record-before-act: a crash between record and send costs at most one repeat, never an unbounded resend loop) — then the routed skill send `/git-pr-review <change>` to the authoring pane (rendered per `_cli-agents.md` § Skill Prompts; §3 pre-send gate — `idle` only). Pane gone (`pane_death` / `agent_exited` acked, or the item already removed) → one recovery spawn per § Recovery spawn below with the same invocation. `/git-pr-review`'s own budget terminalizes on the second consecutive timeout (`review-gate-unavailable`) — the operator never finishes or skips the stage itself | The item's built-in predicate fires (`review-pr` done) → normal merge mode takes over: `merge-auto` arms on completion, `cherry-pick-ladder` / `stacked-prs` wait for "merge all". Budget: 1 re-send per item per activation, persisted as `scope.recovery == "resend"` — durable across compaction/restart, without which a reloaded operator could not tell a re-sent pane from a first observation and would resend every tick. A second idle-at-`review-pr` on an item already carrying the marker is a stuck-agent report (§3), not another send |
...
This is the one sanctioned raw send into a fab-project agent: it **amends a change that already completed the pipeline** — the same class as `/git-pr-review`'s own fix commits — not new work, so Pipeline-first (§6 The pane Kind and Spawn Rules) is not bypassed (its bullet carries the carve-out clause).

**Recovery spawn** (shared by the three rows). When the authoring pane is gone, run the §6 spawn sequence steps 1–8 with: step 3 = the existing-branch route (`--checkout <branch>`); step 7's prompt = the row's invocation (`/git-pr-review <change>` rendered per Skill Prompts for `no-reviewer`; the raw fix request for `ci-failed`; `conflicting` needs no agent — the rebase is an operator maintenance action); step 8 tracks it as a **change-less** pane item `<change>-fix --kind pane --pane … --repo … --branch <branch>` (no `--change`, so the built-in completion — already `done` for a shipped change — does not remove it on the next tick); `track rm <change>-fix` when the round resolves (PR merged, or escalation). Budget: **1 recovery spawn per item per reason** (the §3 Bounded Retries pointer row names it).

**Locating the authoring pane.** Join by change, never by memory: `fab pane map --all-sessions` enriches every pane with its change/stage — the pane whose `change` equals the PR's change id is the authoring pane (its tracked item is typically already `track rm`'d on the tick `review-pr` finished; the pane itself survives, idle). The PR → change link rides the `github-pr` item's opaque `scope` — `"change":"<id>"`, added by Auto-Merge rule 4's `track add` line.

**Report wording** (tick footnote / merge report — bare markdown, §4 render rule):

- `no-reviewer` re-send: `"{change}: review-pr idle {N}m — re-sent /git-pr-review (retry 2 of 2)"` · spawn form: `"{change}: review-pr idle {N}m, pane gone — spawned {wt} → %N for /git-pr-review"`
- Unreviewed merge (MUST): on the verified-merge tick, for every PR merged this tick with no non-`PENDING` review (`gh pr view <n> --json reviews -q '[.reviews[] | select(.state != "PENDING")] | length'` == `0`) the per-merge line appends ` · unreviewed` and the tick emits one summary line `"merged with review gate unavailable: #{n} ({change}), #{m} ({change})"`. Never silent, never a prompt.
- `ci-failed` round: `"{change}: CI failed (check `{name}`, run {id}) — fix round sent to %N"` · exhausted: `"{change}: CI still failing after fix round (check `{name}`, head {sha}). Halted: {sub-sequences}. Completed: {sub-sequences}. Diagnosis: {agent's line}. Fix and retry."`
- `conflicting`: `"{change}: PR #{n} CONFLICTING — rebased onto {default_branch}, pushed {sha}, re-armed"` · exhausted: `"{change}: PR #{n} CONFLICTING again after rebase — disarmed. Halted: {sub-sequences}. Resolve by hand."` · rebase conflict: `"{change}: rebase conflict ({files}) — aborted, disarmed. Halted: {sub-sequences}."`

**Recovery state.** The per-item budget lives on the tracked item's `scope.recovery` (written by `track update --scope`, merged per key, durable across compaction — the items ARE the sequence state per rule 4): `ci-fix`/`rebase` on the `github-pr` item, `resend` on the `no-reviewer` pane item. Every recovery row persists its marker **before** acting — an in-context counter would not survive the operator's compaction/restart boundary, and a restarted operator must be able to distinguish an already-recovered item from a first observation. `merge-auto` has a one-PR sequence: "halt-dependents-only" degenerates to disarm-this-PR + escalate, and the queue's next spawn (which waits on the verified merge) stays deferred until the user acts — today's behavior.
...
4. **Act** — for each source item that passes: run the §6 spawn sequence with the item's `scope.repo` as the target repo, rendering the appropriate initial skill invocation (e.g., `fab-new` with argument `DEV-123`), tracked with `--stop-stage <scope.stop_stage>` and `--spawned-by <id>` (a `scope.repo` with no `fab/` takes §6 Working a Change's no-`fab/` + task cell instead — raw prompt, no `--stop-stage`, still `--spawned-by <id>`; source-driven spawns always carry a task and are never bare); then record the observe — `fab operator track observe <id> --json '{"new":[…]}' --seen <item-id>…` (`--seen` appends idempotently, only after a successful spawn or a deliberate skip).
...
| "Fix the broken brew line in c7-sahil87's README" (the repo has no `fab/`) | spawn per §6 Working a Change → no-`fab/` + task cell (plain agent in a fresh worktree, raw task + PR instruction, no skill prefix), then `fab operator track add c7-sahil87-<wt> --kind pane --pane %N --repo /home/x/code/c7-sahil87 --session work --branch <wt>` (the id is the worktree name, here prefixed with the repo for readability; `--branch` is the branch wt created, so `branch_map` keeps the pair after the pane item is removed); once the PR URL is known, chain `fab operator track add c7-sahil87-<wt>-pr --kind github-pr --scope '{"repo":"/home/x/code/c7-sahil87","pr":N}' --check-every 2m` and `track rm` the pane item |
| "Start an agent in a new worktree" · "open me an agent in ~/code/loom" (no task, fab or not) | bare spawn per §6 Working a Change (no prompt token, no PR instruction), then `fab operator track add <wt> --kind pane --pane %N --repo /home/x/code/loom --session work --branch <wt>`; completes on pane death / agent exit / `track rm`, no chained item — the operator asks at most which repo, never for a task |
...
| Cadence | rk cron operator-tick entry — union predicate (schedule + `wake_on: agent-state-change`), seeded by fab's clock reconcile when the server has none (§4 The Clock); the `track` verbs mute/unmute it via the tracked predicate and re-derive its schedule from the tracked set via `rk cron edit` only on change; lease = bounded snooze, bounded cadence override = `track clock --for` (§4 The Clock, §4 Mute and Lease); live cadence rendered from `fab operator clock sync` (§2 Init step 5, §4 Status Frame Format); quiet ticks render the one-line compact frame (§4 Status Frame Format); tick payload is the bare `operator tick` — never a slash command (§4 Tick Payload) |
