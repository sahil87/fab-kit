...
6. **Open agent tab** — open the composed command per `_cli-agents.md` § Spawn Composition ("Open it in a pane", incl. the one-prompt/no-`&&`-chaining rule), with the operator's window-marker name: `tmux new-window -n "»<wt>" -c <worktree-path> "<spawn_cmd> '<command>'"` (where `<wt>` is the worktree name from step 2 and `<spawn_cmd>` is the target repo's command from step 5)
   ...
   > **A `{prompt}`-carrying session provider is UNSUPPORTED for this spawn.** The positional form above is a no-op for such a provider (agy among the built-ins): `fab agent --print` has already dropped the placeholder pair, and no fab launcher composes an operator worker's arbitrary `/command` prompt — the mechanism is `_cli-agents.md` § Spawn Composition's `{prompt}`-carrying-provider bullet. If the target repo's `agent.session` names one, either point it at a placeholder-free provider (claude, codex) for operator spawns or skip the spawn and tell the user; do not hand-assemble a substitute. Change `3oz7`'s readiness-gate/send-keys delivery redesign removes spawn-time prompt embedding and closes this.
