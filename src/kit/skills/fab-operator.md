...
| Coordinating-agent model | Doing tier — `fab operator` resolves `fab resolve-agent apply` (canonical doing-tier stage), injects the profile via `spawn.WithProfile` (**appends** `--model`/`--effort` for a plain Claude `spawn_command`; **substitutes** into a `{model}`/`{effort}` template for a non-Claude one); falls back to the built-in `{claude-opus-4-8, high}` on any failure (incl. no resolvable `fab/` project) |
