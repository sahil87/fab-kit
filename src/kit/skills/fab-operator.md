...
The operator needs full command vocabulary to make routing decisions (e.g., knowing a fresh idea needs `/fab-new` → `/fab-fff` in a fab project — fab-new creates the branch inline — or a plain-agent spawn in a repo with no `fab/` (§6 Working a Change form 4), while a mis-aligned tab needs `/git-branch` first).
...
- **Pipeline-first** — new work **in a fab project** (the target repo has a `fab/` directory) MUST enter through `/fab-new`, then `/fab-fff`, `/fab-ff`, or `/fab-continue`; never send raw implementation instructions to a fab-project agent or use `/fab-continue` to skip intake. A repo with **no** `fab/` is exempt only from the pipeline — there is none there — never from delegation: its work goes to a plain agent in a fresh worktree (§6 Working a Change form 4). The operator **never runs `fab init`** to manufacture a pipeline — bootstrapping fab into a repo is the user's repo-structure decision; the operator MAY mention it once in a report as an option, never picks it, never offers it as a menu. Direct actions are exactly the §1 maintenance allowlist.
- **Spawn in a worktree** — reserve the operator pane for orchestration. Every pipeline command, including a one-line change — and every form-4 plain-agent spawn — starts with `wt create --non-interactive` (§2 wt Gate) and runs in a fresh agent tab.

A spawn that deliberately parks early — e.g. a `/fab-ff` run, which stops after hydrate — MUST be tracked with `--stop-stage hydrate`; otherwise the item never completes and sits in the list until the user stops it. A form-4 plain-agent spawn has no stages and takes no `--stop-stage`; its completion is the chained `github-pr` item (§6 Working a Change form 4).
...
4. **Activate the change pointer (existence-guarded)** — in the **just-created worktree's directory**, set that worktree's own `.fab-status.yaml` so the worktree is self-describing after the pipeline completes (a bare `fab`/`/fab-*` later resolves the change without naming it). Run the switch **only when the change folder already exists** — `fab resolve --folder <change>` succeeds iff a non-archived change folder matches:
   ...
   **Guard:** switch only an already-existing change, from the just-created worktree CWD, and fail soft. Raw/backlog forms wait for `/fab-new` Step 10; the non-fab form 4 has no change and skips the switch entirely (the guard's `fab resolve` fails in a repo with no `fab/`, which is the intended fail-soft); the dedicated worktree owns its own pointer, and the embedded transient override preserves correctness if activation fails.
...
8. **Track the item** — unconditionally and silently, one command (the `branch_map` pair rides the `track add`; contract in `_cli-fab-operator.md` § fab operator track). The id is the change id when known, else the worktree name from step 3; raw-text spawns are tracked at spawn and their change appears later as a `changed` delta; a form-4 spawn never acquires a change — its id stays the worktree name and its completion is the chained `github-pr` item:
...
> **Pipeline-first routing (§6 The pane Kind and Spawn Rules):** forms 1–3 below MUST go through the fab pipeline (`/fab-new` then a pipeline command for new work; the appropriate stage for already-intaked changes) — never raw implementation instructions to a fab-project agent pane. Form 4 is the non-fab exception: it still spawns an agent in a fresh worktree, but there is no pipeline to enter, so its prompt is the raw task.
...
4. **Non-fab repo → plain agent:** when the target repo has no `fab/` directory (probe `test -d <target-repo>/fab` against the main-worktree root from step 1), the work request still goes to an agent in a fresh worktree through the same sequence — steps 1–2 as usual; step 3's `wt create --non-interactive --worktree-name <name>` with **no branch argument** (there is no change branch; wt puts the worktree on a new branch of the same name, on any git repo); step 4's existence guard trivially skips; step 5 dependencies as usual; step 6's `fab agent default -o yaml --repo <target-repo>` degrades to the project-free config cascade (`_cli-fab-operator.md` § fab agent) and its `command` is still the session command; step 7 opens the tab with the **raw task text as the prompt, followed by the closing instruction `Commit on this branch, push, and open a draft PR against the default branch.`** — it is NOT a skill invocation: no `skill_prefix`, nothing to render per `_cli-agents.md` § Skill Prompts. Step 8 tracks it unconditionally as a `pane` item with no change, item id = the worktree name, and pass the branch wt created — also `<wt>` — so the `{branch, repo}` pair lands in `branch_map[<wt>]` and survives the pane item's later removal (`fab operator track add <wt> --kind pane --pane <pane-id> --session <session> --repo <repo> --branch <wt>`); there are no stages, so `--stop-stage` does not apply — do not pass it. **Completion is a chained `github-pr` item**: once the PR URL is known (the agent's pane shows it, or the user relays it), run `fab operator track add <wt>-pr --kind github-pr --scope '{"repo":"<repo>","pr":<n>}' --check-every 2m` and `fab operator track rm <wt>`; the user may `track rm` the pane item at any time. **Never `fab init` the repo** — the rule and its rationale are owned by §6 The pane Kind and Spawn Rules → Pipeline-first.

On completion (forms 1–3): PR ready, optionally archive; form 4 completes through its chained `github-pr` item. Forms 2 and 3 use `/fab-new` to generate a proper intake with traceability. `/fab-new` captures the raw input in the intake's Origin section — the user just says "fix [description]" and the operator does the rest.
