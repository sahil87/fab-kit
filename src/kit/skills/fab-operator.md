...
2. **Establish target session** — the tmux session the new agent window lands in, pinned at step 7. Candidates: `rk mux sessions --json` rows (the `result` array in run-kit's envelope form) with `role: "user"`, minus the operator's own session. First deciding rung wins: (a) the §8 "Spawn target session" setting, if user-set; (b) the sole candidate; (c) `path` equals the target repo root; (d) most panes whose `repo` (`fab pane map --all-sessions --json`) is the target repo; (e) highest `attached`, then first row. Zero candidates → error `nowhere to spawn` (attended: spawn output; unattended: §5), no spawn. Announce one line: session + deciding rung. Never trust a persisted `scope.session` alone; the ambient session is never an implicit target.
