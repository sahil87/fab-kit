...
**Recovery state.** The per-item budget lives on the `github-pr` item's `scope.recovery` (written by `track update --scope`, merged per key, durable across compaction — the items ARE the sequence state per rule 4); the `no-reviewer` re-send budget is per activation and needs no persisted field (the second idle-at-`review-pr` reads as a stuck agent under §3). `merge-auto` has a one-PR sequence: "halt-dependents-only" degenerates to disarm-this-PR + escalate, and the queue's next spawn (which waits on the verified merge) stays deferred until the user acts — today's behavior.
