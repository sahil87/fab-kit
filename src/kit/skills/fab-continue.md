...
> **Dispatch shorthand (apply / review / hydrate rows).** In the rows above, "`fab resolve-agent <stage> --alias` → dispatch the … sub-agent" is the Sub-agent dispatch contract from Normal Flow Step 1: surface the resolved `model=/effort=/dispatch=`, then **branch on `dispatch=`** — absent ⇒ native Agent-tool dispatch (the two seams), present ⇒ the CLI adapter (`fab dispatch`, whose worker mode auto-resolves — pane inside tmux, headless outside — so no mode flag is passed by default; `--pane`/`--headless` only to force one; and whose poll carries the bounded **recovery policy** — one automatic restart on `orphaned`, peek-on-suspicion, then escalate — per `_preamble.md` § CLI-Adapter Dispatch). The dispatched block carries the block-contract carve-out (no `fab status` transition commands; terminal `fab status refresh`; return results only) — **unchanged on the pane path, where a user may steer the worker mid-stage: steering is contract-neutral**. The `finish`/`fail`/`reset` shown in each row is the **sequencer's** post-return transition, not the block's.
>
