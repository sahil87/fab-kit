...
5. Read the result and run the row's `finish`/`fail`/`reset` transition in this sequencer.

Ship and review-pr are the local delta: they resolve their roles, then delegate full `/git-pr` and `/git-pr-review` behaviors — branching on `dispatch:` key presence exactly like the contract above (native arm through the two model/effort seams; `dispatch:`-present arm per `_preamble.md` § CLI-Adapter Dispatch — mirroring `/fab-fff` Steps 4–5). Those skills manage their own stage transitions on every arm, so their dispatch prompts omit the block-contract transition prohibition (the carve-out is owned by `_preamble.md` § Dispatch-Prompt Obligations).
...
| `hydrate` | `active`/`ready` | `fab agent hydrate -o yaml` → dispatch the hydrate sub-agent → on success run `finish <change> hydrate fab-continue` |
| `ship` | `active` | *(`ready` is unreachable — ship's AllowedStates are `{pending, active, done, skipped}`.)* `fab agent ship -o yaml` (surface the resolved YAML, then branch on `dispatch:` presence — native arm: the two seams; `dispatch:` present: the CLI adapter per `_preamble.md` § CLI-Adapter Dispatch — mirroring `/fab-fff` Step 4) → Execute `/git-pr` behavior **with the resolved change as the explicit `<change>` argument** (`/git-pr {name}` — transient override; its branch guard verifies the checked-out branch) → git-pr finishes ship internally (its Step 4b); only if the stage is still `active` after it returns, run `finish <change> ship fab-continue` (auto-activates review-pr) |
| `review-pr` | `active` | *(`ready` is unreachable — review-pr's AllowedStates are `{pending, active, done, failed, skipped}`.)* `fab agent review-pr -o yaml` (surface the resolved YAML, then branch on `dispatch:` presence — mirroring `/fab-fff` Step 5, same rule as the ship row) → Execute `/git-pr-review` behavior **with the resolved change as the explicit `<change>` argument** (`/git-pr-review {name}` — same transient-override + branch-guard contract) → it routes all terminal paths through its Step 6 and runs its own transitions. Pass/no-reviews: only if the stage is still `active` after it returns, run `finish <change> review-pr fab-continue`. Timeout (Copilot review requested but not yet available): the stage is deliberately left `active` — report and stop, no re-finish (first timeout in the activation only; a second consecutive timeout on the same PR is a `no-reviews` outcome with reason `review-gate-unavailable` — report its `summary`). Fail: only if the stage is still `active` after it returns (its Step 6 normally runs `fail` itself), run `fail <change> review-pr` |
...
6. **Regenerate indexes**: run `fab docs-index docs/memory` to regenerate the root (domains-only), domain, and sub-domain indexes — never hand-edit index rows. The hand-managed `manual` block in generated primary landings is likewise preserved verbatim, not edited; its rule is owned by `_cli-fab.md` § fab docs-index. On any merge conflict in a generated `docs/memory/**/index.md` or `log.md`, do **not** hand-merge: resolve the topic files, re-run `fab docs-index docs/memory`, take its output wholesale (FKF §5, `$(fab kit-path)/reference/fkf.md`).
   - **Refuse-before-regen guard (defense-in-depth)**: before that regen, consult `fab docs-index docs/memory --check`; on **exit 2** (destructive loss) refuse to regenerate and surface the pointer `→ run /docs-reorg-memory to remediate ...` (the orchestrator that relocates tombstone rows and dispatches `/docs-hydrate-memory` backfill mode for descriptions; backfill alone does not relocate tombstones). For first-run adoption semantics, consult `_cli-fab` § fab docs-index; retain the guard for subsequent navigation losses.
   - **Shape SHOULD guidance**: aim for ~5–12 files/folder, depth ≤3, introduce a sub-domain only for a cohesive ≥8-file cluster; `_shared/` and `_unsorted/` are width-exempt. Heed any non-fatal shape warnings `fab docs-index docs/memory` prints (advisory only).
