...
>
> **CLI-dispatched review worker (nesting degradation)**: when the sequencer dispatched *this* review block via the CLI adapter (`dispatch=` present), the nested inward/outward resolution above happens **inside** the CLI worker, on a harness where Agent-tool sub-agent support may be absent. Per `_review.md` § Parallel Dispatch → Nesting degradation, the worker then runs the inward + outward reviewers + merge **sequentially inline in one context** instead of as parallel sub-agents — **only the concurrency degrades; the outcome contract (same merged findings + verdict) is identical**. The CLI-path review dispatch prompt carries this degradation instruction (a cross-harness worker may never read fab's skill files beyond the prompt).
