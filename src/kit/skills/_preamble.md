...
- Output is a byte-stable `model=<id>` stdout line, then optional `effort=<level>` and `provider=<name>` lines (each omitted when its field is empty), **plus an optional `dispatch=<command>` line** emitted when the resolved provider carries a `dispatch_command` (the CLI-dispatch opt-in — its absence means native Agent-tool dispatch, with NO fallback to a session command) — **or**, when `dispatch.watchable: true` and the orchestrator sits inside tmux, for a `session_command`-only provider (the **watchable pane opt-in**: tmux presence decides pane vs native, and the line then carries the substituted `session_command`, which is what pane mode composes anyway). **Dispatch-seam skills branch on `dispatch=` presence** (see § CLI-Adapter Dispatch below): absent ⇒ the native Agent-tool path (`model=`/`effort=` consumed through the two seams below, byte-preserving); present ⇒ the CLI adapter (`fab dispatch`), where the profile rides the `dispatch=` command itself. See `_cli-fab.md` § fab resolve-agent.
- **Empty model** ⇒ omit the dispatch `model` param entirely (inherit the orchestrator/session model). **Empty effort** ⇒ omit the effort instruction (below).
- The resolver maps `<stage>` → its fixed fab-owned **role** → a `{provider, model, effort}` profile, then looks up the provider's `{session_command, dispatch_command}` in the top-level `providers:` table (the `dispatch_command` drives the optional `dispatch=` line — its absence means native Agent-tool dispatch unless the `dispatch.watchable` opt-in applies). The provider comes from an `agent.profiles.<role>.provider` override, else the role's **depth knob** (`agent.session` for the Tier-1 roles `default`/`operator`, `agent.workers` for the Tier-2 roles `doing`/`review`/`hydrate`/`fast`), else the built-in claude; model and effort come per field from an `agent.profiles.<role>` field, else that provider's `profiles.<role>` fill, else its `profiles.default` fill, else empty. There is NO cross-role inheritance on the agent side. The stage→role mapping and the role→depth partition are NOT user-overridable; the two knobs, `agent.profiles` (per-role override), and `providers` (grammar + fills) are the override surfaces. Full design: `docs/specs/stage-models.md`.
...
**User-directed per-stage overrides ride the SAME single call — and bind the NATIVE arm only.** When the user directs a provider/model/effort for specific stages ("run review on codex", "apply on sonnet this once"), the dispatch site adds the override flags to its **existing single** `fab resolve-agent <stage> --alias` call — `--provider <name>` and/or `--model <id>` / `--effort <level>` (see `_cli-fab.md` § fab resolve-agent for the flags and the fill precedence). Nothing else about the seam changes: still one resolve call per stage, still the two seams below for a native dispatch, still the branch on `dispatch=` presence, still the compliance-visibility obligation (surface the resolved lines — an override makes *what* was resolved even more worth showing). **There is no new dispatch machinery and no persistent state**: an override is per-invocation, so "use codex for the next N stages" means passing the same flags on those N resolve calls. **An invocation-time override reaches only the native Agent-tool arm, and CANNOT move a stage onto CLI dispatch**: `fab dispatch start` takes no override flags — it re-resolves the stage from config itself — so an overridden profile whose `dispatch=` line appeared would be silently discarded there (the headless arm errors on the unoverridden provider's missing `dispatch_command`; `--pane` composes the unoverridden provider's `session_command`). Treat a `dispatch=` line that appears **only because of an override** as not actionable, and note that the two remedies are **not interchangeable**: dispatching the stage natively with the overridden model/effort is executable only for a **within-claude** `--model`/`--effort` override (the Agent tool's `model` param is a Claude-alias enum — `opus`/`sonnet`/`haiku`/`fable` — so a non-Claude model has no native seam to ride), which makes a **config override** (the `agent.workers`/`agent.session` knob, or `agent.profiles.<role>.provider`) the **sole executable path for a cross-provider `--provider` override** — it is what `dispatch start`'s own re-resolution sees. One asymmetry worth knowing: swapping to a **built-in** refills from that provider's own shipped `providers.<name>.profiles` (all three carry one), so only a provider with no fills anywhere resolves an **empty** `model=`, which the seams read as "inherit the session model"; pair that case with an explicit `--model`, or give that provider per-role fills, when the stage should run a specific model.
...
- **Effort → an explicit instruction in the subagent prompt.** The Agent tool has **no `effort` parameter**, so the only available seam for the effort half is the dispatched prompt itself: append an imperative line such as ``Operate at `high` reasoning effort for this task.`` so the sub-agent self-selects its reasoning effort. Empty effort ⇒ omit the instruction entirely (mirroring the empty-model omit rule). This is imperfect — and worse than imperfect in practice: **the instruction is not reliably honored, because the session-level effort dominates** (a known Claude Code limitation, GitHub issues #64033/#39220), so treat the effort half as **advisory on this arm and binding only where `--effort` rides a composed CLI command** (`fab dispatch` headless/pane, the operator launcher). Model differentiation works on both arms; see `docs/specs/stage-models.md` § Effort asymmetry. It remains the only per-sub-agent effort seam available today; a per-sub-agent `effort` parameter on the Agent tool would close it cleanly and is tracked as the residual harness ask (`docs/findings/per-stage-model-tier-application.md` § Suggested directions item 4).

**Compliance visibility.** Each dispatch site MUST **surface the resolved `model=/effort=` lines** — carry them into the dispatch prompt (the effort line is already there per the seam above; co-locate the model line) and/or emit them in the orchestrator's own step output — so a *skipped* `fab resolve-agent` call (the sub-agent silently inherits the session profile) or a mis-resolved role is **visible in output rather than silent**. There is no code-level guard fab can install (dispatch is harness-internal), so visibility is the available seam. An all-empty resolution (both `model=` and `effort=` empty) is itself worth surfacing — treat a non-empty resolved line as the expected case and a fully-empty result as a signal to flag rather than dispatch blind.

**Harness-adapter boundary (Claude Code).** The resolution (stage→role→`{model, effort}`) is **provider-neutral**. Injecting the resolved model into the actual dispatch is **harness-specific**: for Claude Code that adapter is the **Agent tool's `model` parameter**, and the effort half is injected via the subagent-prompt instruction described above. One concrete harness detail: the Agent tool's `model` param takes a **short alias** (`opus` / `sonnet` / `haiku` / `fable`), **not** the full versioned id (`claude-opus-4-8`) the plain command emits. So for Agent-tool dispatch, **resolve the model half with `fab resolve-agent <stage> --alias`** — the `--alias` flag emits the Agent-tool-valid short alias directly on the `model=` line (empty ⇒ omit/inherit; a non-Claude override passes through verbatim), so no agent ever hand-maps the id. This is named explicitly as the Claude-Code adapter, not as universal truth. *(The operator launcher path is the deliberate exception: it resolves the **operator**-role profile WITHOUT `--alias` — `spawn.WithProfile` composes a `claude` **CLI** invocation, which accepts full IDs. `WithProfile` is grammar-forgiving: when the resolved provider's `session_command` is a **template** containing `{model}`/`{effort}` — including the built-in claude default, which is templated, as well as a codex `session_command` — it **substitutes** the resolved values into the template (all-or-nothing; an empty value drops the placeholder's token and a preceding `-`-flag); when the command carries **no placeholder** it appends `--model <full-id> --effort <level>`. Placing the built-in claude default's placeholders last preserves that token order, so a non-Claude worker CLI is configurable without the launcher emitting Claude-only flags. See `stage-models.md` § Skill wiring.)*

**Per-stage selection applies on every post-intake stage.** Per-stage selection is a property of dispatched sub-agent runs — and **every post-intake stage now dispatches a sub-agent**, including plain `/fab-continue` (which is a one-stage sequencer that resolves `fab resolve-agent <stage>` and dispatches its stage's block — see `fab-continue.md` Normal Flow Step 1). So there is no post-intake foreground execution path left to be the exception; `fab resolve-agent` applies uniformly across apply/review/hydrate regardless of caller. Intake is pre-boundary (it runs in the main session and gets no per-stage profile from `/fab-continue`). The only residual "advisory" case is a stage skill genuinely run with no dispatch at all — fab cannot switch the session model mid-run, so such a skill MAY note "this stage is configured for X; you're on Y" but MUST NOT attempt to switch. *(The effort half of the profile is now injected via the subagent-prompt instruction described above — see "The two halves dispatch through different seams"; the lone remaining residual is a first-class per-sub-agent `effort` parameter on the Agent tool, which is a harness ask outside fab's control, not a fab gap.)*

### CLI-Adapter Dispatch (the `dispatch=` path)

This is the **canonical** cross-harness dispatch procedure. Dispatch sites (`_pipeline.md`, `fab-continue.md`, `fab-adopt.md`) **reference this subsection** and do NOT restate the five-state machine or the pane-mode option. The full cross-adapter contract — three adapters (native Agent-tool / headless CLI / interactive pane), with the per-adapter delivery mechanism and observable-state subset — is `docs/specs/harness-adapters.md`; the runtime is `_cli-fab.md` § fab dispatch.
...
- **`dispatch=` absent** ⇒ **native Agent-tool dispatch** — the two seams above (model on the Agent `model` param, effort via the prompt instruction). This is the default for every fab-kit built-in role (all of which resolve to claude, which carries no `dispatch_command`).
- **`dispatch=` present** ⇒ the **CLI adapter** (`fab dispatch`). There is **NO fallback** to a session command (a provider's `session_command` is a separate, independent field; the absence of a resolved provider `dispatch_command` is itself the native-dispatch signal). The choice is per-stage/per-role: one pipeline run can mix native and CLI dispatches across stages.

**The watchable pane opt-in (`dispatch.watchable`) adds a second reason the line can be present.** With `dispatch.watchable: true` in config (scope `both`, so it is settable once machine-wide in `~/.fab-kit/config.yaml`; default `false`), a provider carrying only a `session_command` — the built-in `claude`, which ships no `dispatch_command` — also emits `dispatch=` **when the orchestrator itself sits inside tmux** (`$TMUX` set). **Tmux presence is what decides pane vs native**: outside tmux the line is omitted and the stage stays on native Agent-tool dispatch (never headless CLI — headless stays gated on a real `dispatch_command`). A provider's own `dispatch_command` always **wins**; watchable only ADDS eligibility for providers that have none. Dispatch sites still branch on presence and never execute the line's value — `fab dispatch start` re-resolves internally, and inside tmux its auto ladder selects **pane** mode and composes the `session_command` itself, which is exactly what the emitted line shows. *(Edge: if tmux dies between the resolve and `fab dispatch start`, start's auto ladder soft-falls-back to headless and then errors on the missing `dispatch_command` — rare and self-explaining at the CLI.)*
...
1. **Start.** `fab dispatch start <change> <stage>` with the full stage prompt on **stdin** — the same block prompt the Agent tool would receive, composed per § Dispatch-Prompt Obligations below. `start` resolves the role → provider → `dispatch_command` internally and launches it detached. **The wiring passes no `--timeout` on `start`** — the flag exists on `start`/`restart` (a POSIX `timeout` inside the headless wrapper that KILLS the worker; see `_cli-fab.md` § start), but the pipeline sets no default bound, because orphan detection, peek-on-suspicion, and `fab dispatch kill` cover the failure modes. Pass one only on an explicit directive. *(Do not confuse it with `wait --timeout` in step 2, which bounds the OBSERVER and never touches the worker.)*
2. **Wait (push, not poll).** `fab dispatch wait <change> <stage> --timeout 300` — a **blocking** call that returns the moment the dispatch's state leaves `running` (re-derived on `wait`'s internal ~2s tick), or when the bound expires. Do **not** loop `fab dispatch status` on a `sleep`: every poll is a full inference turn, so a long stage costs hundreds of turns observing that nothing happened.
   - **Preferred — run it as a BACKGROUND command** (the harness's notify-on-exit seam; in Claude Code, Bash `run_in_background`). The orchestrator then does other work or ends its turn, and the harness re-invokes it when the command exits, carrying the printed state. Zero turns until something actually happens.
   - **Fallback (degraded, still 10×)** — a harness with no notify-on-exit background command runs the **same** `wait --timeout 300` as a plain **foreground blocking call**: one turn per 5 minutes instead of ten. Same command, same flags; only the harness seam differs.
   - **Read the printed state string.** A terminal state → step 3's handling below. `running` → the bound expired: that wake **IS** the peek-on-suspicion moment (§ Recovery policy → *Peek on suspicion*) — peek, classify (a/b/c), then re-arm a fresh `wait`. Timeout expiry exits **0**; only real errors (no dispatch record, unresolvable change) exit non-zero.
   - **`--timeout 300` carries the five-minute peek cadence, not a poll interval.** Change it only on an explicit directive.
3. **Five-state handling** (the state `wait` printed; `fab dispatch status` remains the one-shot probe for a spot check) — a dispatch that does not reach `done` gets **bounded recovery** rather than an unconditional stop, per § Recovery policy below:
   - `running` → the wait timed out: take a read-only peek (§ Recovery policy → *Peek on suspicion*) and re-arm `wait`, since a wedged or parked worker reads `running` forever.
   - `done` → read `.fab-dispatch/{4-char-change-id}/{stage}-result.yaml` as the block's returned result, then run **`fab dispatch reap <change> <stage>`**, and proceed with the normal sequencer transition (finish/fail per the stage's contract). A review `verdict: fail` **inside** a `done` result is a **review outcome**, not a dispatch failure — the orchestrator takes the normal review-fail path.
     - **The reap call is unconditional and dumb — make no mode check and no config check.** A pane-mode worker never exits on completion (it writes its result and sits at its prompt), so without reaping every finished stage keeps its slice of the carved worker column for the rest of the run and the panes you actually watch shrink with each completed stage. The **whole guard lives in Go**: `reap` acts only when the record is pane-mode **and** the derived state is `done` **and** `dispatch.reap_done` (default `true`) resolves true — so a headless record is a reported no-op (the process already exited) and the knob is read through the three-layer config cascade, which a skill reading `fab/project/config.yaml` directly would get wrong (it would miss the machine-wide system layer). Every no-op reports its reason and **exits 0**; only a missing dispatch record or an unresolvable change is an error. See `_cli-fab.md` § fab dispatch → *reap*.
   - `failed` → infrastructure/worker failure (a non-zero exit, incl. `124` timeout) — NOT a review-verdict fail: surface `fab dispatch logs <change> <stage> --tail N`. **No automatic restart**; the orchestrator MAY spend the single restart budget on a *clearly transient* failure it can read in that log tail (§ Recovery policy), else stop per the stage's failure path.
   - `failed (no-result)` → **contract violation** (clean exit, no result file); **NEVER treat as done** and **never restart** — surface logs and escalate (§ Recovery policy → *Escalation*).
   - `orphaned` → the worker died with no recorded exit (reboot / `kill -9` / crash / pane killed): **run ONE automatic `fab dispatch restart <change> <stage>`** if the budget is unspent, then **re-arm `wait`**; if the budget is spent, escalate (§ Recovery policy). `wait`'s liveness tick surfaces this in ~2s rather than at the next poll, so the restart fires almost immediately.
4. **No STATE cleanup after `done`.** `.fab-dispatch/` is transient comms with **no automatic GC** — state cleanup is archive-time deletion + explicit `fab dispatch clean` only, and the wiring adds no state-cleanup call after a `done` dispatch. The step-3 `fab dispatch reap` call is **not** a counter-example: reap is **pane hygiene** — it kills a finished pane-mode worker's tmux pane to reclaim visual space and **removes no files at all**. The record, result, prompt, and log all survive a reap (which is exactly why a reaped dispatch still reads `done` forever).

**Recovery policy — bounded, and the orchestrator's alone.** Tier 2 (orchestrator → stage worker) has one recovery verb: **restart**, never a nudge. A stage dispatch is not a session — fab checkpoints stage state into artifacts (`plan.md` task checkboxes, the result-file contract, idempotent stages), so a restarted worker resumes from the last `[x]` and loses almost nothing. `fab dispatch restart <change> <stage>` relaunches from the prompt `start` already persisted (`{stage}-prompt.md`), so the orchestrator does **not** need the block prompt still in context, and it **re-derives the mode from the current environment** — restarting an `orphaned` pane dispatch after a tmux server death correctly lands headless.

- **Budget: exactly ONE restart per stage dispatch, tracked in THIS orchestrator's context.** Nothing is written to disk — no attempt counter in `{stage}.yaml`, no attempt history (last-attempt-only is preserved). After a context loss the worst case is one extra restart, which is acceptable. Do not re-derive the budget from the record; there is nothing there to read.
- **`orphaned` spends it automatically** (a death with no recorded exit is transient by nature), after which the orchestrator re-arms `wait`. **`failed` does not**: a deterministic failure (bad config, a real test failure, a `124` timeout) would loop. Because the orchestrator is an agent rather than a script, it MAY read the `fab dispatch logs --tail N` output and, on a *clearly transient* signature (provider 5xx / overload / rate-limit exhaustion), spend the **same single** budget on one restart. Anything it cannot positively read as transient stops as before. **`failed (no-result)` never restarts** — a contract violation needs eyes.
- **Peek on suspicion.** A worker parked at an error banner or waiting on a prompt is indistinguishable from one working hard: both read `running`. On **every timeout-return of `fab dispatch wait`**, take a **read-only** peek: `fab dispatch logs <change> <stage> --tail 40` (headless) or `fab pane capture [-L <server>] <pane>` (pane; take the exact command from `fab dispatch logs`, which prints it socket-included) — and classify three ways:
  - **(a) visibly progressing** → re-arm `wait`, peek again on its next timeout-return.
  - **(b) parked at an error banner / dead-ended** → `fab dispatch kill <change> <stage>` then restart, **within the same single budget** (escalate instead if it is spent).
  - **(c) waiting on genuine human input** (a permission prompt, a question) → **escalate without killing**. A human may want to answer, and steering stays human.
- **Escalation** (budget spent, or a `failed (no-result)`, or classification (c)): surface the evidence for the dispatch's mode (`fab dispatch logs <change> <stage> --tail N` headless / `fab pane capture [-L <server>] <pane>` pane), send `rk notify` **gated on `command -v rk`** and fail-silent per § Run-Kit (rk) Reference, and stop per the stage's existing failure path. Escalation adds no new state and no new transition — it is the same stop the unconditional rows used to take, now reached only after recovery is exhausted.
- **The pipeline NEVER sends keys to a worker.** Its verb set is exactly **peek** (read-only), **kill**, **restart**, **notify**, **stop**, **reap**. Nudging and answering are the operator's and the user's affordances: send-keys delivery is documented-flaky (the printed-prompt trap, the probe-then-retype choreography) and a pipeline nudge channel would fork the cross-adapter contract, since a native worker has no such channel. Classification (c) notifies a human; it never types.
  - **`reap` is NOT `kill`, and it is not a recovery verb at all.** `kill` is recovery — valid in any state, spent by classification (b) within the single restart budget. `reap` is **hygiene**: it fires only on `done` (step 3's success path, never from this policy), is knob-gated on `dispatch.reap_done`, and can never terminate a `running`, `orphaned`, `failed`, or `failed (no-result)` dispatch. Nothing in this Recovery policy changes because of it.

**Pane mode — an OPTION WITHIN this branch, never a third branch.** A CLI-dispatched stage may run **interactively in a tmux pane** the user can *watch and steer* ("run review where I can watch") instead of as a detached headless process. When the dispatching agent is itself a tmux pane on the target server, that worker pane is **split into the agent's own window** (stacking down a right-hand column carved at `dispatch.column_width`, Claude-teams style); otherwise it opens as a new window named `fab-{id}-{stage}`. The branch keys on `dispatch=` presence, and the mode is a per-invocation choice inside the `dispatch=`-present arm (there is no resolver output for it, and no provider config field: pane mode composes the resolved provider's existing `session_command`, the same string `fab agent` composes). The full contract is `docs/specs/harness-adapters.md` § 3. Interactive-pane adapter; the runtime is `_cli-fab.md` § fab dispatch. What the wiring needs:

- **When to pass a mode flag**: usually **never** — `fab dispatch start`'s mode **defaults to auto** and resolves itself: **pane inside tmux** (`$TMUX` set — the run-kit context, where a pane is visible to the user) and **headless outside**. So a dispatch from a session the user is watching is already watchable without asking. Pass a flag only to *override* that: **`--pane`** to force a pane worker (e.g. on an explicit watch-and-steer directive from a dispatcher that is itself headless), or **`--headless`** for an unattended run that merely *happens* to live inside a tmux tab. The two are mutually exclusive (usage error). Forcing `--pane` **requires both a reachable tmux server and a `session_command` on the resolved provider**, and is a hard error without either, so it must never be added speculatively; an **auto**-selected pane instead **soft-falls-back to headless** with a one-line stderr notice for whichever prerequisite is missing — an unreachable server (a stale `$TMUX`) or a `dispatch_command`-only provider — so auto never breaks an unattended dispatch and never regresses a provider that only ever dispatched headless. The `dispatched …` output line names the selection source (`auto: tmux` / `auto: no tmux` / `auto: tmux unreachable` / `auto: no session_command`) whenever auto fired — surface it like the rest of the resolved profile.
- **Steps 1–2 apply to both modes**: the same block prompt goes in on **stdin** (`start` persists it and hands the worker a one-line pointer to the prompt file — a delivery-mechanism detail, not a prompt-content change), and the same background `fab dispatch wait <change> <stage> --timeout 300` observes it. `wait` derives a pane dispatch's state from the same result-file-plus-pane-liveness rule `status` does.
- **Step 3 handles a SUBSET of three states**: `running` / `done` / `orphaned`. `failed` and `failed (no-result)` are **unreachable on the pane path** — an interactive worker never exits on task completion, so there is no exit-code channel and a crashed or killed worker collapses into `orphaned`. The state **strings are byte-identical**; a pane dispatch simply never emits those two. Handle `done` and `orphaned` exactly as above — **including the reap call and the recovery policy**. `done` inherits step 3's `fab dispatch reap` verbatim, and **this is the path it exists for**: a pane worker never exits on completion, so its pane is the one that would otherwise linger in the worker column (on the headless path the same unconditional call is simply a reported no-op). An `orphaned` pane dispatch gets the same **one automatic restart** from the same single budget, and because `restart` re-derives its mode, a pane dispatch orphaned *by* a dead tmux server soft-falls-back to headless instead of re-failing.
- **Peek on suspicion is where the pane path pays off most**: a worker parked at an error banner or sitting on a permission prompt is exactly what tmux scrollback shows and an exit code never would. Same cadence (every timeout-return of `wait`), same three-way classification — and classification (c) is *especially* live here, since a pane worker can genuinely be waiting for the human watching it.
- **`logs` has no pane analogue**: a pane worker's output is tmux scrollback, not `{stage}.log`. So both the peek and the escalation evidence on this path read the worker's pane with `fab pane capture [-L <server>] <pane>` instead of `fab dispatch logs` — pane-ID keyed, so identical in both pane shapes. **Include `-L <server>` whenever the dispatch was started with `--server`** — a pane on a non-default socket is invisible to a default-socket capture. Don't hand-assemble it: `fab dispatch logs <change> <stage>` on a pane dispatch **prints the exact command**, socket included when the record carries one (the pane ID alone is also in `fab dispatch status --json`, which does not carry the socket).
- **Steering changes no contract**: a user typing into the worker's pane is human input into the worker's context, exactly like answering a native sub-agent's question. The worker still owes `{stage}-result.yaml` and its terminal `fab status refresh`, and still runs no `fab status` **transition** command — **the orchestrator still owns every transition**. A worker steered away from producing a result needs no new state: it never reaches `done` and surfaces through the never-`done` escalation the orchestrator already owns — concretely, the peek-on-suspicion classification in § Recovery policy, which sees a steered worker as (a) progressing or (c) awaiting a human and in neither case kills it.
- **Two-tier tmux hierarchy — you do NOT get a window per worker.** An **operator** opens worktree agents as tmux **windows**. A **worktree agent** dispatching its own stage workers gets them as **panes splitting its own window**, stacked in a right-hand column: `start` splits the last live sibling worker pane (`-v`, unsized) or, when there is none, **carves** the column off the agent's own pane (`-h -l <n>%`, `dispatch.column_width` — default 35, so the agent you are watching keeps the rest). Sibling detection reads the **dispatch records' pane IDs**, not pane titles (a harness rewrites its worker's title within seconds), so the **column invariant** holds: the left/right separator is created once and never touched again — no `select-layout`, no rearranging your panes, no fighting your resizes. Placement is cosmetic and degrades warn-only — every failure is surfaced on stderr, naming both what failed and where the worker landed (a failed `list-panes`, or an unreadable record tree, still carves a sized column; an unreadable *single* record keeps the rest and can still stack; a tmux without `-l <n>%` retries unsized). The **new-window shape is the fallback** for a dispatcher with no pane of its own to split (`$TMUX_PANE` unset, e.g. a headless orchestrator passing `--pane`) and for `--server <name>` (that socket may not be the one the caller's pane lives on). Nothing here needs a flag: the shape resolves from the environment, and `restart` re-derives it the same way. The `dispatched …` line reports which shape ran (`pane %N, split, title fab-…` vs. `pane %N, window fab-…`).
- **Not an operator enrollment**: the dispatch's identity string is `fab-{4-char-change-id}-{stage}` — carried by the **pane title** in the split shape and the **window name** in the new-window shape — and carries no `»`/`›` operator marker in **either**: a pipeline dispatch borrows tmux, it does not join the operator's monitored set.
...
Confidence scoring provides a numeric measure of how well-resolved a change's decisions are, used as the single intake gate for fast-forward pipeline execution via `/fab-ff` and `/fab-fff`. Scoring reads `intake.md` as its sole source.
...
**Intake gate**: threshold **3.0 for all seven change types** (flat). `CheckGate` obtains it via `getGateThreshold(changeType)`, so future per-type divergence is a data-only change.
