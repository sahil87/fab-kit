...
**Allowed values**: `_generation`, `_review`, `_cli-fab`, `_cli-fab-operator`, `_cli-fab-pane`, `_cli-external`, `_cli-agents`, `_srad`, `_pipeline`, `_intake`.
...
The structured output carries the resolved `provider`, full `model`, `model_alias`, `effort`, composed `command`, per-field `source`, and optional `dispatch:` mapping. `model_alias` is the native Agent-tool alias for recognized Claude model IDs and is empty for non-Claude IDs; `dispatch:` is absent exactly when the native rung resolves and otherwise carries the labelled `rung` plus the composed `command`. The resolver's role/depth/fill precedence and the YAML schema live in `_cli-fab-operator.md` § fab agent. Resolved strings pass through verbatim — fab validates no effort enum and corrects no incompatible pair.

| Profile half | Native Agent-tool seam | Empty value | Constraint |
|--------------|------------------------|-------------|------------|
| Model | Agent tool `model` parameter, using the YAML `model_alias` | Omit the parameter for an empty Claude profile; inherit the orchestrator/session model. An empty alias for a non-Claude model has no native seam | The parameter accepts the Claude aliases `opus`/`sonnet`/`haiku`/`fable` |
...
User-directed overrides are per invocation and ride the same single `fab agent <stage> -o yaml` call:
...
- **Profile fixity.** A resumed worker keeps the model and effort it was first dispatched with; `fab agent apply -o yaml` is **NOT** re-run on the resume path. Resolution runs — and is surfaced — only on **fresh** dispatches, initial or fallback. Re-resolving on a resume would surface a value the cycle cannot honor, which is the opposite of what the surfacing rule is for.
...
#### Pane-arm continuation (the `dispatch:`-present branch)
...
4. **Profile fixity** holds identically: a resumed pane keeps the model and effort it was opened with, and `fab agent apply -o yaml` is NOT re-run on the resume path.

### CLI-Adapter Dispatch (the `dispatch:`-present path)

This is the canonical cross-harness dispatch procedure. Dispatch sites (`_pipeline.md`, `fab-continue.md`, `fab-adopt.md`) reference it instead of restating the machine; `_cli-fab-pane.md` § fab dispatch owns runtime details.

Branch once, at the single surfaced `fab agent <stage> -o yaml` result:

| `dispatch:` key | Branch | Profile handling |
|-----------------|--------|------------------|
| Absent | Native Agent-tool dispatch; the resolver selected the native rung | Use the model/effort seams above |
| Present | CLI adapter (`fab dispatch`); stages may mix adapters | The mapping carries the selected pane/headless `command` with full model ID and substituted effort, so do not apply the native seams, execute the value, or resolve again |

`dispatch.mode` is a preference ceiling over `pane → native → headless`. The resolver starts at that rung and descends only, using provider capabilities (`interactive_command`, `native`, `headless_command`) plus `$TMUX`; command presence describes how a rung runs and never selects policy by itself. The result still has only the two branches above: the `dispatch:` key is absent iff native resolved and present with the already-substituted pane/headless command otherwise. Dispatch sites branch only on key presence and never execute `dispatch.command`; `_cli-fab-operator.md` § fab agent owns the ladder and no-rung errors.

When `dispatch:` is present:

1. **Launch, by rung.** The two modes have different entries, because a pane worker is spawned and delivered to in separate steps; the surfaced YAML names the selected entry at `dispatch.rung`, so **branch on that label directly**. Defense-in-depth, not discovery: a mislabelled or stale-environment landing cannot launch wrong — `fab dispatch start` refuses a pane landing cleanly, naming `open` — and `_cli-fab-pane.md` § fab dispatch owns that refusal's mechanics.
   - **`rung: headless` — `fab dispatch start`.** Send the full stage prompt on stdin to `fab dispatch start <change> <stage>`. Pass no `--timeout` by default: `start`/`restart --timeout` bounds and KILLS the headless worker; `wait --timeout` in step 2 bounds only the OBSERVER. Use a start timeout only on explicit direction.
   - **`rung: pane` — `fab dispatch open`.** Run § The pane readiness gate below — `open` → gate → `deliver` — then continue at step 2. **Remember this landing**: `rung: pane` is the pane arm, and step 3's deferred apply reap fires only here.
