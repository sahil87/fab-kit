...
**Pane mode — an OPTION WITHIN this branch, never a third branch.** The user may ask for a stage they can *watch and steer* ("run review where I can watch"). On the CLI-adapter path that is `fab dispatch start <change> <stage> --pane`, which runs the worker **interactively in a tmux window** instead of as a detached headless process. The **branch itself is unchanged** — it still keys on `dispatch=` presence, and `--pane` is a per-invocation option inside the `dispatch=`-present arm (there is no resolver output for it, and no provider config field: pane mode composes the resolved provider's existing `session_command`, the same string `fab agent` composes). The full contract is `docs/specs/harness-adapters.md` § 3. Interactive-pane adapter; the runtime is `_cli-fab.md` § fab dispatch. What changes for the wiring:

- **When to pass it**: only on an explicit user directive for a watchable/steerable stage. Unattended pipelines keep the headless default — `--pane` **requires a reachable tmux server** and is a hard error without one, so it must never be added speculatively.
