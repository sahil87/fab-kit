...
- **The pipeline NEVER sends keys to a worker.** Its verb set is exactly **peek** (read-only), **kill**, **restart**, **notify**, **stop**. Nudging and answering are the operator's and the user's affordances: send-keys delivery is documented-flaky (the printed-prompt trap, the probe-then-retype choreography) and a pipeline nudge channel would fork the cross-adapter contract, since a native worker has no such channel. Classification (c) notifies a human; it never types.
