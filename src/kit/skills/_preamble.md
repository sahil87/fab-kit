...
- **`fab/project/config.yaml`** — project configuration: identity (name/description), `source_paths`/`test_paths`, true-impact excludes, plan-acceptance extra categories, the `providers:` table (per-provider `session_command`/`dispatch_command` plus the optional `model`/`effort` default fill), `agent.tiers` (the six role tiers), `dispatch.watchable` (the watchable-pane opt-in) and `dispatch.column_width` (the pane-worker column's width), optional `stage_hooks`
...
- The resolver maps `<stage>` → its fixed fab-owned tier → a `{provider, model, effort}` profile, then looks up the provider's `{session_command, dispatch_command}` in the top-level `providers:` table (the `dispatch_command` drives the optional `dispatch=` line — its absence means native Agent-tool dispatch unless the `dispatch.watchable` opt-in applies; `agent.tiers` / `providers` project overrides per-field-merged over fab-kit's defaults, with `default`-tier inheritance). The stage→tier mapping is NOT user-overridable; `agent.tiers` (tier redefinition) + `providers` (command grammar) are the override surfaces. Full design: `docs/specs/stage-models.md`.
