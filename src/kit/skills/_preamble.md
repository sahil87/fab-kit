...
- **`dispatch=` absent** ⇒ **native Agent-tool dispatch** — the two seams above (model on the Agent `model` param, effort via the prompt instruction). Unchanged; byte-preserving in behavior. This is the default for every fab-kit built-in role (all of which resolve to claude, which carries no `dispatch_command`).
...
**The watchable pane opt-in (`dispatch.watchable`) adds a second reason the line can be present — the branch is UNCHANGED.** With `dispatch.watchable: true` in config (scope `both`, so it is settable once machine-wide in `~/.fab-kit/config.yaml`; default `false`), a provider carrying only a `session_command` — the built-in `claude`, which ships no `dispatch_command` — also emits `dispatch=` **when the orchestrator itself sits inside tmux** (`$TMUX` set). **Tmux presence is what decides pane vs native**: outside tmux the line is omitted and the stage stays on native Agent-tool dispatch (never headless CLI — headless stays gated on a real `dispatch_command`). A provider's own `dispatch_command` always **wins**; watchable only ADDS eligibility for providers that have none. Dispatch sites need **no branch change**: they still branch on presence and never execute the line's value — `fab dispatch start` re-resolves internally, and inside tmux its auto ladder selects **pane** mode and composes the `session_command` itself, which is exactly what the emitted line shows. *(Edge: if tmux dies between the resolve and `fab dispatch start`, start's auto ladder soft-falls-back to headless and then errors on the missing `dispatch_command` — rare and self-explaining at the CLI.)*
...
2. **Wait (push, not poll).** `fab dispatch wait <change> <stage> --timeout 300` — a **blocking** call that returns the moment the dispatch's state leaves `running` (re-derived on `wait`'s internal ~2s tick), or when the bound expires. Do **not** loop `fab dispatch status` on a `sleep`: every poll is a full inference turn, so a long stage costs hundreds of turns observing that nothing happened.
   ...
   - **Read the printed state string.** A terminal state → step 3's handling below, unchanged. `running` → the bound expired: that wake **IS** the peek-on-suspicion moment (§ Recovery policy → *Peek on suspicion*) — peek, classify (a/b/c), then re-arm a fresh `wait`. Timeout expiry exits **0**; only real errors (no dispatch record, unresolvable change) exit non-zero.
   - **`--timeout 300` carries the peek cadence, not a poll interval** — it reproduces the former "every 10th poll at 30s" = 5 minutes exactly. Change it only on an explicit directive.
...
- **Peek on suspicion.** A worker parked at an error banner or waiting on a prompt is indistinguishable from one working hard: both read `running`. So on **every timeout-return of `fab dispatch wait`** — a `running` state printed after the `--timeout 300` bound expired, i.e. the same ~5-minute cadence the former "every 10th poll at 30s" gave — take a **read-only** peek: `fab dispatch logs <change> <stage> --tail 40` (headless) or `fab pane capture [-L <server>] <pane>` (pane; take the exact command from `fab dispatch logs`, which prints it socket-included) — and classify three ways:
