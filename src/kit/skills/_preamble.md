...
1. **Start.** Send the full stage prompt on stdin to `fab dispatch start <change> <stage>`. Pass no `--timeout` by default: `start`/`restart --timeout` bounds and KILLS the headless worker; `wait --timeout` in step 2 bounds only the OBSERVER. Use a start timeout only on explicit direction.
2. **Wait (push, not poll).** Run blocking `fab dispatch wait <change> <stage> --timeout 300`, preferably as a background command through the harness notify-on-exit seam; use the same call in the foreground when that seam is unavailable. `wait` re-derives state on its internal ~2s tick, returns immediately on a terminal state, and prints `running` when its five-minute peek bound expires. Timeout expiry exits **0**; only real errors are non-zero. Never replace it with a `sleep`/`status` poll loop, and change the peek cadence only on explicit direction.
3. **Handle the printed state.** Use the table below. On `done`, read the result and run `fab dispatch reap <change> <stage>` unconditionally — no mode or config check. The Go-side guard acts only for a pane record in `done` with `dispatch.reap_done` true; every no-op reports why and exits 0, while a missing record or unresolved change errors.
...
| `done` | Result exists; review `verdict: fail` remains a review outcome | Read `.fab-dispatch/{4-char-change-id}/{stage}-result.yaml`, reap, then take the normal sequencer transition |
...
Escalation surfaces per-mode evidence, sends `rk notify` only behind the fail-silent `command -v rk` gate, and stops on the existing failure path; it creates no state or transition. **The pipeline NEVER sends keys to a worker.** Its verb set is exactly **peek** (read-only), **kill**, **restart**, **notify**, **stop**, **reap**. Nudging and answering remain operator/user actions. **`reap` is NOT `kill`**: kill is ungated recovery; reap is `done`-only, knob-gated hygiene that cannot terminate `running`, `orphaned`, or `failed` work.
