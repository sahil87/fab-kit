...
3. **Five-state handling** (observed via `fab dispatch status`) — a dispatch that does not reach `done` gets **bounded recovery** rather than an unconditional stop, per § Recovery policy below:
   ...
   - `orphaned` → the worker died with no recorded exit (reboot / `kill -9` / crash / pane killed): **run ONE automatic `fab dispatch restart <change> <stage>`** if the budget is unspent, then resume polling; if the budget is spent, escalate (§ Recovery policy).
...
- **`orphaned` spends it automatically** (a death with no recorded exit is transient by nature). **`failed` does not**: a deterministic failure (bad config, a real test failure, a `124` timeout) would loop. Because the orchestrator is an agent rather than a script, it MAY read the `fab dispatch logs --tail N` output and, on a *clearly transient* signature (provider 5xx / overload / rate-limit exhaustion), spend the **same single** budget on one restart. Anything it cannot positively read as transient stops as before. **`failed (no-result)` never restarts** — a contract violation needs eyes.
- **Peek on suspicion.** A worker parked at an error banner or waiting on a prompt is indistinguishable from one working hard: both read `running`. So on every **10th result-less poll** (~5 min at the fixed 30s cadence) take a **read-only** peek — `fab dispatch logs <change> <stage> --tail 40` (headless) or `fab pane capture [-L <server>] <pane>` (pane; take the exact command from `fab dispatch logs`, which prints it socket-included) — and classify three ways:
  - **(a) visibly progressing** → keep polling, peek again in 10.
...
- **Steps 1–2 are unchanged**: the same block prompt still goes in on **stdin** (`start` persists it and hands the worker a one-line pointer to the prompt file — a delivery-mechanism detail, not a prompt-content change), and the same fixed `sleep 30` poll of `fab dispatch status` applies.
...
- **Peek on suspicion is where the pane path pays off most**: a worker parked at an error banner or sitting on a permission prompt is exactly what tmux scrollback shows and an exit code never would. Same cadence (every 10th result-less poll), same three-way classification — and classification (c) is *especially* live here, since a pane worker can genuinely be waiting for the human watching it.
