...
- **`fab/project/config.yaml`** — project configuration: identity (name/description), `source_paths`/`test_paths`, true-impact excludes, plan-acceptance extra categories, the `providers:` table (per-provider `session_command`/`dispatch_command` plus the optional `model`/`effort` default fill), `agent.tiers` (the six role tiers), `dispatch.watchable` (the watchable-pane opt-in), optional `stage_hooks`
