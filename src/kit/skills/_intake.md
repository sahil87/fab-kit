...
- **Backlog ID** (4-char — embedded in the folder-name prefix): `fab resolve --id {id} --or-none` — then compare its stdout (the canonical 4-char ID of the matched change, or `(none)` when nothing matches) for **equality** with `{id}`. `(none)` ⇒ no existing change for this ID (it can never equal a 4-char ID, so the compare naturally rejects it). Only an exact ID match names an existing change for this ID; resolution is substring-based, so `{id}` occurring inside another change's *slug* also resolves — with a different canonical ID — and MUST NOT route to resume. On an exact match, get the folder name via `fab resolve --folder {id}`.
