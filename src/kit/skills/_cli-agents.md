...
3. **Have the worker announce itself** — instruct the worker in its prompt to run a notification as its last act (e.g. `rk notify` — fail-silent per `_preamble.md` § Run-Kit (rk) Reference). This converts polling into an event, at the cost of depending on the worker honoring the instruction.
