...
> **Not every provider has a session form.** Composition needs an `interactive_command`, and all four built-ins ship one — `claude`, `codex`, `agy`, and `kimi`. Either form errors actionably against a custom provider with none, naming the `providers.<name>.interactive_command` key to set.
...
**Pane-capable.** The agy built-in ships both command fields, making it eligible for interactive sessions and pane-mode dispatch. Its interactive first-run behavior (a trust prompt on a fresh workspace, even under `--dangerously-skip-permissions`) is cleared by an ordinary readiness-gate judgment round, just like kimi's. The trust store can additionally be user-seeded (`~/.gemini/antigravity-cli/settings.json`, exact-match) if desired, but the gate suffices.
