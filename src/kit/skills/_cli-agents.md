...
The `fab` commands referenced here (`fab agent`, `fab pane open`/`ready`/`deliver`, `fab pane capture`, `fab pane map`) are documented in `_cli-fab.md` — load it too when you need their exhaustive flag surface. Messaging into an existing agent pane rides `rk mux send`/`rk mux await` (tool-owned contract: `rk skill`; fab-owned usage summarized in § Pre-Send Validation and § Await below), with a raw-`tmux` fallback when rk is absent.
