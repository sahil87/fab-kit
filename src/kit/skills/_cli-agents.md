...
**The third placeholder: `{prompt}`.** An `interactive_command` MAY carry `{prompt}` alongside `{model}`/`{effort}`, marking **where the initial prompt is delivered**. It follows the same substitute-or-drop rule, but it is resolved on a *different* axis — not by the profile, by whether the launch **carries an initial prompt at all**:

| Launch | `{prompt}` resolves to | Result for the agy built-in `agy --dangerously-skip-permissions --model {model} -i {prompt}` |
|--------|------------------------|-------------------------------------------------------------------------|
| **Promptless** — bare `fab agent` alone | empty ⇒ the token and its preceding `-`-flag drop | `agy --dangerously-skip-permissions --model <id>` — a plain session |
| **Prompt-carrying** — `fab operator`, `fab batch new`/`switch`, `fab dispatch start --pane` | the **shell-quoted** prompt | `agy … --model <id> -i '/fab-operator'` · `-i 'Read <path> and execute it.'` |

Every prompt-carrying launch runs the *same* two-shape delivery rule, in one implementation: substitute the shell-quoted prompt at `{prompt}` when the command declares one, **append** it as a trailing quoted positional when it does not. A command carrying **no** `{prompt}` (claude, codex, anything written before the placeholder existed) therefore composes byte-for-byte as it always has. Use the placeholder when the CLI's initial-prompt flag **takes a value** — agy's `-i`/`--prompt-interactive` does, so a bare positional is discarded and a bare trailing `-i` hard-errors (`flag needs an argument`); the placeholder is the only shape that serves both launches from one field. `{prompt}` does **not** affect `{model}`/`{effort}` handling: a command carrying only `{prompt}` still gets `--model`/`--effort` appended.
...
- **`{prompt}`-carrying providers need the placeholder form instead.** `fab agent --print` composes a *session* command, so it has already dropped the `{prompt}` token pair — appending a positional to that output would hand agy an argument it discards. For a provider whose `interactive_command` declares `{prompt}` (agy among the built-ins), **let a fab launcher compose the window command** — `fab dispatch start --pane` for a stage worker, `fab batch new`/`switch` for a change worker, `fab operator` for the operator itself. Each delivers its own prompt through the shared two-shape rule, so the placeholder is still standing when the prompt arrives; hand-assembling from `--print` output is the one path that cannot recover it. **Where no launcher covers the spawn, the spawn is UNSUPPORTED for such a provider** — the operator's per-change worker spawns (`fab-operator.md` step 6) are today's case: they embed an arbitrary `/command` prompt in a hand-assembled `tmux new-window`, and no launcher composes that. Point the target repo's `agent.session` at a placeholder-free provider (claude, codex) for those spawns until change `3oz7`'s readiness-gate/send-keys delivery redesign lands, which removes spawn-time prompt embedding entirely. There is deliberately no `fab agent --print` mode that emits an undropped placeholder.
...
> **Pipeline consumer**: `fab dispatch start <change> <stage> --pane` (the interactive-pane dispatch adapter — `_cli-fab.md` § fab dispatch, contract in `docs/specs/harness-adapters.md`) is built on exactly this procedure. It composes the resolved provider's `interactive_command`, opens it with the form above, and — because a multi-thousand-token stage prompt cannot ride argv or send-keys — embeds a **one-line pointer** to a prompt file as the single quoted argument — substituted at `{prompt}` when the command declares one, appended positionally otherwise, shell-escaped per the rule above either way (the pointer names a repo-derived path) — applying § Await's "prefer asking for an artifact over a screen pattern" rule to completion detection too (the worker's `{stage}-result.yaml`).
...
- **Built-ins:** `claude`, `codex`, `agy`, and `kimi` use the independent `interactive_command` / `headless_command` / `native` capability grammar in `internal/agent`'s embedded `defaults.yaml`; all resolve without a `providers:` block. Claude ships all three capabilities; codex and agy are non-native (pane + headless); kimi alone is headless-only (no `interactive_command`, so no pane capability). `dispatch.mode` chooses the starting rung of the descending `pane → native → headless` ladder.
...
| Aspect | Value |
|--------|-------|
| Interactive | `agy [-i "<prompt>"]` — `-i`/`--prompt-interactive` opens the TUI with an initial prompt. It is a **value-taking** flag: a bare `-i` exits 2 (`flag needs an argument`), and a bare positional prompt is silently discarded. fab's built-in `interactive_command` is `agy --dangerously-skip-permissions --model {model} -i {prompt}` (see § Pane capability, below) |
...
| Approvals | `--dangerously-skip-permissions` on both of fab's shipped forms, interactive and headless — unattended stage workers cannot answer approval prompts. It does **not** cover the once-per-worktree workspace-trust dialog; see the trust-wall caveat below |
...
**Pane capability, via `{prompt}`.** The agy built-in ships **both** commands, so `fab agent --provider agy` / `fab operator` open a session and `dispatch.mode: pane` inside tmux selects the pane rung — no user config needed. The interactive grammar is:

```yaml
interactive_command: 'agy --dangerously-skip-permissions --model {model} -i {prompt}'
```

The `{prompt}` placeholder (§ Spawn Composition) is what makes one field serve every launch. A prompt-carrying launch would otherwise append its prompt as a **positional argument**, which agy silently discards — the TUI would open empty with nothing to read. `-i` fixes that but **takes a value**, so a static trailing `-i` would hard-error the promptless launch instead. With the placeholder, `fab operator` / `fab batch` / pane dispatch each substitute their shell-quoted prompt at `-i`, and the one promptless launch (bare `fab agent`) drops the `-i {prompt}` pair.

**Trust-wall caveat (once per worktree).** agy gates a **fresh workspace** behind an interactive **trust dialog** — `Do you trust the contents of this project?` — even under `--dangerously-skip-permissions`. A pane-dispatched agy worker in a not-yet-trusted worktree therefore **parks at that dialog** until a human answers it; the pipeline's wait-timeout peek classifies this as "waiting on genuine human input" and escalates *without* killing the worker. The `-i` prompt is deferred, not eaten — it lands as the first user message once the dialog is answered. The trust store is `~/.gemini/antigravity-cli/settings.json` → `trustedWorkspaces`, an array of **EXACT-match absolute paths** (a trusted parent does *not* cover a child directory) with no CLI verb to add entries; **fab does not seed it**. Answer it once per worktree, or point stage roles at a different provider for fresh worktrees.
...
**Dispatch-only — the sole dispatch-only built-in.** The kimi built-in ships a `headless_command` and **no `interactive_command`**. Pane-mode dispatch delivers the worker's prompt-file pointer through that command — substituted at a `{prompt}` placeholder, or appended as a **positional argument** when there is none (§ Spawn Composition) — and kimi can receive it **neither way**: it parses a bare positional as a **SUBCOMMAND** and exits non-zero (`unknown command '…'`), and it has **no interactive-initial-prompt flag at all** (`-p` is non-interactive; upstream issue #2240 tracks the gap), so there is nothing for a `{prompt}` placeholder to target. That is the difference from agy, whose value-taking `-i` the placeholder does target. Shipping an `interactive_command` for kimi would only make pane selection reach a mode that cannot work. Without one, automatic resolution descends to headless (`descended: pane unavailable: no interactive_command`) and explicit `--pane` hard-errors.
...
**Caveat**: doing so re-enables pane-mode eligibility, and a kimi pane worker handed a pointer *exits immediately* rather than idling — no grammar you can write there delivers the prompt. Pair it with `--headless` on those dispatches, or point stage roles at a different provider.
