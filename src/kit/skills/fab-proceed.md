...
> **Per-stage model (prefix steps resolve a role too)**: each prefix-step dispatch resolves a **role by name** immediately before dispatching, then applies the resolved profile through the two seams per `_preamble.md` § Subagent Dispatch → Per-Stage Model Resolution — the model on the Agent tool's `model` param (via `--alias`), the effort as an imperative prompt line (empty ⇒ omit either). The resolver accepts a role name positionally (the same path `fab agent <role>` uses), so this needs no Go change — it is skill wiring only. The roles: **`/fab-switch` and `/git-branch` → `fab resolve-agent fast --alias`**; the **`_intake` create-intake dispatch → `fab resolve-agent default --alias`** (this closes the one intake path that is a dispatched subagent yet formerly resolved no role; intake itself stays advisory-only on the foreground `/fab-new` path). Each dispatch **surfaces** the resolved `model=/effort=` (compliance visibility) so a skipped/mis-resolved role is visible rather than silent. This keeps the invariant that a dispatched step resolves the same role regardless of caller; the final `/fab-fff` invocation resolves `fab resolve-agent <stage>` for each of its own pipeline stages as before.
...
2. **Resolve the role**: run `fab resolve-agent default --alias` and surface the resolved `model=/effort=` (compliance visibility). Apply through the two seams — model on the Agent tool's `model` param (empty ⇒ omit/inherit), effort as an imperative prompt line (empty ⇒ omit).
...
1. **Resolve the role**: run `fab resolve-agent fast --alias` and surface the resolved `model=/effort=` (compliance visibility). Apply through the two seams — model on the Agent tool's `model` param (empty ⇒ omit/inherit), effort as an imperative prompt line (empty ⇒ omit).
