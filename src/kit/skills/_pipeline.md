...
3. **Re-dispatch apply (resume-first)**: if the native-arm apply worker `apply-{id}` from **this** orchestrator session is reachable, **continue it** — send the item-2 rework instructions to `apply-{id}` per `_preamble.md` § Worker Continuation (which owns the naming, block-contract restatement, profile-fixity, and fallback rules). Otherwise run the Stage Dispatch Procedure for `apply` with the Step 1 target — today's behavior verbatim, and the mandatory fallback in every unreachable case `_preamble.md` § Worker Continuation enumerates. Either way, on success run `fab status finish <change> apply {driver}` — this auto-activates review (review → `active`), the **one** counted transition that advances `stage_metrics.review.iterations` for this cycle (`status.go:627`). Re-entering review here via `finish apply` (not `reset review`, not any non-`active` path) is what makes the cycle count truthfully; this `finish apply` MUST run every cycle, even when item 2 was a trivial fix.
...
**Worker release**: once review passes (item 5's `finish review`) or the loop stops at exhaustion, the orchestrator STOPS continuing `apply-{id}` — never message it again. Release is passive: no teardown call exists or is needed, the handle is simply never used. Hydrate (Step 3) and every later stage always dispatch a fresh worker.
