...
**LIGHT lane — task execution inline.** Execute `/fab-continue` Apply Behavior's Task Execution sub-step inline in this orchestrator's own context per § Light Lane (no dispatch, no `fab resolve-agent`).
