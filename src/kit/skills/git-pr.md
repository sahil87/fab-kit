...
Evaluate this ladder in order; first match wins:

| Rung | Source | Condition | Result |
|------|--------|-----------|--------|
| 1 | Explicit argument | Argument is a valid type, case-insensitive; non-types are Step 0's `<change>` | Normalize to lowercase |
| 2 | `.status.yaml` | `{has_fab}` and `{change_type}` is a valid non-null type | Use `{change_type}` |
| 3 | `fab/changes/{name}/intake.md` | `{has_fab}` and `{has_intake}`; case-insensitive first keyword match | `fix` for fix/bug/broken/regression; `refactor` for refactor/restructure/consolidate/split/rename; otherwise `feat` |
| 4 | First non-empty of `git diff --name-only HEAD`, `git diff --name-only --cached`, `git diff --name-only @{u}..HEAD` (when upstream exists) | Fallback over uncommitted, staged, or unpushed paths | Empty → `chore`; all CI → `ci`; all docs/non-code Markdown → `docs`; all tests → `test`; otherwise `chore` |
...
1. Run `fab docs-index docs/memory` (byte-stable; writes only `docs/memory/` index + log files; a no-op when nothing drifted).
...
4. If the regen OR the commit fails → report the error and STOP. The 3a content commit is already made and intact; a failed refresh leaves a benign stale `log.md`, recoverable by re-running `fab docs-index docs/memory` — never a torn state.

> **Never hand-merge a generated index/log (FKF §5).** If a rebase/merge during ship produces a conflict in a generated `docs/memory/**/index.md` or `log.md`, do **NOT** hand-resolve the generated file. Resolve the conflict in the **topic files** (and any `.status.yaml` / `log.seed.md` seed inputs) only, then re-run `fab docs-index docs/memory` and take its output **wholesale** — the index/log is a pure function of content, so the regenerated bytes are authoritative. See `$(fab kit-path)/reference/fkf.md` §5.
...
```bash
lease_oid=$(git rev-parse -q --verify '@{u}' 2>/dev/null || true)   # the upstream tip as THIS checkout last saw it — captured BEFORE the fetch
git fetch origin
if [ -n "$lease_oid" ] && [ "$(git rev-parse '@{u}')" != "$lease_oid" ]; then
  : # the PR branch moved on the remote since this checkout last fetched → STOP (see step 2)
fi
...
1. Capture `{lease_oid}` — the upstream tip as this checkout last saw it — **before** fetching (empty when there is no upstream), then `git fetch origin`. If the fetch fails → report the git error output and STOP. The pre-fetch capture is load-bearing: the fetch also updates `origin/<branch>`, so a bare `--force-with-lease` in 3b would take the *just-fetched* tip as its lease and happily overwrite commits another actor pushed to the PR branch — the exact case the lease exists to refuse.
2. **Remote-branch divergence guard** — if `{lease_oid}` is non-empty and `git rev-parse '@{u}'` now differs from it, the PR branch gained commits from elsewhere (a reviewer's push, another session) that this rebase would replay over. STOP before rebasing, nothing rewritten:

   ```
   origin/{branch} moved since this checkout last fetched ({lease_oid}…→ {new tip}…).
   Integrate it first (git pull --rebase), then re-run /git-pr.
   ```

3. If `refs/remotes/origin/$base_branch` does not resolve → print the one-line warning above and continue to 3b without rebasing (a merged-and-deleted stacked base after Step 1's fallback already re-pointed `$base_branch` is the expected shape).
4. Otherwise `git rebase "origin/$base_branch"`. Using the **recorded** base — not the default branch — is what keeps a `stacked-prs` dependent on its dependency branch.
5. **On conflict**, classify each conflicting file:
...
6. Record whether a rebase ran (`{rebased}`) and keep `{lease_oid}` — 3b's push form depends on both.
...
3. If upstream exists and 3a-ter rebased (`{rebased}`): `git push --force-with-lease="$(git branch --show-current):$lease_oid"` — the rebase rewrote history, so a plain push would be rejected. The lease is the **explicit pre-fetch OID** from 3a-ter step 1, never the bare flag: after 3a-ter's fetch the remote-tracking ref already holds whatever the remote has now, so a bare `--force-with-lease` would accept a stranger's push as the lease and overwrite it. With the explicit OID, git refuses if the remote tip is anything other than what this checkout rebased from
...
3. **Generate PR body**: the `## Meta` block is rendered mechanically by `fab pr-meta`; `## Summary` and `## Changes` stay agent-generated (they require prose synthesis from the intake).
   ...
   **Render the `## Meta` block** only when `{has_fab}`. `fab pr-meta` owns the complete ordered block and its impact/provenance semantics (see `_cli-fab.md` § fab pr-meta); pass Step 0 `{name}`, Step 0b `{type}`, and Step 1 `{issues}`:
