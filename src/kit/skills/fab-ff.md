...
> **Per-stage model + adapter**: each stage dispatch in the bracket resolves once with `fab resolve-agent <stage> --alias`, surfaces the resolved `model=/effort=/provider=/dispatch=` lines, then branches on `dispatch=` presence per `_preamble.md` § CLI-Adapter Dispatch: absent ⇒ native Agent-tool dispatch through the two seams; present ⇒ the CLI adapter.
