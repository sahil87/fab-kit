...
**Fill precedence** — where the resolved `{model, effort}` comes from, most specific first:

```
invocation flag  >  explicit tier field  >  named provider's default fill  >  empty
```

The provider fill rung is `providers.<name>.model` / `.effort` (see § fab config → `providers`); `empty` keeps its existing meaning (an empty `model=` line = "inherit the session model"; on a `dispatch=` command the placeholder's token, plus a preceding `-`-flag, is dropped so the CLI's own default applies).

**Cross-provider fill (the footgun fix)**: when a tier **explicitly sets `provider:`** to a provider **NAME** differing from the built-in tier's, its unset `model`/`effort` fill from **that provider's** default fill and then empty — **never** from the built-in/default-tier values, which belong to the other provider name. The cutoff is **name-based, not vendor-based**: fab compares the two provider names and knows nothing about which vendor a name fronts, so a second entry naming the same vendor under a different name (say `providers.claude-alt`) also loses the tier's model/effort inheritance. A tier that sets no `provider:` inherits exactly as before, and the all-claude default world is byte-unchanged. So `agent.tiers.doing: { provider: codex }` with no `providers.codex.model` resolves an empty `model=` (not a claude model). Pin `model:` on the tier to be explicit.
...
- The `dispatch=` line is emitted when the resolved tier's provider carries a `dispatch_command` (the CLI-dispatch opt-in), mirroring the effort-omit rule — **or**, when `dispatch.watchable: true` and the orchestrator sits inside tmux, for a `session_command`-only provider (the **watchable pane opt-in**, below). Its **absence** is the signal for **native Agent-tool dispatch** — and a **headless** dispatch has **NO fallback to a session command** (a provider's `session_command` is a separate, independent field). The emitted command has its `{model}`/`{effort}` placeholders **already substituted** via `internal/spawn`'s template resolution (reused, not reimplemented). Consumed by the `fab dispatch` command family; dispatch-seam skills that only inject `model=`/`effort=` do not read it. *(`fab dispatch start --pane` composing the provider's `session_command` is not a fallback and not a resolver change: mode selection is per-invocation and `resolve-agent`'s output is identical either way — the pane mode reads the provider table itself, exactly as `fab agent` does. See § fab dispatch.)*
- **`dispatch.watchable` (the watchable-pane opt-in)** — a bool config field, **default `false`**, **scope `both`** (settable once machine-wide in `~/.fab-kit/config.yaml`). When `true` **AND** `$TMUX` is set **AND** the resolved provider carries a `session_command` but **no** `dispatch_command` (the built-in `claude`), the `dispatch=` line is emitted carrying the profile-substituted **`session_command`** — so the stage dispatches into a **watchable tmux pane** instead of a native sub-agent. **Tmux presence decides**: with `$TMUX` unset the line is omitted and the stage stays on **native Agent-tool dispatch**, never headless CLI (headless remains gated on a real `dispatch_command`). A provider's own `dispatch_command` always **wins** — watchable only ADDS eligibility for providers that have none, which is why it replaces the old "uncomment claude's `dispatch_command`" workaround (that also flipped every out-of-tmux dispatch to headless CLI). Dispatch-seam skills need **no branch change**: they branch on the line's *presence* and never execute its value, and `fab dispatch start` re-resolves internally — inside tmux its auto ladder selects pane mode and composes the same `session_command`.
...
# with the doing tier pinned to a dispatch-carrying provider
# (`agent.tiers.doing: { provider: codex, model: gpt-5.3-codex, effort: high }`) —
...
**Invocation-time overrides** — `--provider <name>`, `--model <id>`, `--effort <level>` are the top rung of the fill precedence above:

- **`--provider`** swaps the resolved provider and **re-derives `dispatch=` from the NAMED provider's `dispatch_command`** — so the emitted `dispatch=` presence can differ from the stage's unoverridden one. That is a **query result, not an adapter move**: `fab dispatch start` accepts no override flags and re-resolves the stage from config itself, so only a **config/tier** override (`agent.tiers.<tier>.provider` + a `providers:` entry) actually relocates a stage between native Agent-tool dispatch and CLI dispatch. An override-only `dispatch=` line is not actionable, and the two remedies are **not interchangeable**: dispatching the stage natively with the overridden model/effort works only for a **within-claude** `--model`/`--effort` override (the native seam is the Agent tool's `model` param, a Claude-alias enum), so for a cross-provider `--provider` override the config/tier override above is the **sole executable path** (see § fab dispatch and `_preamble.md` § Per-Stage Model Resolution). A swap does **not** retain the tier's `model`/`effort` (they belong to the old provider): an unoverridden field refills from the new provider's default fill, then empty — the same cross-provider rule tier resolution applies, and the same **name-based** comparison (a swap to a differently-named provider fronting the same vendor still drops the tier's model/effort). **Swapping *back* to `claude` from a non-claude tier lands on an empty `model=`**: claude's fill lives on the **built-in tiers**, not on the provider, so there is no `providers.claude.model` rung for the swap to refill from. So "run this one stage on claude" needs an explicit `--model` alongside `--provider claude` (or a `providers.claude.model` fill added to config) — otherwise the stage inherits the session model. (`--provider claude` against a tier that *already* resolves to claude is not a swap at all — nothing refills and the tier's model stands.)
- **`--model` / `--effort` are valid WITHOUT `--provider`** here — a within-tier override of the profile this pure query would otherwise print. **Deliberate asymmetry with `fab agent`**, where they stay a usage error without `--provider`: `resolve-agent` is a pure query whose whole output is a profile (overriding one field is unambiguous), while `fab agent` is a session launcher with two mutually exclusive addressing modes, where a bare `--model` would invent an undocumented tier-override surface. See § fab agent.
- All three guards key on whether the flag was **supplied** (not on value emptiness), so `--model=` explicitly clears the model (emitting the inherit signal) rather than being ignored.
...
# a bare within-tier override (valid here; a usage error on `fab agent`):
...
**Exit code**: non-zero only on a real error — an unreadable/malformed config, an unknown stage/tier name, or a supplied `--provider` that resolves to no provider. A stage/tier resolving to a default is success (exit 0).
...
**Full schema coverage**: covers BOTH the binary-consumed keys (modeled on the `Config` struct) AND the skill-consumed keys (read by markdown skills, invisible to Go reflection) — `project.*`, `source_paths`, `test_paths`, `true_impact_exclude`, `checklist.extra_categories`, `providers.*` (`session_command`/`dispatch_command`/`model`/`effort`), `agent.tiers.*` (`provider`/`model`/`effort`), `dispatch.watchable`, `stage_hooks.*`, `branch_prefix`. (The retired `review_tools` block moved to `fab/project/code-review.md` § Review Tools; `agent.spawn_command` moved to `providers.claude.session_command`; `fab_version` moved OUT of config.yaml to the plain-text sibling `fab/.fab-version` in 2.15.0 and is no longer a config-file key.) Baseline keys appear live with example values (including the `providers:` claude `session_command` and the six `agent.tiers`); the opt-in override blocks (`stage_hooks`, `branch_prefix`) appear commented-out with fab-kit's built-in defaults shown, so uncommenting is opting in.

The `providers:` block documents fab-kit's **three built-in providers** — `claude` (the default), `codex`, and `gemini` — as **grammar only**: their command templates live in the binary (`internal/agent`'s built-in provider table), so **naming `codex`/`gemini` in a tier or on a `--provider` flag resolves with no `providers:` block at all**. No built-in carries a `model`/`effort` fill: non-claude model IDs rot at CLI cadence, so the fill belongs in config (`providers.<name>.model`/`.effort`, scope `both` — settable once per machine in `~/.fab-kit/config.yaml`). The codex/gemini blocks render **commented** because they merely restate a built-in default, exactly like every other non-overridden default — a commented block registers no project override, and a built-in provider is inert until a tier or flag names it. Note codex/gemini DO carry a `dispatch_command` (so naming one flips that tier's stages to CLI dispatch) while claude's built-in deliberately does not (native Agent-tool dispatch; its commented `dispatch_command` line flips claude's stages the same way when uncommented). Gemini's commands carry no `{effort}` placeholder (the gemini CLI has no reasoning-effort flag) and no `-p` on `dispatch_command` (gemini reads the `fab dispatch` stdin-piped prompt in non-TTY mode).
...
|------|------------------------------------------------------------------------|--------|------------------|-------------------------|------|
| **headless** | `--headless`, `--timeout`, or auto **outside** tmux | detached `sh -c` process | the provider's `dispatch_command` | `{stage}.exit` + pid liveness + result file | never touched |
| **pane** | `--pane`, `--server`, or auto **inside** tmux | an interactive agent session in a tmux pane you can watch and steer — **split into your own window** when you are a tmux pane on the target server, else a new window | the provider's `session_command` | **result file** + pane liveness | **required** — as is a `session_command`; either prerequisite missing is a hard error when explicit, a soft fallback to headless when auto |

The two provider command fields are **never merged and never fall back to each other in either direction**. Headless mode stays **tmux-independent**; pane mode borrows tmux. **Neither is an unconditional default** — the mode is resolved per invocation by the explicit-first ladder in § start, whose last rung is **auto**: pane when `$TMUX` is set (the run-kit context, where a pane is visible to the caller), headless when it is not. Headless dispatch remains parallel to and independent of `fab pane` / `fab operator` (the interactive *operator-visibility* path); pane **dispatch** borrows tmux as a launch surface but does **not** join the operator's monitored set (see `--pane` below). **POSIX-only (v1)** — headless `start`/`kill` error clearly on Windows (`fab dispatch requires a POSIX shell (setsid/timeout); Windows is not supported in v1`) rather than half-working.
...
Resolves `<change>` → 4-char ID; reads the stage prompt on **stdin** → `{stage}-prompt.md`; resolves the stage's tier → provider internally (via `internal/agent` + `internal/spawn` `{model}`/`{effort}` substitution — the same resolution `fab resolve-agent` performs). **This re-resolution reads CONFIG ONLY — `start` has no `--provider`/`--model`/`--effort` surface**, so `fab resolve-agent`'s invocation-time overrides do NOT reach it: an override binds the **native Agent-tool arm** only, and moving a stage onto CLI dispatch requires a config/tier override (`agent.tiers.<tier>.provider` + a `providers:` entry) rather than a flag (see § fab resolve-agent → Invocation-time overrides). Everything up to the launch is shared by both modes; the launch differs.

**Mode selection = an explicit-first ladder, last rung auto.** Evaluated in order, the first match winning (each explicit rung keys on whether the flag was **supplied**, not its value, so `--timeout 0` / `--server ""` still count):
...
| 5 | *(none of the above)* | `$TMUX` set ⇒ **pane**; unset ⇒ **headless** | **auto** |

- **`$TMUX` is the DEFAULTING signal only** — it means "a pane opened without `-L` lands on the server the caller is attached to". It never replaces the tmux **reachability probe**, which stays the *validation* step once pane mode is chosen and is a real tmux query (so an explicit `--pane` still works from a headless orchestrator, where `$TMUX` is unset). An **empty** `$TMUX` reads as unset. **`$TMUX_PANE` is a separate signal** and does not affect this ladder at all — it selects the pane **shape** (split vs. new window, see `--pane` below) *after* pane mode has already been chosen.
- An auto-selected pane targets the **current** server — no `-L` is passed unless `--server` was given.
...
|---|-----------|-------|-----------|---------------------|
| 1 | `$TMUX_PANE` set **and** no `--server` | **split** — a pane inside the **dispatching agent's own window** | `tmux split-window {-h\|-v} -t <target> -c <repo-root> "<resolved-cmd> <shell-quoted-pointer>"` then `select-pane -T fab-{id}-{stage}` | the **pane title** |
...
- **Split placement = a stacked right column** (the Claude-teams layout). `start` probes the caller's window with `tmux list-panes -t "$TMUX_PANE" -F '#{pane_id} #{pane_title}'` and keeps the **last** pane whose title starts `fab-`. If one exists, the new worker splits **that** pane with `-v` (stacking down the column); if none does, it splits `$TMUX_PANE` itself with `-h` (creating the column). A **failing probe degrades** to the `-h` split off `$TMUX_PANE` with a stderr warning — placement is cosmetic and must never fail a dispatch.
- **The identity string is the same either way** — `fab-{id}-{stage}` — and is recorded in `{stage}.yaml`'s `window` field in **both** shapes, whose meaning is therefore "window name (new-window) or pane title (split)". No schema change, no migration. A **failed title set** is non-fatal (stderr warning only): the pane ID is the real identity and the worker is already running.
...
- **The pane path has TWO prerequisites — a reachable tmux server AND a `session_command` — and BOTH failures are asymmetric by SELECTION SOURCE.** Under an **explicitly** selected pane (`--pane`, or `--server` acting as the pane signal) either shape exits non-zero, launching nothing and writing no state — a caller who asked for watchability must not be silently downgraded: an unreachable server gives `pane mode requires a reachable tmux server, but <target> is unreachable; start tmux (or pass --server <name>), or pass --headless (drop --pane/--server) to dispatch headless` (the message names **pane mode**, not `--pane`, since `--server` alone can be the explicit signal), a missing `session_command` gives the usual `providers.<name>.session_command` config-key hint. Under **auto**, either shape instead **soft-falls-back to headless**, printing its one-line notice to **stderr** and proceeding with a headless launch that produces a headless-shaped record:
  ...
  | Shape | Trigger | stderr notice | output suffix |
  |-------|---------|---------------|---------------|
  | (a) | tmux unreachable (e.g. a **stale `$TMUX`** inherited from a killed server) | `pane auto-selection: tmux unreachable, falling back to headless` | `auto: tmux unreachable` |
  | (b) | tmux reachable, but the provider carries no `session_command` (a `dispatch_command`-only provider) | `pane auto-selection: provider has no session_command, falling back to headless` | `auto: no session_command` |

  The asymmetry exists so nothing that never asked for a pane can break: a stale `$TMUX` must not fail an unattended dispatch, and a `dispatch_command`-only provider must dispatch **identically inside and outside tmux** (its stages ran headless before auto selection existed). Pane-command composition is therefore **deferred until the mode is validated** — composing it first would hard-fail shape (b) before the fallback decision. The fallback re-composes from `dispatch_command`, so the no-cross-fallback rule survives the mode change (a provider carrying **neither** field still errors, with the usual `dispatch_command` config-key hint). Reachability is a real tmux query, **not** an `$TMUX` env read, so a headless orchestrator can dispatch into a socket it names explicitly.
- **`--server <name>` / `-L <name>`** targets a tmux socket (`tmux -L <name>`), mirroring the `fab pane` family's flag; defaults to `$TMUX` / tmux's default socket. Persisted in the record so `status`/`kill` reach the same server without re-supplying it. **Implies pane mode** under auto (rung 4 above) — naming a socket while meaning headless is incoherent — and is **ignored in headless mode** (headless touches no tmux).
- **`--pane` + `--timeout` is a usage error** (`--pane and --timeout are mutually exclusive: --timeout is enforced by the headless launch wrapper (POSIX timeout), which pane mode does not use`) — pane mode builds no wrapper, so accepting the flag would advertise a bound nothing enforces. Only the **explicit** `--pane` conflicts: a bare `--timeout` inside tmux is itself rung 3 of the ladder and selects headless rather than erroring, so scripted invocations that never mention panes keep working unchanged.
- **`--headless`** forces a detached headless worker, opting out of auto pane selection — the escape hatch for an **unattended run that happens to live inside a tmux tab**. It is the mirror of `--pane` and mutually exclusive with it.
- **Not an operator enrollment.** The identity string `fab-{id}-{stage}` deliberately carries **neither** the operator's `»` enrollment prefix **nor** its `›` done marker — in **both** shapes, so a split worker's pane title is unmarked for the same reason a dispatch window's name is: those markers assert the operator owns the window's lifecycle, which a pipeline dispatch does not. An operator that genuinely enrolls a window adds the marker itself via `fab pane window-name ensure-prefix`.

Common to both modes:

- **Missing command field → error, no cross-fallback**: if the resolved tier's provider lacks the field the **finally-selected** mode needs, `start` errors naming the stage, tier, provider, and config key — `…which has no dispatch_command; configure providers.<name>.dispatch_command to dispatch this stage` (headless) or the same sentence with `session_command` (pane) — and does **NOT** fall back to the other field. The auto-only soft fallback is a **mode** change, not a cross-field one: it re-selects headless and re-composes from `dispatch_command`, so a provider carrying **neither** field still errors with the `dispatch_command` hint.
...
- Output: `dispatched <id>/<stage> (pid N, pgid N)` (headless), `dispatched <id>/<stage> (pane %N, split, title fab-<id>-<stage>)` (pane, split shape), or `dispatched <id>/<stage> (pane %N, window fab-<id>-<stage>)` (pane, new-window shape — byte-identical to before the split shape existed). When the mode was chosen by **auto**, the report appends the selection **source** so a surprising mode is explainable from the output alone (the compliance-visibility principle): `, auto: tmux` (auto-selected pane), `, auto: no tmux` (auto-selected headless), `, auto: tmux unreachable` (soft fallback shape (a)), `, auto: no session_command` (soft fallback shape (b)). An **explicitly** selected mode's line carries **no** `auto:` suffix and is byte-identical to before auto selection existed.

### restart — `fab dispatch restart <change> <stage> [--timeout <secs>] [--pane] [--headless] [--server <name>]`

Relaunches a **non-running** dispatch from the prompt `start` already persisted. It is `start` with **exactly one** difference — **the prompt comes from `.fab-dispatch/{id}/{stage}-prompt.md` instead of stdin** — so the caller does not need the multi-thousand-token block prompt still in context (it may have been lost to compaction). Both subcommands share one launch path in Go (one prologue, one launch, one save, one report), so nothing else can drift between them:

- **Same prologue**: change resolution → config + tier→provider re-resolution (**config only** — `restart` has no `--provider`/`--model`/`--effort` surface either) → pane validation → **refuse-if-running** → stale `{stage}.exit`/`-result.yaml`/`.log` clearing → launch → save.
- **Same mode-selection ladder** (the § start table), with the **same flags and the same exclusions** (`--pane`+`--headless` and `--pane`+`--timeout` are usage errors; `--headless`+`--timeout` composes) and the same explicit-hard / auto-soft asymmetry on the pane path's two prerequisites. **The mode is re-derived from the CURRENT environment — the prior attempt's mode is NOT inherited** (and there is no stored discriminator to inherit it from). That is the point: restarting a pane dispatch that was orphaned *by* a dead tmux server correctly soft-falls-back to headless instead of re-failing on the condition that killed it. The **pane shape** (§ start's split-vs-new-window table) is likewise re-derived, so a restart issued from inside a tmux pane splits **that** pane's window even when the prior attempt had opened a window.
- **Same output and record shape**: the same `dispatched <id>/<stage> (<report>)` line including the `, auto: …` selection-source suffix rules, and the same `{stage}.yaml` keys. A restart **is** a fresh attempt under last-attempt-only, so there is **no new state string, no attempt counter, no attempt history, and no `restarted:` marker** anywhere — `status` cannot tell a restart from a `start`, by design.
- **The prompt file is the input, not an output**: `restart` reads `{stage}-prompt.md` and leaves it **byte-identical** (re-writing it with its own bytes would only risk corruption on a partial write). The *prior attempt's* exit/result/log are still cleared, exactly as `start` clears them over a completed attempt.
- **Refuse-if-running wins over a missing prompt**: the refusal check runs first, so a genuinely running dispatch gets the actionable `already running (…); run fab dispatch kill first` message even when no prompt file exists.
- **Missing prompt → clear error, nothing touched**: `no persisted prompt at <path> — nothing to relaunch; run \`fab dispatch start\` with the prompt on stdin`. No worker is launched and no record is written or overwritten (a prior record survives intact).
...
Multi-target operations: `fab batch <new|switch|archive> [flags] [targets...]`. The `new` and `switch` subcommands take `[--list] [--all]` (plus `--quiet|-q` on `switch` — see below), create tmux windows, and — on their tab-launching paths — require `$TMUX` **and `wt` on PATH** (`--list`, and the no-arg invocation that defaults to it, returns before both guards, so listing needs neither tmux nor `wt`) — `wt` is a standalone formula (`brew install sahil87/tap/wt`, not a fab-kit Homebrew dependency), and each command checks it upfront via `exec.LookPath` (after the `$TMUX` check, before any per-item work), exiting 1 with `ERROR: wt is required for 'fab batch new' — install it via: brew install sahil87/tap/wt` (respectively `'fab batch switch'`) when absent — one actionable error instead of N per-item exec failures; `archive` runs in-process (no `$TMUX`, no `wt`) and has its own flag surface (`[--yes|-y] [--dry-run] [--quiet|-q]` — see below), having diverged from `--list`/`--all` for safety. Both `switch` (`[--list] [--all] [--quiet|-q]`) and `archive` (`[--yes|-y] [--dry-run] [--quiet|-q]`) also take `--quiet`/`-q`, which suppresses per-change progress on stdout while keeping all stderr, data output, and (for archive) the summary footer and consent flow (`new` does NOT take `--quiet`).
