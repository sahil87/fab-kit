...
**`--json`**: emits the same field table as a flat, deterministic JSON array (table/rendering order) — per-field objects `{key, default, description, scope, advertise, renamed_from}` with `renamed_from` omitted when empty (empty on every row today). This is the tooling surface the config-upgrade cascade/upgrade commands (Changes 2-3) and external tools consume. Without the flag, output is the commented YAML exactly as before. Both renderings are pure queries and byte-stable for a given binary version; the JSON key set is guarded against drift from the YAML reference's documented keys. (`scope`/`advertise`/`renamed_from` are metadata-only today — no fab command branches on them yet; the cascade resolver, `fab config init --system`, and `fab config upgrade` that consume them land in later config-upgrade changes.)
