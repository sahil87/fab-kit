...
| `set-base-branch` / `get-base-branch` | `set-base-branch <change> <branch>` / `get-base-branch <change> [--json]` | Per-change base branch (`.status.yaml` `base_branch:` field — a plain branch name such as `main` or a stacked parent, never a remote-tracking ref). `set-base-branch` writes it under the status flock; a non-empty branch is required and remote existence is NOT validated (a dependency branch may not be pushed yet). `get-base-branch` prints it (empty line when absent — callers then fall back to the resolved default branch). `omitempty`: an empty base round-trips to absent. No stage auto-populates it — the branch-creation skills (`/git-branch` Step 4, `/fab-new` Step 11) write it at branch creation, and the operator's `stacked-prs` mode writes the dependency branch. `get-base-branch --json` → `{"base_branch":"…"}` (object-wrapped; absent → `{"base_branch":""}`) |
...
Prints the absolute resolved kit directory. A non-empty `FAB_KIT_PATH` wins over the normal exe-sibling `kit/` next to `fab-go`; relative values are absolutized and a missing/non-directory value errors loudly naming the variable, with no exe-sibling fallback. Output is a single newline-terminated line — the path followed by exactly one `\n` — with no other decoration. Exit 0 on success; non-zero with stderr error on failure. Used by skills to reference kit content: `$(fab kit-path)/templates/`, `$(fab kit-path)/migrations/`, etc. — and by the user-level operator skill `fab sync` writes, which resolves the operator skill and its helpers as `$(fab kit-path)/skills/*.md` when `/fab-operator` runs outside any project. The same per-process variable is honored by fab-kit's reader paths (`sync`, `migrations-status`), so every kit-content reader follows one source; binary resolution remains version-pinned and unchanged.
...
- **User-level operator skill** — presence of the machine-level `fab-operator` pointer `fab sync` writes (`~/.agents/skills/fab-operator/SKILL.md` always expected; `~/.claude/skills/fab-operator/SKILL.md` expected only when `claude` is on PATH, otherwise an informational "not expected" note); a present file whose `description:` line differs from the kit's `skills/fab-operator.md` warns as stale with a re-sync hint. Warn/Info only — never affects the exit code.
...
Consumers: `fab pr-meta` (which renders the PR body `**Impact**` line via the same `internal/impact` package) and the apply-finish, hydrate-finish, and ship-finish hooks (write the result into `.status.yaml` `true_impact`; ship-finish is the authoritative write in the standard pipeline — the earlier writes see `HEAD == merge-base` until commits exist). Both resolve the diff base through the shared helper in that order: the change's recorded `base_branch` (`origin/<base_branch>`, when set and resolvable), else `origin/HEAD`'s target, else `origin/main`, else `origin/master`. `/git-pr` delegates the whole `## Meta` block to `fab pr-meta`.
