...
- **Provider-addressed** (`--provider <name>`) — **bypasses role resolution entirely**: looks up `providers.<name>` directly (project config per-field merged over fab-kit's built-in provider table, exactly as the role path's provider lookup does) and composes its `session_command` with the `--model`/`--effort` values through the same `WithProfile`. This is the "spawn a codex session right here" form — no knob or role need name the provider first, and no `providers:` block either: `codex`, `agy`, and `kimi` are **built-in** providers, so `fab agent --provider codex` works on a fresh project. It composes a `session_command`, though, so it works only for a provider that HAS one: `--provider agy` / `--provider kimi` error with the `providers.<name>.session_command` config-key hint, because those two ship dispatch grammar only (see § fab config).
