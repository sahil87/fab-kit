...
| **pane** | `--pane`, `--server`, or auto **inside** tmux | an interactive agent session in a tmux window you can watch and steer | the provider's `session_command` | **result file** + pane liveness | **required** — as is a `session_command`; either prerequisite missing is a hard error when explicit, a soft fallback to headless when auto |

The two provider command fields are **never merged and never fall back to each other in either direction**. Headless mode stays **tmux-independent**; pane mode borrows tmux. **Neither is an unconditional default** — the mode is resolved per invocation by the explicit-first ladder in § start, whose last rung is **auto**: pane when `$TMUX` is set (the run-kit context, where a window is visible to the caller), headless when it is not. Headless dispatch remains parallel to and independent of `fab pane` / `fab operator` (the interactive *operator-visibility* path); pane **dispatch** borrows tmux as a launch surface but does **not** join the operator's monitored set (see `--pane` below). **POSIX-only (v1)** — headless `start`/`kill` error clearly on Windows (`fab dispatch requires a POSIX shell (setsid/timeout); Windows is not supported in v1`) rather than half-working.
...
- **`$TMUX` is the DEFAULTING signal only** — it means "a window opened without `-L` lands on the server the caller is attached to". It never replaces the tmux **reachability probe**, which stays the *validation* step once pane mode is chosen and is a real tmux query (so an explicit `--pane` still works from a headless orchestrator, where `$TMUX` is unset). An **empty** `$TMUX` reads as unset.
