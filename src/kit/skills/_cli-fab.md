...
- `start` sets `{ queue, current: <first>, completed: [], state: running, mode: <flag> }`. `--mode` defaults to `stack-then-review`; an unknown value exits non-zero with a one-line error naming the valid modes and writes no state.
...
- An `autopilot` block lacking `mode` (a pre-existing state file) reads as `stack-then-review`; the typed re-marshal writes the field on the next mutation. The binary only stores/validates/prints the mode (via `fab operator state`) — all merge choreography stays in `fab-operator.md` prose.
