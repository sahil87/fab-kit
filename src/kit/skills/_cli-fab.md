...
The `providers:` block documents fab-kit's **four built-in providers** — `claude` (the default), `codex`, `agy`, and `kimi` — each carrying its independent launch capabilities plus per-role fills in the binary (`internal/agent`'s built-in provider table), so **naming one on a depth knob, in `agent.profiles.<role>.provider`, or on a `--provider` flag resolves with no `providers:` block at all**. Claude ships `interactive_command`, `native: true`, and the stdin-driven `headless_command` `claude -p --dangerously-skip-permissions --model {model} --effort {effort}`; codex, `agy` and `kimi` ship both command fields and no native capability, so **all four built-ins are pane-capable**. Presence describes how each rung runs and never selects `dispatch.mode`: under the default `native`, claude resolves native while the non-claude built-ins descend to headless. Pane capability being universal is the point rather than a coincidence: every agent CLI has an interactive mode, `interactive_command` is pure launch grammar, and a pane worker's prompt is typed in afterwards by `fab dispatch deliver` — so **headless** grammar is the half that genuinely varies per CLI and needs probing. A first-run wall is a readiness-gate input, not a missing capability: `agy` and `kimi` both gate a fresh workspace behind a trust prompt even under their full-auto flags, and worktree-per-change makes every dispatch a fresh workspace, so the gate spends one judgment round and the answer is remembered per folder — see `_cli-agents.md` § agy and § kimi for each wall and its caveat. An **absent** `interactive_command` remains a valid shape in your own `providers:` block (the ladder descends past the pane rung, `pane unavailable: no interactive_command`); it is simply no longer a shipped one. Claude's fill map is exhaustive (all six roles); codex's and agy's are **sparse** — a role they omit resolves that provider's `default` entry. `kimi` ships **no fills at all**, deliberately: its `-m` takes a *user-config* model alias rather than a vendor catalog ID, so a pinned value would break non-managed installs; the empty `{model}` drops the `-m` token pair and kimi falls back to the user's own `default_model`. Non-claude fills are **refreshed at kit-release cadence** and pass through unvalidated; pin a newer model with one config line (`providers.<name>.profiles.<role>.model`, scope `both` — settable once per machine in `~/.fab-kit/config.yaml`, which outranks the project file), which is also how you give `kimi` per-role differentiation. The non-claude blocks render **commented** because they merely restate built-in defaults — a commented block registers no project override, and a built-in provider is inert until a knob, a role override, or a flag names it. All four ship a deliberate full-auto posture because unattended stage workers cannot answer approval prompts: claude carries `--dangerously-skip-permissions`, codex `--dangerously-bypass-approvals-and-sandbox` on both its forms, agy `--dangerously-skip-permissions` on both its forms, and kimi `--auto` on its **interactive** command only — its headless form carries no approval flag at all, because `kimi -p` already auto-approves tools and *errors* when combined with `--yolo`/`--auto`. Override the corresponding provider command to restore approvals. Two grammar specifics are load-bearing: neither of agy's commands carries an `{effort}` placeholder (its model IDs *embed* the reasoning level, e.g. `gemini-3.1-pro-high`, so its fills carry no effort either), and both agy's and kimi's `headless_command`s **nest a shell** — `sh -c '<cli> … -p "$(cat)"'` — because each CLI takes the prompt as the `-p` ARGUMENT and ignores stdin, while POSIX expands `$(cat)` before `fab dispatch`'s stdin redirect applies.
