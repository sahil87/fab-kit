...
| Mode | Output |
|------|--------|
| `fab config show` | Fully composed config as YAML — the built-in defaults tier merged beneath the environment-over-system-over-project layers, so unset fields surface at their built-in values |
...
Accepts at most one known dotted key; unknown keys fail non-zero naming the key. Requires a fab repo (walks up for `fab/`, like `fab preflight`). Writes no file.
...
- **Provider-addressed** (`--provider <name>`) — **bypasses role resolution entirely**: looks up `providers.<name>` directly (project config per-field merged over fab-kit's built-in provider table, exactly as the role path's provider lookup does) and composes its `interactive_command` with the `--model`/`--effort` values through the same `WithProfile`. This is the "spawn a codex session right here" form — no knob or role need name the provider first, and no `providers:` block either: `codex`, `agy`, and `kimi` are **built-in** providers, so `fab agent --provider codex` works on a fresh project. It composes an `interactive_command`, though, so it works only for a provider that HAS one: `--provider agy` / `--provider kimi` error with the `providers.<name>.interactive_command` config-key hint, because those two ship dispatch grammar only (see § fab config).
...
- **Error**: a resolved provider with no `interactive_command` errors with a config-key hint (`configure providers.<name>.interactive_command`) on either path — reachable for a provider name that has **no built-in row at all** (a wholly user-defined, dispatch-only `providers:` entry) **or that names a built-in which ships none** — since 260810-ki9v that is `agy` alone, the one dispatch-only built-in. Overriding a built-in that *does* ship one can never reach it: `ResolveProvider` per-field merges over the built-in row, so `providers.codex: {headless_command: …}` still inherits the built-in `interactive_command` and still launches a session. An unknown role name errors and names the valid set.
