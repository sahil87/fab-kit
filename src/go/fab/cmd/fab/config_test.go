...
func TestConfigReferenceDocumentsBuiltInProviders(t *testing.T) {
	...
	// carry both commands. The expectations are DERIVED from the agent command
	// vars (never literal copies), so a grammar change touches only
	// internal/agent. They are compared in their YAML-SCALAR form — the
	// nested-shell dispatch commands contain single quotes, which the renderer
	// must double, so asserting the raw string would demand exactly the invalid
	// YAML the escaping exists to prevent.
	...
	// agy is pane-capable like the rest: its block opens on interactive_command,
	// the rendered ordering a reader hoisting the block gets — same as codex's
	// and kimi's. Its first-run trust wall is an ordinary readiness-gate
	// judgment round (the kimi precedent).
	...
	// The retired dispatch-only framing must not survive — it renders into every
	// project's config fence, so a stale claim there is a user-facing
	// documentation inaccuracy. The first-run-wall prose must say the wall rides
	// the readiness gate.
	for _, retired := range []string{"agy ships NO interactive_command", "DISPATCH-ONLY", "agik"} {
		if strings.Contains(out, retired) {
			t.Errorf("providers block still carries the retired dispatch-only claim %q", retired)
		}
	}
	if !strings.Contains(out, "readiness-gate judgment") {
		t.Error("providers block must record the first-run trust walls as readiness-gate judgment rounds")
	}
