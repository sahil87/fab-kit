...
// TestModeCommand_AgyComposesBothModes: agy ships an interactive_command, so BOTH
// rungs compose from the shipped defaults — asserted at the seam `dispatch start`
// branches on, because this is where a regression would show as a stage silently
// descending to headless again. Its fresh-workspace trust prompt is an ordinary
// readiness-gate judgment round (the kimi precedent), so pane capability rides
// launch grammar alone.
//
// modeCommand is the composition seam both modes go through, so exercising it
// needs no tmux server. It returns the provider's TEMPLATE — substitution is the
// caller's next step (dispatchStart's spawn.WithProfile) — so the rung assertions
// compare against the shipped grammar.
