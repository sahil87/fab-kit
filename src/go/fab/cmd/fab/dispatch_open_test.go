...
// (the motivating case): a dispatcher in an 80x24 window would carve a 36-column
// worker at the default 45% — below the 50x20 floor — so the launch must NOT split.
...
func TestDispatchOpen_BelowFloorDemotesToSizedWindow(t *testing.T) {
	...
	wantWarning := "window 80x24 too narrow for a 45% column (36 cols < 50): opening worker in its own window"
