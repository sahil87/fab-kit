...
// result converges on regeneration); unset and empty render nothing.
func TestDocsIndexNavNoteGenericRoot(t *testing.T) {
	...
	t.Run("unset", func(t *testing.T) {
		...
		if root := docContent(t, targets, "specs/index.md"); strings.Contains(root, "New here?") {
			t.Fatalf("unset nav_note must render nothing on a generic root:\n%s", root)
		}
	})
...
// through the full gather: set replaces the built-in line verbatim, empty
// omits it, and both converge on regeneration.
func TestDocsIndexNavNoteLegacyRoot(t *testing.T) {
	...
	t.Run("set", func(t *testing.T) {
		...
		if !strings.Contains(root, note) || strings.Contains(root, "../specs/glossary.md") {
			t.Fatalf("set nav_note must replace the legacy line:\n%s", root)
		}
		docIdempotent(t, repo, c, targets)
	})
	...
	t.Run("empty", func(t *testing.T) {
		...
		targets, _ := docGather(t, repo, c)
		root := docContent(t, targets, "memory/index.md")
		if strings.Contains(root, "New here?") {
			t.Fatalf("empty nav_note must omit the nav line:\n%s", root)
		}
		docIdempotent(t, repo, c, targets)
	})

	t.Run("unset keeps legacy bytes", func(t *testing.T) {
		repo := t.TempDir()
		c := config.DefaultDocsIndexRoots()[0]
		docWrite(t, repo, "docs/memory/auth/login.md", "# Login\n")
		targets, _ := docGather(t, repo, c)
		root := docContent(t, targets, "memory/index.md")
		if !strings.Contains(root, legacyNavNote) {
			t.Fatalf("unset nav_note must render the legacy line:\n%s", root)
		}
		docIdempotent(t, repo, c, targets)
	})
}
