...
type RootData struct {
	...
	// nil renders legacyNavNote (the pre-config default), a pointer to ""
	// omits the line entirely, and a pointer to a non-empty value renders it
...
// legacyNavNote is the navigation line RenderRoot emits when the root carries
// no configured nav_note — the pre-config default, kept byte-identical so
// standard-layout repos see zero drift.
const legacyNavNote = "> **New here?** Start with the [README](../../README.md) for setup and a walkthrough. For terminology, see the [Glossary](../specs/glossary.md).\n\n"
...
func RenderRoot(d RootData) string {
	...
	switch {
	case d.NavNote == nil:
		b.WriteString(legacyNavNote)
	case *d.NavNote != "":
		b.WriteString(*d.NavNote + "\n\n")
	}
