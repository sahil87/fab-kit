...
// FileEntry is one topic file within a domain folder — any regular file the
// walker indexes, markdown or not.
type FileEntry struct {
	Label string
	// Link is the row's link target: empty for markdown (rendered as
	// Base + ".md"), the filename verbatim for every other type.
	Link string
	// Base is the sort key and fallback label: the stem without the .md
	// extension for markdown, the filename (with extension) for other types.
	Base string
	// Title is the file H1 (markdown) or <title> (HTML), used as the row label
	// when description is missing. A title is never a description source.
	Title string
	// Description is the `description:` frontmatter value (markdown) or the
	// <meta name="description"> value (HTML); "" → rendered as the missing-cell
	// fallback. Types with no metadata source always render the fallback.
	Description string
}
...
func (w Warning) String() string {
	...
	case KindMissingDescription:
		if ext := strings.ToLower(filepath.Ext(w.Path)); ext == ".html" || ext == ".htm" {
			return fmt.Sprintf("⚠ %s has no <meta name=\"description\"> — using <title> and placeholder", w.Path)
		}
...
func RenderRoot(d RootData) string {
	...
	b.WriteString("> **Generated by `fab docs-index`** — everything outside the manual block below is\n")
	b.WriteString("> regenerated from the tree and each file's own metadata (`description:` frontmatter,\n")
	b.WriteString("> HTML `<title>`/`<meta name=\"description\">`): don't edit it, re-run the command. The manual\n")
	b.WriteString("> block is the one hand-managed region: edit it, it is preserved verbatim.\n\n")
	if d.NavNote != "" {
...
func RenderDomain(d DomainData) string {
	...
	fmt.Fprintf(&b, "# %s\n\n", d.Title)
	b.WriteString("> **Generated by `fab docs-index`** — everything outside the manual block below is regenerated from the tree and each file's own metadata (`description:` frontmatter, HTML `<title>`/`<meta name=\"description\">`): don't edit it, re-run the command. The manual block is the one hand-managed region: edit it, it is preserved verbatim.\n\n")
