...
//
// Every built-in ships an interactive_command — all four are pane-capable. What is
// probed per provider is its interactive FIRST-RUN behavior and input echo — not its
// prompt grammar, since fab appends nothing to the command and types a pane worker's
// pointer in afterwards. agy's fresh-workspace trust prompt (it fires even under
// --dangerously-skip-permissions, and worktree-per-change makes every dispatch a
// fresh workspace) is an ordinary readiness-gate judgment round: one answer,
// workspace-scoped, the same shape that clears kimi's wall.
//
...
func TestResolveProvider_NonClaudeBuiltIns(t *testing.T) {
	cases := []struct {
		name string
		// session is the shipped interactive_command; every built-in carries one.
	...
	// The agy grammar deliberately omits {effort} on BOTH forms: its model IDs
	// embed the reasoning level as an ID suffix, so a separate effort flag would
	// fight the suffix.
	if strings.Contains(DefaultAgyHeadlessCommand, "{effort}") || strings.Contains(DefaultAgyInteractiveCommand, "{effort}") {
	...
	// agy and kimi are both pane-ELIGIBLE at the seam the dispatcher reads:
	// shipping an interactive_command is exactly what makes a provider eligible
	// for pane-mode dispatch. Asserting it here — over the same resolved
	...
	if agy.InteractiveCommand == "" {
		t.Error("agy must be eligible for pane dispatch — its first-run trust wall is an ordinary readiness-gate judgment round (the kimi precedent)")
	}

	// kimi's {model} must stay droppable — kimi ships no fills, so a pinned model
	// here would be a value no install is guaranteed to accept.
