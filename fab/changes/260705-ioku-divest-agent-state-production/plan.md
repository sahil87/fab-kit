...
- `src/go/fab/cmd/fab/hook.go` shims `hookSessionStartCmd`/`hookStopCmd`/`hookUserPromptCmd` (+ `noOpHookShim`, and y022's `hookArtifactWriteCmd`) — deliberate one-release lifecycle; slated next-release deletion once the `2.13.6-to-2.14.0` migration has propagated.
- The whole sync path, retained one release per R11 and now **fully inert**: `hooklib.Sync` + `DefaultMappings`/`HookMapping`/`hasDuplicate` (`src/go/fab/internal/hooklib/sync.go`), fab-kit's `syncHooks`/`defaultHookMappings`/`hookMapping`/`hookHasDuplicate` (`src/go/fab-kit/internal/hooksync.go`), and `hookSyncCmd` (`cmd/fab/hook.go`). Full removal is the recorded follow-up. Within it, already-dead sub-pieces as of cycle 2:
  - `oldScriptToSubcommand` (sync.go:47, hooksync.go:35) is `{}` and `migrateOldHookCommands` (sync.go:185-205, hooksync.go:154-172) is a guaranteed no-op — the three legacy rewrite rows were dropped (T011) so sync can never re-mint the entries the 2.14.0 migration deletes; the functions survive only as the documented seam until the sync path dies.
  - The desired-entry merge loops (sync.go:116-130, hooksync.go:95-109) — `desired` is always empty with the emptied mapping tables.
  - The `migrated > 0` / `added > 0` result branches (sync.go:156-180, hooksync.go:135-150) — `added` and `migrated` are both always 0, so only the OK path can execute; the "Created"/"Updated" messages are unreachable.
  - `hasDuplicate`/`hookHasDuplicate` — sole callers are the dead merge loops above.
- `src/go/fab/internal/hooklib/sync.go:132-148` (the unconditional settings write in `Sync`) + the fab-kit mirror (`hooksync.go:111-127`) — with the emptied mapping table, a fresh `fab hook sync` still materializes a `.claude/settings.local.json` containing only `"hooks": {}` before reporting OK; dies with the sync path above (next-release follow-up).

*(All entries re-verified in review cycle 4: line references and callerless status still accurate; the cycle-4 fix was help-text/comment-only and introduced no new redundancy.)*
...
16 assumptions (5 certain, 10 confident, 1 tentative).
