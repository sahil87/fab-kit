...
- **R8** (exit-2 pointer): On tier 2, the human-readable stderr output MUST end with a pointer to `/docs-hydrate-memory` backfill mode.
  - GIVEN a tier-2 tree (no `--json`), WHEN `--check` runs, THEN stderr ends with `→ run /docs-hydrate-memory (backfill mode) to backfill description: frontmatter and relocate removal-history rows before regenerating.`
...
- **R12** (regen-site guards): Every site that runs `fab memory-index` to regenerate MUST first consult `fab memory-index --check` and refuse to regenerate on exit 2, surfacing the `/docs-hydrate-memory` backfill pointer. Sites: `/docs-hydrate-memory` regen step(s), `/docs-reorg-memory` (via R11), `/fab-continue` hydrate stage. Each guard MUST carry a brief annotation that it is a no-op for born-compatible trees.
  - GIVEN a born-compatible tree, WHEN any regen site runs, THEN the guard is a no-op (exit 0/1) and regeneration proceeds. GIVEN a pre-fab-kit tree, THEN the guard refuses (exit 2) and points to backfill mode.
