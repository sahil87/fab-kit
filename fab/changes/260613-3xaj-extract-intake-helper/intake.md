...
| 9 | Tentative | The lifted Step 4 (Conversation Context Mining) text is genericized in `_intake.md` (referring to "the invoking skill" rather than "this `/fab-new` invocation"), structurally retiring `fab-draft`'s "read self-name as `/fab-draft`" instruction. <!-- assumed: genericize self-name references in the lifted body — resolves the existing fab-draft prose-delta concern at the structural level; alternative is a {self-name} parameter, judged unnecessary since the text only needs to be invocation-agnostic, not invocation-named --> | The description and finding both call out the self-name concern. Genericizing is simpler than threading a `{self-name}` parameter, but exact wording is a judgment call left to apply. | S:55 R:75 A:65 D:55 |
