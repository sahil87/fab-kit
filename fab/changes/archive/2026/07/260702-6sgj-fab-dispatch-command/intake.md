...
| 16 | Certain | This change authors `docs/specs/harness-adapters.md` fixing the full dispatch protocol (both adapters, prompt obligations, five states, `review` nesting degradation, hooks-enhance-never-own) as pre-implementation intent; 3d is wiring-only against it | Discussed — user approved re-slicing after challenging the 3c/3d separation; Constitution VI + the stage-models.md (#406) precedent make spec-authored-by-implementing-change the established pattern | S:95 R:65 A:95 D:90 |

16 assumptions (11 certain, 4 confident, 1 tentative).
