...
**Why**: The pane-count-only rule ties at zero on the most common spawn (a repo with nothing open yet), and its tie-break — the §8 setting — resets on every compaction, so the "one ask" recurred per compaction. `rk mux sessions` already supplies `path` (a repo join that needs no open panes) and `attached` plus row order (always defined, even at zero), so the list bottoms out deterministically; a sole candidate needs no evidence at all. A wrong landing is cheap to correct (`move-window`; `rk tab new --json` confirms the landing), so asking has negative value.
