...
### I. Pure Prompt Play
All workflow logic MUST live in markdown skill files and shell scripts. There SHALL be no package managers and no build steps required to run the workflow. Single-binary runtime CLI tools MAY be used when they are invoked directly by scripts and documented in project memory (for example: `yq`, `git`, `gh`). Any AI agent that can read markdown and execute shell commands can drive the workflow.
