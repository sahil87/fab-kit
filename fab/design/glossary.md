...
| Term | Definition |
|------|-----------|
| **Artifact** | A markdown file generated by a skill at a specific stage — `brief.md`, `spec.md`, `tasks.md`, or `checklists/quality.md`. Each artifact lives inside a change folder. |
...
| **Hydration** | The process of merging change artifacts (spec.md) into centralized docs. Performed by `/fab-archive` on completion, or by `/fab-hydrate` from external sources. |
...
| `/fab-review` | Validates implementation against spec, checklist, and constitution. On failure, offers rework options (fix code, revise tasks/spec). |
...
| `/fab-backfill` | Identifies gaps between `fab/docs/` and `fab/design/`, proposes additions with interactive confirmation. No active change required. |
...
| `checklists/quality.md` | Auto-generated quality checklist inside a change folder. Items derived from spec and constitution. Verified by `/fab-review`; all items must pass before archive. |
...
| **Rework loop** | The review-to-earlier-stage feedback cycle. `/fab-review` can loop back to apply (fix code), tasks (revise tasks), or spec (revise requirements). |
