...
| Term | Definition |
|------|-----------|
| **Artifact** | A markdown file generated by a skill at a specific stage — `proposal.md`, `spec.md`, `plan.md`, `tasks.md`, or `checklists/quality.md`. Each artifact lives inside a change folder. |
| **Centralized docs** | The source-of-truth documentation in `fab/docs/`. Organized by domain, updated by `/fab-archive` hydration and `/fab-hydrate` ingestion. Contrast with **spec.md** (change-scoped). |
| **Change** | A unit of work tracked as a folder under `fab/changes/`. Contains all artifacts from proposal through archive. Identified by its folder name (e.g., `260115-a7k2-add-oauth`). |
...
| **Hydration** | The process of merging change artifacts (spec.md, plan.md) into centralized docs. Performed by `/fab-archive` on completion, or by `/fab-hydrate` from external sources. |
...
| **Specs** | Human-curated, pre-implementation design documents in `fab/specs/`. Capture architectural intent, design rationale, and the "why" behind the system. Not managed by Fab tooling — organized however makes sense for the project. Contrast with **centralized docs** (post-implementation, AI-maintained via hydration). |
| **spec.md** | A change-level specification file inside a change folder. Describes requirements relevant to that specific change using RFC 2119 keywords and GIVEN/WHEN/THEN scenarios. Not to be confused with **specs** (the project-wide design documents in `fab/specs/`). |
| **Stage** | One of the 7 sequential phases a change passes through: proposal, specs, plan, tasks, apply, review, archive. Tracked in `.status.yaml`. |
...
| Term | Definition |
|------|-----------|
| **Proposal** | Stage 1. Captures intent, scope, and approach for a change. Output: `proposal.md`. Created by `/fab-new` or `/fab-discuss`. |
| **Specs** | Stage 2. Defines requirements for the change using RFC 2119 keywords and scenarios. Output: `spec.md`. |
| **Plan** | Stage 3 (optional). Documents how to implement — technical decisions, research, file changes. Output: `plan.md`. Skippable for small changes; recorded as `plan: skipped` in status. |
| **Tasks** | Stage 4. Breaks the plan (or spec) into an actionable checklist with phased execution order. Output: `tasks.md` + `checklists/quality.md`. |
| **Apply** | Stage 5. Executes tasks from `tasks.md`, marking each complete. Runs tests after each task. |
| **Review** | Stage 6. Validates implementation against spec, checklist, and constitution. Can loop back to earlier stages on failure. |
| **Archive** | Stage 7. Hydrates change artifacts into centralized docs, moves the change folder to `archive/`, and clears the pointer. |
...
| `/fab-new` | Starts a new change from a description. Creates the change folder, `.status.yaml`, and `proposal.md`. One-shot capture with max ~3 questions. |
| `/fab-discuss` | Explores and develops a proposal through free-form conversation. No question cap. Includes gap analysis. Outputs a proposal with a confidence score. |
...
| `/fab-ff` | Fast-forward — generates all remaining planning artifacts (specs, plan, tasks) in one pass. Frontloads questions, then runs autonomously. |
...
| `/fab-apply` | Executes unchecked tasks from `tasks.md`. Resumable — re-invoking picks up from the first unchecked item. |
| `/fab-review` | Validates implementation against spec, checklist, and constitution. On failure, offers rework options (fix code, revise tasks/plan/specs). |
| `/fab-archive` | Completes the change — hydrates artifacts into centralized docs, moves to archive, clears pointer. |
...
| `/fab-backfill` | Identifies gaps between `fab/docs/` and `fab/specs/`, proposes additions with interactive confirmation. No active change required. |
...
| Term | Definition |
|------|-----------|
| `.status.yaml` | The status manifest inside each change folder. Tracks current stage, progress of all stages, checklist counts, confidence score, and optional branch name. Single source of truth for where a change is. |
...
| `checklists/quality.md` | Auto-generated quality checklist inside a change folder. Items derived from spec, plan decisions, and constitution. Verified by `/fab-review`; all items must pass before archive. |
| `fab/current` | Pointer file containing the active change name. Read by every skill; updated by `/fab-new`, `/fab-switch`; cleared by `/fab-archive`. |
...
| `fab/specs/` | Human-curated pre-implementation specifications. Flat structure, not managed by Fab tooling. |
...
| **Confidence score** | A 0.0–5.0 numeric measure of how well-resolved a change's decisions are. Formula: `max(0.0, 5.0 - 0.1 * confident - 1.0 * tentative)` (0.0 if any unresolved). Stored in `.status.yaml`. |
...
| **Gap analysis** | Evaluation performed by `/fab-discuss` before creating a proposal. Checks whether the change is already covered by existing mechanisms. |
...
| **Rework loop** | The review-to-earlier-stage feedback cycle. `/fab-review` can loop back to apply (fix code), tasks (revise tasks), plan (revise plan), or specs (revise requirements). |
...
| **[BLOCKING]** | Priority marker for open questions in proposals. Must resolve before advancing to specs. Max 3 per proposal. |
| **[DEFERRED]** | Priority marker for open questions in proposals. Can resolve during plan stage. |
| **[P]** | Task marker in `tasks.md` indicating the task is parallelizable — can execute alongside other `[P]` tasks in the same phase. |
| **CHK-###** | Checklist item ID in `checklists/quality.md` (e.g., CHK-001). Sequential. Verified by `/fab-review`. |
