...
| **Memory files** | The source-of-truth documentation in `fab/memory/`. Organized by domain, updated by `/fab-continue` (archive) hydration and `/fab-hydrate` ingestion. Contrast with **spec.md** (change-scoped). |
...
| **Domain** | A topic area in `fab/memory/` (e.g., `auth/`, `payments/`). Each domain has its own `index.md` and one or more memory files. |
| **Hydration** | The process of merging change artifacts (spec.md) into memory files. Performed by `/fab-continue` (archive) on completion, or by `/fab-hydrate` from external sources. |
...
| **Design specs** | Human-curated, pre-implementation design documents in `fab/specs/`. Capture architectural intent, design rationale, and the "why" behind the system. Not managed by Fab tooling — organized however makes sense for the project. Contrast with **memory files** (post-implementation, AI-maintained via hydration). |
| **spec.md** | A change-level specification file inside a change folder. Describes requirements relevant to that specific change using RFC 2119 keywords and GIVEN/WHEN/THEN scenarios. Not to be confused with **design specs** (the project-wide design documents in `fab/specs/`). |
...
| **Archive** | Stage 6. Hydrates change artifacts into memory files, moves the change folder to `archive/`, and clears the pointer. |
...
| `/fab-hydrate` | Ingests external documentation (Notion, Linear, local files) into `fab/memory/` with domain mapping. |
...
| `fab/memory/` | Memory files. Organized by domain with index files. The authoritative post-implementation source of truth. |
