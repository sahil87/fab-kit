...
### V. Portability
Kit content (skills, templates, migrations) lives in the system cache at `~/.fab-kit/versions/<version>/kit/`, managed by the `fab` binary (`brew install fab-kit`). User projects contain only `fab/` (config, changes, backlog), `.claude/skills/`, and `.agents/skills/` (deployed copies, ignored via the generated per-target `.gitignore` manifests `fab sync` writes). The kit SHALL have no assumptions about the host project's structure, language, or toolchain beyond the presence of a `fab/` directory. Project-specific configuration belongs in `fab/project/config.yaml` and `fab/project/constitution.md`, not in the kit.
...
- `src/kit/` is the canonical source for all kit content (skills, templates, migrations). `.claude/skills/` and `.agents/skills/` contain deployed copies produced by `fab sync` — fab's copies are ignored via generated per-target `.gitignore` manifests, and the directories are never edited directly
