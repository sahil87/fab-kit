...
- `src/kit/` is the canonical source for all kit content (skills, templates, migrations). `.agents/skills/` (always) and `.claude/skills/` (when `claude` is available) contain deployed copies produced by `fab sync` — fab's copies are ignored via generated per-target `.gitignore` manifests, and the directories are never edited directly
...
**Version**: 1.8.0 | **Ratified**: 2026-02-06 | **Last Amended**: 2026-09-11
