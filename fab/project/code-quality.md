...
- **Editing `.claude/skills/` directly.** That tree holds deployed copies regenerated by `fab sync` (fab's copies are ignored via the generated per-target `.gitignore` manifest) — edits there are silently overwritten on the next sync. Always edit the canonical source `src/kit/skills/*.md` (Constitution V).
