...
- **Canonical source only** — flag any edit under `.claude/skills/` (deployed copies regenerated by `fab sync`, ignored via the generated per-target manifest); kit changes belong in `src/kit/`.
