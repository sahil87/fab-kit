...
- `tasks.md` MUST exist
- If stage is `tasks`: set `tasks: done`, `apply: active` before starting

### Task Execution

1. Parse tasks: `- [ ]` = remaining, `- [x]` = skip
2. If all checked: set `apply: done`, `review: active`. Stop.
...
5. On completion: `apply: done`, `review: active`
...
**Pass**: Set `review: done`, `hydrate: active`. Update `checklist.completed`. Output report + `Next: /fab-continue`

**Fail**: Set `review: failed`, `apply: active`. Update `checklist.completed`. Output failure details + rework options:
...
3. **Reset `.status.yaml`**: Target → `active`, all after → `pending`, all before → preserved
