...
- If omitted, process **all** `.md` files in `fab/.kit/skills/` except `_context.md` and `_generation.md` (shared preambles, not skills).

---

## Pre-flight

1. Read `fab/.kit/skills/_context.md` — this is the shared context all skills reference. Anything fully defined here does NOT need re-explaining inside individual skills.
...
|--------|-----------------|
| **Redundant re-explanation** | Concepts already defined in `_context.md` or `_generation.md` being re-stated (SRAD rules, confidence formula, context loading layers, preflight behavior). Replace with a brief reference. |
...
3. **Preserve the H1 heading and context reference** — `# /skill-name` and the `_context.md` blockquote stay.
4. **Reference shared docs instead of re-explaining** — e.g., replace a 10-line SRAD re-explanation with "Apply the SRAD framework (see `_context.md`)."
...
7. **Keep error tables** — but remove rows already covered by preflight or `_context.md`.
...
- DO NOT merge skills or move content between skills (beyond referencing `_context.md`)
- DO NOT touch `_context.md` or `_generation.md` themselves — they're the reference, not the target
