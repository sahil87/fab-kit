...
- [ ] [hin2] 2026-04-22: Operator /fab-operator: rule 4 (numbered menu → 1) is ambiguous. Distinguish routine numbered prompts (tool permission, trivial defaults) from strategic ones (scope/PR split/pipeline shape) — only auto-answer the former. Strategic prompts (multi-option choices that affect queue shape, commit content, or checklist compliance) should escalate even when a stated default exists.
- [ ] [i1l6] 2026-04-22: Operator /fab-operator: add configurable auto-default-after-N-minutes for idle escalations. Keep the pipeline moving is highest priority — if an agent has been idle on an escalated prompt past a threshold (default 30m), operator auto-picks the agent's stated default and logs the action. Configurable per-change (or globally via .fab-operator.yaml). Trades user oversight for throughput; essential for long/headless autopilot runs. Pairs with rule-4 ambiguity fix (hin2).
