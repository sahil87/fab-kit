...
| hop (Multi-Repo Navigator) | The genuinely-optional identity + fail-silent gate, and the fab-owned "why it matters" (discovering a sibling repo's main-worktree root for spawning); discovery commands are tool-owned (`hop skill`) |
