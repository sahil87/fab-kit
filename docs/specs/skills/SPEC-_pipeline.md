...
**Source organization:** `_pipeline` owns shared ff/fff framing and the five-step stage procedure; the dispatch contract itself remains in `_preamble`.

Shared pipeline bracket for `/fab-ff` and `/fab-fff`; `/fab-adopt` is a partial consumer of its rework loop and hydrate dispatch. It owns common driver framing, the intake gate, context loading, resumability, Steps 1–3, bounded rework, exhaustion handling, shared errors, and a five-step Stage Dispatch Procedure. That procedure resolves and surfaces the stage profile, points to `_preamble.md` § CLI-Adapter Dispatch and Dispatch-Prompt Obligations, reads the result, and leaves all stage transitions with the sequencer.

**CLI completion**: the bracket's adapter note explicitly follows the canonical `done` path — read the result, reap per the owner's stage-aware timing, then perform the sequencer transition. Reap remains pane hygiene, not kill or `.fab-dispatch/` state cleanup; `_preamble.md` § CLI-Adapter Dispatch owns the full contract, including *when* each stage's reap fires and the arm gate on the deferred apply one.
...
3. **Re-dispatch apply (resume-first, 260808-tv3g / 260809-3oz7)**: reach the existing apply worker by its arm's own mechanism — the native `apply-{id}` handle from **this** orchestrator session via SendMessage, or the still-live apply **pane** via `fab dispatch deliver <change> apply --prompt-file .fab-dispatch/{id}/apply-continuation.md` — carrying the item-2 rework instructions; otherwise dispatch a fresh `/fab-continue` Apply Behavior subagent (the verbatim prior behavior, and the mandatory fallback on both arms). Either path carries the no-`fab status`-transition prompt contract; on success `fab status finish <change> apply {driver}` — the auto-activation of review here (review → `active`) is the **one** counted transition that advances `iterations` for this cycle (`status.go:627`, `Iterations++` only on `active`). It MUST run every cycle, even after a trivial fix. `_preamble.md` § Worker Continuation owns the naming/continuation/fallback/profile-fixity/scope mechanics; this item carries only the wiring.
...
**Worker release (260808-tv3g / 260809-3oz7)**: after item 5's `finish review` (pass) or the exhaustion stop, the orchestrator stops continuing the apply worker. On the native arm release is passive (no teardown call, the handle is simply never used again); on the **pane** arm this is the moment `_preamble.md` § CLI-Adapter Dispatch step 3 deferred the apply reap to so the pane could be resumed, and the skill takes that action **there** — the reap rule, including its arm gate, is stated once by the owner and only pointed at here (owner-or-pointer). Hydrate and every later stage always dispatch fresh.
...
│   by default; a pane landing is entered through fab dispatch open → ready →
│   deliver (three reachable states running/done/orphaned; requires a reachable
│   tmux server, so never speculative) and --headless forces a detached worker
│   inside tmux (260805-zxe0, 260809-3oz7) — 260613-l3ja established
...
│  │        native branch names the worker apply-{id}; the pane branch needs no name — the pane IS
│  │        the handle, which is why its reap is deferred — _preamble.md § Worker Continuation,
│  │        260808-tv3g / 260809-3oz7)
...
│     item 3 RESUMES the existing apply worker when reachable (native handle
│     or live pane via deliver --prompt-file) —
...
| Agent | Step | Purpose |
|-------|------|---------|
| /fab-continue (Apply) | 1, rework item 3 | Plan co-generation + task execution; no `fab status` transition calls. Named `apply-{id}` on the native branch, or reached as its own live tmux pane on the pane branch, and **resumed** at item 3 when reachable (260808-tv3g / 260809-3oz7); a fresh worker on the initial dispatch and on every fallback |
