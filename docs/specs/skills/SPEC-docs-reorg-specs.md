...
**No FKF frontmatter on spec moves (Constitution VI).** FKF (`type: memory` + `description:`) governs `docs/memory/` only; specs are out of FKF scope and stay frontmatter-free, human-curated per **Constitution VI**. When the skill moves a spec file it MUST NOT stamp, add, or synthesize `type:` / `description:` frontmatter — a moved spec carries exactly its prior bytes (only its path and the `index.md` row change). This is the deliberate mirror of `docs-reorg-memory`'s frontmatter-*preserving* moves: memory moves keep FKF frontmatter, spec moves add none. A generated-index model for `docs/specs/index.md` is **not adopted** (Constitution VI) — no `fab specs-index` generator; the specs index stays hand-rewritten and spec links stay ordinary repo-relative.
...
│
└─ [if approved]
   ├─ Write/Edit: reorganized spec files (moved bytes verbatim — no FKF frontmatter stamped, Constitution VI)
