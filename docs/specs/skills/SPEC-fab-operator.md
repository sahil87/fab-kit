...
**Prose optimization** (260620-skop): skill content trimmed to remove re-explanation of partial-owned concepts (launcher degraded behavior → `_cli-fab.md` § fab operator + §9; state-file path/migration → §2/§9; tick-step field semantics → `_cli-fab.md` § fab pane map; `rk notify` **usage** → `_cli-external.md` § rk + `_preamble.md` § Run-Kit (the `rk notify` *contract* itself has since moved tool-owned to `rk skill` — `260718-clix`); implicit `--base` chaining defined once in Queue ordering) and a `## Contents` TOC added; no behavioral change (Flow / Tools / Sub-agents unchanged). A `## Contents` TOC was also added to this SPEC (>100 lines, structural rule).
...
| The operator's fab-owned `wt` spawn choreography — run `wt create` in the target repo's directory, and the probe-and-route that picks `--checkout <existing-branch>` vs. the new-branch positional (the routing decision is fab's; wt's branch-selection *contract* is tool-owned via `wt skill` as of `260718-clix`) — plus `tmux` commands and `/loop` | `_cli-external.md` |
