...
5. Auto-nudge (input-waiting detection + answer model, incl. non-blocking strategic handling per Auto-Nudge below) — no post-intake `/git-branch` send (260612-w7dp: `/fab-new` Step 11 creates/renames the branch inline; `/git-branch` is sent only for a detected branch/change mismatch per §3 pre-send validation)
6. Stuck detection (excludes input-waiting agents)

The loop-lifecycle step then adapts the cadence (260615-mmmt): tighten the single loop to `90s` if any monitored agent was detected menu-waiting this tick, relax to `3m` otherwise — re-establishing the one loop, never a second.
...
| Coordinating-agent model | Doing tier (260613-2sdj) — `fab resolve-agent apply` → profile injected via `spawn.WithProfile` (**appends** `--model`/`--effort` for a plain Claude spawn command, last-wins/empty ⇒ omit; **substitutes** into a `{model}`/`{effort}` template for a non-Claude one, 260702-6tmi); built-in fallback `{claude-opus-4-8, high}` on any failure (incl. no resolvable `fab/` project) |
| Uses `/loop`? | Yes — adaptive heartbeat (260615-mmmt): `3m` normally, tightens to `90s` (§8) when any monitored agent is menu-waiting, relaxes back to `3m`; one loop at a time |
