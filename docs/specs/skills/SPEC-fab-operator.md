...
| Requires a `fab/` project? | No (260613-2sdj) — session command is the project's `providers.claude.session_command` when `fab/` is resolvable, else `spawn.DefaultSpawnCommand` (`claude --dangerously-skip-permissions`); `resolve.FabRoot()` failure is non-fatal (no project `providers`/`agent.tiers` read on a `fab/`-less launch) |
