...
- **Independent re-scan, no caller manifest**: backfill walks `docs/memory/` itself to find topic files (non-`index.md` `.md`) missing `description:` — it receives no file list from its caller. This holds for both the direct-user form (`/docs-hydrate-memory backfill`) and the reorg-dispatched form (reorg's prompt names the operation, "backfill this tree"). The loose seam between the two independently-invocable skills.
