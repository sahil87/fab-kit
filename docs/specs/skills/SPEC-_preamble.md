...
│  │    never a third branch (260805-zxe0): --pane on an explicit
│  │    watch-and-steer directive only; three reachable states
│  │    (running/done/orphaned — failed/failed (no-result) have
│  │    no exit-code channel); requires a reachable tmux server;
