...
| `## Run-Kit (rk) Reference` | The universal silent-fail **detection rule** (`command -v rk`, skip silently when absent) plus a **pointer**. As of `260718-clix` the `rk` command surface (`rk context` — server-URL discovery, iframe windows, proxy URL pattern, Visual Display Recipe — and the `rk notify` contract) is **tool-owned**, read at use-time via `rk skill`; the pointer now forwards only to the **fab-owned** rk usage (the operator's escalation `rk notify` send) in `_cli-external.md` § rk (run-kit), which only operator skills pay for. Every skill still carries the inline detection/fail-silent rule. | `_preamble.md` (detection rule; command surface delegated to `rk skill`; fab-owned usage in `_cli-external.md` § rk) |
...
│   command surface — context, iframe, proxy, server
│   URL, visual recipe, notify contract — delegated to
│   `rk skill`; fab-owned escalation usage in
│   _cli-external § rk)
│
├─ Common fab Commands
