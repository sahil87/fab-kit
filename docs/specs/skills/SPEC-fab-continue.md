...
**CLI completion and pane override**: the source's five-step dispatch procedure reaches the canonical `done` path through its `_preamble.md` § CLI-Adapter Dispatch pointer, where result handling and the unconditional `fab dispatch reap <change> <stage>` live; the source then owns the sequencer transition in its local final step. Pane prerequisite/hard-error rules are reached through the same pointer rather than restated.
