...
> **Dispatch annotation** (260613-fgxx; 260702-aetz; 260805-zxe0): in the APPLY / REVIEW / HYDRATE boxes above, the stage *work* runs inside a dispatched sub-agent (resolved via `fab resolve-agent <stage> --alias`, then **branched on `dispatch=`** — native Agent-tool dispatch when absent, the CLI adapter `fab dispatch` when present, 260702-aetz, with the worker's mode an option *inside* that CLI arm that **auto-resolves** — pane inside tmux, headless outside — so no mode flag is passed by default and `--pane`/`--headless` only force one, 260805-zxe0 / 260805-l9ng — dispatched by the one-stage sequencer). The `Bash: fab status finish/fail/reset` lines shown in those boxes are run by the **sequencer** after the block returns — the dispatched block runs **no `fab status` transition command** (its prompt carries the carve-out: no transition commands, but a terminal `fab status refresh` — a pull-based recompute — is required), and that holds **unchanged on the pane path, where a user may steer the worker mid-stage: steering is contract-neutral**. The boxes show the end-to-end stage picture, not block-internal actions. INTAKE is the only box that runs in the main session.
