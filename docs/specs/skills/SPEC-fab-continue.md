...
**CLI completion and pane override**: the post-intake dispatch contract makes the canonical `done` path explicit — read the result, run `fab dispatch reap <change> <stage>` unconditionally, then let the sequencer transition — and its `--pane` override wording points to `_preamble.md` § CLI-Adapter Dispatch for the prerequisite/hard-error rules instead of restating them.
