...
**Source organization:** Archive and restore share one comparative Key Properties table.

Archives a completed change (post-hydrate) or restores an archived change. Delegates all mechanical operations — move, `.fab-dispatch/{id}/` state-dir deletion, index, backlog mark-done, and pointer — to the `fab change archive` CLI; the skill only formats the YAML output. Backlog marking is mechanical (exact change-ID match, flipped in place), not interactive. The dispatch state dir deleted on archive — shared by both `fab dispatch` modes, headless and pane — is one of the two `fab dispatch` cleanup paths (transient comms, not history) and is **not recreated on restore** (6sgj).
