...
| gemini | Interactive `gemini` / headless `gemini` with the prompt on **stdin** (read as the prompt in non-TTY mode); the **`-p` caveat** — `-p` takes prompt TEXT appended after stdin, so it is not the piping flag and the built-in `dispatch_command` omits it; `-m <id>`, and **no effort flag** at all (the built-in grammar omits `{effort}`); discovery via `gemini --help`. Draws the template consequence: a resolved effort has nowhere to go and is not injected, which is intended — provider grammar is the provider's and fab never appends a flag the CLI lacks |
...
The command strings and native flag are the same independent capabilities fab-kit ships as **built-in providers** (`fab config explain`): `claude`, `codex`, and `gemini` are all in the built-in `defaultProviders` table as of 260805-j3cm (its rows live in `internal/agent`'s embedded `defaults.yaml`), so naming one resolves with **no `providers:` config at all** — a `providers:` block is now for *overriding* capabilities or supplying fills, not for registering codex/gemini. Claude ships `session_command`, `dispatch_command`, and `native: true`; codex/gemini ship both command fields and no native capability. Capability presence says how, while `dispatch.mode` chooses a preference ceiling and descends `pane → native → headless`. The built-ins carry **per-role fills** too (260806-ywkx), so naming one on a *fill-consuming* path — a depth knob, `agent.profiles.<role>.provider`, or `fab resolve-agent --provider` — resolves a real model for every role; `fab agent --provider` is excluded, since it bypasses both fill sources per the § Spawn Composition caveat. The non-claude fills are refreshed at kit-release cadence and overridden by `providers.<name>.profiles.<role>.{model,effort}` (scope `both`) or per invocation via `--model`/`--effort`. This dictionary is markdown, so it updates per kit release with no behavior risk.
...
4. **Markdown dictionary, data-carrying binary built-ins.** The built-in `defaultProviders` table — shipped inside the binary as `internal/agent`'s embedded `defaults.yaml` — carries the three launch capabilities and the *per-role fills* for all three providers; the operational knowledge — quirks, discovery recipes, the MCP bridge — stays in markdown. This keeps the dictionary updatable per kit release with zero behavior risk while making a non-claude provider resolvable, with a real model per role, at zero config. The opt-in posture is preserved where it matters — for *behavior*: a built-in provider is inert until a depth knob, a role override, or a flag names it. *(Lineage: 260731-ho9y kept codex/gemini as uncomment-to-opt-in template text; 260805-j3cm promoted them to built-ins for grammar strings only; 260806-ywkx added the fills.)*
