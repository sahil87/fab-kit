...
4. **Markdown dictionary, grammar-only binary built-ins.** The built-in `defaultProviders` table — shipped inside the binary as `internal/agent`'s embedded `defaults.yaml` — carries the *grammar* for all three providers (260805-j3cm promoted codex/gemini from uncomment-to-opt-in template text to built-ins) and fill values for none of them; the operational knowledge — quirks, discovery recipes, the MCP bridge — stays in markdown. This keeps the dictionary updatable per kit release with zero behavior risk while making a non-claude provider resolvable with zero config. The opt-in posture is preserved where it matters — for *behavior*: a built-in provider is inert until a depth knob, a role override, or a flag names it. *(Originally: the Go table stays claude-only, codex/gemini remain template text — reversed narrowly, for grammar strings only, in 260805-j3cm.)*
