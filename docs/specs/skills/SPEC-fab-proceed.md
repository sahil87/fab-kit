...
**Source organization:** One role table governs prefix dispatch; `_intake`, switch/branch, and `fab-fff` retain their distinct local roles.

Context-aware orchestrator — detects pipeline state via a 5-step detection pipeline, runs prefix steps (create-intake via `_intake`, fab-switch, git-branch) as subagents, then delegates to `/fab-fff` via the Skill tool. No arguments, no flags — infers everything from context. Idempotent — re-running detects completed steps and skips them. Reads `_preamble.md` (per skill convention) but skips running preflight and defers project-context loading to `/fab-fff`. **Per-stage model** (260613-l3ja; prefix steps resolve a role 260719-g55d): each prefix-step dispatch resolves a **role by name** before dispatching, surfaces `model=/effort=/provider=/dispatch=`, and always dispatches through the native two seams; a resolved `dispatch=` line—selected by the shared `dispatch.mode` capability ladder—is surfaced for compliance visibility but not taken or executed — `/fab-switch` and `/git-branch` resolve `fab resolve-agent fast --alias`, while the `_intake` create-intake dispatch resolves `fab resolve-agent default --alias` (closing the one dispatched path that formerly resolved no role; intake stays advisory-only on the foreground `/fab-new` path). This carve-out is required because `default`/`fast` are not pipeline stages accepted by `fab dispatch start <change> <stage>`, and prefix steps have no `{stage}-result.yaml` contract. Role-name resolution reuses the resolver's positional role-name acceptance (the same path `fab agent <role>` uses) — skill wiring only, no Go change — and preserves the invariant that a dispatched step resolves the same role regardless of caller. The delegated `/fab-fff` resolves each of its own pipeline stages per `_preamble.md` § Subagent Dispatch → Per-Stage Model Resolution.
...
| Agent | When | Purpose |
|-------|------|---------|
| _intake (Create-Intake Procedure) | Substantive + no intake, OR substantive + ≥1 intake but none clearly relevant | Create change from synthesized description (conversation only — never bypassed drafts) via the shared procedure (read `.claude/skills/_intake/SKILL.md`) with `{questioning-mode} = promptless-defer` (260613-3xaj — replaces the prior full-`/fab-new` dispatch). `promptless-defer` IS the **defer-and-surface contract** (260612-w7dp): asks no questions; would-be-asked Unresolved decisions land in the intake's `## Assumptions` as `Deferred — promptless dispatch` rows, are returned in the result, and `/fab-proceed` surfaces them before delegating to `/fab-fff` (whose intake gate is the structural backstop — a deferred decision blocks the gate by itself only when its composite is below 20, though a composite ≥ 20 row still adds penalty and can help fail the gate alongside other weak rows; blocking is emergent from the demerit curve, not a special gate, so a genuine unknown scored with honestly-low dimensions fails the gate). This is the `_srad.md` § Critical Rule promptless-dispatch carve-out — defer-and-surface satisfies the MUST-ask when no user is reachable. The procedure stops at intake `ready` (no activate/branch), so the create-new rows chain `/fab-switch` + `/git-branch` after it |
