...
│     │      **Impact** = one normalized table (Scope | + / − | Net) with the
│     │      raw/true/impl/tests/excluded taxonomy (true always = post-exclude
│     │      diff), rows DROP not reshape (raw only when ≠ true; bold true
│     │      always; nested └ impl/└ tests only with a tests pair) + an italic
│     │      provenance caption (excludes via actual true_impact_exclude values,
│     │      never hardcoded · generated by fab-kit vX.Y.Z);
