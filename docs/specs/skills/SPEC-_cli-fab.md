...
| fab hook | Claude Code hook subcommands. fab registers no hooks (empty mapping table, ioku): the former session-scoped handlers (`session-start`, `stop`, `user-prompt`) that wrote `.fab-runtime.yaml` `_agents` state are divested to run-kit's `@rk_agent_state` pane-option convention — they survive one release as no-op shims; `sync` is retained one release but now fully inert — it registers nothing and no longer rewrites legacy scripts (so it cannot re-mint the entries the `2.13.6-to-2.14.0` migration deletes). Artifact bookkeeping is likewise no longer a hook — pull-based via `fab status refresh` |
