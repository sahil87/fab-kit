...
| fab config (reference / show / init --system) | `config` command group over the project configuration. **`reference`** prints the fully-commented reference config.yaml (all available options — binary- and skill-consumed keys), generated by walking a per-field metadata table (`internal/configref`: canonical `default`/`description`/`scope`/`advertise`/`renamed_from` per row) whose constant-backed defaults are sourced from the binary's Go constants (`agent.DefaultSessionCommand`, tier profiles, stage names — no second copy); pure query, byte-stable stdout, exit 0 (a usage error from `cobra.NoArgs` exits non-zero). `--json` emits the same table as a deterministic flat JSON array (`{key, default, description, scope, advertise, renamed_from(omitempty)}`). **`show [--origin]`** prints the config; plain output is the merge of the two FILES (project over system) — built-in defaults are NOT materialized here (they apply at point-of-use), surfaced explicitly only by `--origin`, which adds per-field provenance (project path / system path / `default`, the `git config --show-origin` precedent) with per-key drill-down for map-valued fields — pure query, `cobra.NoArgs`, requires a fab repo. **`init --system`** writes a `~/.fab-kit/config.yaml` scaffold of only the system-overridable fields (`scope: system`/`both`), all commented, generated from the same registry (cannot drift); refuses to overwrite an existing file (non-zero exit, no `--force`); bare `init` (no `--system`) is a usage error. The three-layer cascade (`internal/config.LoadPath`: project > system `~/.fab-kit/config.yaml` > defaults; per-field deep merge — maps per-key, lists replace, scalars replace; fail-open; project-scoped fields in the system file pruned with a `fab: warning:`) is what `show` renders and what `scope` now drives — `scope` gained behavioral consumers (cascade enforcement + the `init --system` filter); `advertise`/`renamed_from` remain metadata-only until `fab config upgrade` (Change 3). Schema recorded in `docs/specs/config.md` |
