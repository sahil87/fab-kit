...
| **Confidence score** | A 0.0–5.0 numeric measure of how well-resolved a change's decisions are. Resolution-Average formula: `score = (mean / 20.0) * cover` where `mean` is the average per-row composite (`0.25*S + 0.30*R + 0.25*A + 0.20*D`) over rows with parseable dimensions and `cover = min(1.0, total_decisions / expected_min)`. Score is 0.0 if any row is Unresolved or trips the Critical Rule (`R < 25 AND A < 25`). Stored in `.status.yaml`. |
