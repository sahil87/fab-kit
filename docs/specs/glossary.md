...
| **Dispatch adapter** | The mechanism that turns "run stage S as a worker" into a launched worker and an observed result. Three exist, sharing one protocol (result file + standard context files + terminal `fab status refresh`) and one byte-stable state vocabulary: **native Agent-tool** (in-harness sub-agent), **headless CLI** (`fab dispatch start` — a detached process observed via its exit file), and **interactive pane** (`fab dispatch start --pane` — an interactive worker in a tmux window the user can watch and steer, observed via result-file presence + pane liveness, so only `running`/`done`/`orphaned` are reachable). Resolution (stage → tier → `{provider, model, effort}`) is adapter-independent. See [harness-adapters](harness-adapters.md). |
