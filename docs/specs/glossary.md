...
| `/fab-adopt` | Brings a completed off-pipeline change (scenario B — a branch authored without fab, with an OPEN or not-yet-created PR) into the pipeline. The real pipeline entered late with **apply** marked `skipped`; intake is reconstructed from the diff, review runs outward-only, hydrate/ship/review-pr run for real. A MERGED PR (scenario A) is out of scope. See **Adopt**. |
