...
| Term | Definition |
|------|-----------|
| **Artifact** | A markdown file generated by a skill at a specific stage — `intake.md`, `spec.md`, or `plan.md`. Each artifact lives inside a change folder. |
...
| **Stage** | One of the 7 sequential phases a change passes through: intake, spec, apply, review, hydrate, ship, review-pr. Tracked in `.status.yaml` via the `progress` map (the entry marked `active` is the current stage). |
...
| **Apply** | Stage 3. Two sub-steps in a single skill invocation: (1) entry — generate `plan.md` from `spec.md` (Plan Generation Procedure); (2) main — execute the unchecked tasks under `## Tasks`, run tests after each, mark `[x]` on completion. Resumable via `plan.md`'s checkbox state. |
| **Review** | Stage 4. Validates implementation against spec and `plan.md` `## Acceptance` items, runs tests, applies code-quality checks. Can loop back to earlier stages on failure. |
| **Hydrate** | Stage 5. Validates review passed and hydrates change artifacts into memory files. The change folder remains in `fab/changes/` — archiving is a separate step via `/fab-archive`. |
| **Ship** | Stage 6. Commits all local changes, pushes to the remote branch, and creates a GitHub PR in draft state. Handled by `/git-pr`. Idempotent — skips steps already done. |
| **Review-PR** | Stage 7. Fetches PR review comments (from humans or automated reviewers — Copilot, Codex, or Claude), triages each as fix/defer/skip, applies fixes, and posts replies. Handled by `/git-pr-review`. |
...
| `/fab-continue` (apply) | Generates `plan.md` from `spec.md` (entry sub-step) and executes the unchecked tasks under `## Tasks` (main sub-step). Resumable — re-invoking skips plan generation when `plan.md` already exists and picks up from the first unchecked task. |
| `/fab-continue` (review) | Validates implementation against `plan.md` `## Acceptance` items, spec, and constitution. On failure, offers rework options (fix code, revise plan, revise spec). |
...
| **Rework loop** | The review-to-earlier-stage feedback cycle. `/fab-continue` (review) can loop back to apply (fix code), plan (revise tasks/acceptance), or spec (revise requirements). |
