...
| `config.yaml` | Project configuration at `fab/project/config.yaml`. Defines project identity, source/test paths, true-impact excludes, plan-acceptance categories (`checklist.extra_categories` for backward-compat), the `/fab-dedupe` seed commands (`consolidate.detectors`), provider session/dispatch commands (`providers:`), agent role tiers (`agent.tiers`), and optional stage hooks. (Automated PR reviewer toggles moved to `code-review.md` § Review Tools.) |
| `providers` | Top-level `config.yaml` table mapping an opaque provider name to its command grammar: `session_command` (opens an interactive agent session — `fab operator`/`fab batch`/`fab agent`, and the interactive-pane dispatch `fab dispatch start --pane`) and/or `dispatch_command` (runs one headless stage task via `fab dispatch start`). Absent `dispatch_command` = native Agent-tool dispatch (no fallback between the two fields, in either direction). fab-kit ships `claude` as the built-in default. Tiers name a provider; the invocation command lives here, not on the tier. A provider is also addressable **directly**, bypassing tiers, via `fab agent --provider <name>`. See [stage-models](stage-models.md). |
| **Role tier** | A named `{provider, model, effort}` profile under `agent.tiers`. Six tiers — `default`, `operator`, `doing`, `review`, `hydrate`, `fast` — with a **fixed, fab-owned** stage→tier mapping (not overridable); projects override only what each tier *means*. A tier is stage-named only where it maps 1:1 to a single referent (`review`/`hydrate`); `fast` keeps its role name because it is multi-referent (the ship stage plus the `/fab-proceed` prefix steps). Per-field inheritance from the `default` tier. Resolved by `fab resolve-agent <stage\|tier>`. See [stage-models](stage-models.md). |
