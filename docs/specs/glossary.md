...
| `providers` | Top-level `config.yaml` table mapping an opaque provider name to its command grammar: `session_command` (opens an interactive agent session — `fab operator`/`fab batch`/`fab agent`) and/or `dispatch_command` (runs one headless stage task via `fab dispatch`). Absent `dispatch_command` = native Agent-tool dispatch (no fallback between the two fields). fab-kit ships `claude` as the built-in default. Tiers name a provider; the invocation command lives here, not on the tier. A provider is also addressable **directly**, bypassing tiers, via `fab agent --provider <name>`. See [stage-models](stage-models.md). |
