...
| **Change** | A unit of work tracked as a folder under `fab/changes/`. Contains all artifacts from brief through hydrate. Identified by its folder name (e.g., `260115-a7k2-add-oauth`). |
...
| **Stage** | One of the 6 sequential phases a change passes through: brief, spec, tasks, apply, review, hydrate. Tracked in `.status.yaml` via the `progress` map (the entry marked `active` is the current stage). |
