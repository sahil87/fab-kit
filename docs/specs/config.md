...
- The `agent.profiles` `default` is a map **keyed by role name** (one entry per `agent.RoleNames()` role —
  ...
  unavailable, so each block shows exactly what it ships): claude all three, codex, `agy`, and `kimi` both
  commands without native. Every entry carries **no** `model`/`effort` — see § Default semantics.
