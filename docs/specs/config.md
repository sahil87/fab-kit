...
| expected kind | Internal YAML shape (`string`/`bool`/`int`/`float`/`null`/`sequence`/`mapping`) used by `fab config set`; dynamic map leaves resolve their exact kind. This does not alter the established `--json` wire shape. |
...
- `fab config set <key> <value> [--system]` and `unset <key> [--system]` — surgical writers through
  ...
  documented deep map leaves; `set` validates the YAML scalar/flow-collection kind while `unset` is
  deliberately kind-ungated so it can repair malformed overrides. Project writes materialize above
  and reconcile the managed fence. System writes accept only `scope: system`/`both`, never add a
  project fence, and create a missing file with the canonical scaffold header. Unsetting an absent
  known key is an exit-zero notice.
