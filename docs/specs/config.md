...
three; codex and agy ship pane/headless grammar without native capability; kimi ships headless
grammar only. The
...
- The `agent.profiles` `default` is a map **keyed by role name** (one entry per `agent.RoleNames()` role —
  ...
  unavailable, so each block shows exactly what it ships): claude all three, codex and agy both
  commands without native, and `kimi` a `headless_command` alone — it is the sole **dispatch-only**
  built-in (`260809-agik`), since an `interactive_command` also confers pane-mode eligibility and pane
  dispatch delivers the stage prompt through that command in a shape kimi accepts in neither form (a
  bare positional is a subcommand error, and it has no initial-prompt flag for a `{prompt}` placeholder
  to target). Every entry carries **no** `model`/`effort` — see § Default semantics.
