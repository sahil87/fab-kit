...
the five `dispatch` defaults (`mode: native`, `column_width: 45`, `min_cols: 50`, `min_rows: 20`, `reap_done: true`), and the four
...
`dispatch.column_width`'s `45`, `dispatch.min_cols`' `50`, `dispatch.min_rows`' `20`, and `dispatch.reap_done`'s `true`); every other row is `null`.
