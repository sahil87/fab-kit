...
# session, native, and headless capabilities; codex, agy, and kimi ship both command fields
# without native capability. Under the default mode, claude resolves native
...
# claude, codex, agy, and kimi all ship an interactive_command, making them
# PANE-mode eligible. The command is pure launch grammar: fab appends nothing
# to it, and a pane worker's prompt is typed in afterwards by `fab dispatch deliver`.
# Both agy and kimi gate a fresh workspace behind an interactive trust prompt, but
# this first-run behavior is cleared by an ordinary readiness-gate judgment round
# (remembered per folder) and their input boxes verify under delivery's echo check.
