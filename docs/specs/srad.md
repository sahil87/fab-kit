...
|-------|---------|--------|
| Computation | `/fab-new`, `/fab-draft` (after intake generation — both via the shared `_intake` Step 7) | `fab score --stage intake` scans `intake.md`, writes to `.status.yaml` |
...
The remaining two planning skills that declare `_srad` are covered by these columns: **`/fab-draft`** follows the `/fab-new` column exactly (a thin delta over fab-new Steps 0–9 — same SRAD posture and budget, minus activation/branch). **`/fab-clarify`** is the escape valve itself: suggest-mode questions are SRAD-prioritized (max 5 per invocation), resolved assumptions are re-graded in the artifact's table, and the intake score is always recomputed. (The report-only analysis skills `/code-reorg` and `/code-dedupe` also declare `_srad`, but only to grade the confidence of report items — no intake posture applies.)
