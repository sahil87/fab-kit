...
- `stage_metrics` is populated by the `fab status` CLI as stages progress — tracks `started_at`, `completed_at`, `driver`, and `iterations` per stage.
...
<!-- The user's raw input/prompt that initiated this change. Preserves original language
     and intent for downstream plan generation (apply entry). -->
