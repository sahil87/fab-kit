...
> **Both index levels are generated artifacts — never hand-edit them.** `fab memory-index`
> regenerates the root and every domain index deterministically from folder contents
> (each file's `description:` frontmatter + H1, plus `git log -1 --date=short` dates).
> The output is byte-stable / idempotent, so two branches can never produce conflicting
