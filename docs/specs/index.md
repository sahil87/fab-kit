...
| [hooks](hooks.md) | Claude Code hooks in fab-kit — fab registers, writes, and owns none; the `fab hook` family removed outright in 2.14.0, agent state consumed from run-kit's `@rk_pane_agent_state` pane-option convention, artifact bookkeeping pull-based via `fab status refresh` |
