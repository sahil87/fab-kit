...
| [stage-models](stage-models.md) | Per-stage model selection via named tiers — `{provider, model, effort}` role tiers (`default`/`operator`/`doing`/`review`/`hydrate`/`fast`) over a top-level `providers:` table, fixed fab-owned stage→tier mapping, `agent.tiers` override, `fab resolve-agent <stage\|tier>`, verbatim pass-through (provider-neutral), default profiles |
| [config](config.md) | Config schema as a per-field metadata table (`default`/`description`/`scope`/`advertise`/`renamed_from`/`init-seed`) — the single source `fab config reference` renders (commented YAML + `--json`); scope taxonomy (project/system/both, decision 6), advertise A/B/C category model, renamed_from carry-forward; the landed three-layer cascade (project > system `~/.fab-kit/config.yaml` > defaults, per-field deep merge, fail-open scope enforcement) + the `fab config show [--origin]` / `fab config init --system` visibility commands (Change 2); the landed `fab config upgrade` managed fence + presence=intent + `fab_version` → `fab/.fab-version` relocation + registry-driven `fab config init --project` (scaffold config.yaml deleted) (Change 3) |
