...
| [stage-models](stage-models.md) | Per-stage model selection via named tiers — `{model, effort}` tiers (`thinking`/`doing`/`ship`), fixed fab-owned stage→tier mapping, `agent.tiers` override, `fab resolve-agent <stage>`, verbatim pass-through (provider-neutral), default profiles |
