...
| [config](config.md) | Config schema as a per-field metadata table (`default`/`description`/`scope`/`advertise`/`renamed_from`) — the single source `fab config reference` renders (commented YAML + `--json`); scope taxonomy (project/system/both, decision 6), advertise A/B/C category model, renamed_from carry-forward; the landed three-layer cascade (project > system `~/.fab-kit/config.yaml` > defaults, per-field deep merge, fail-open scope enforcement) + the `fab config show [--origin]` / `fab config init --system` visibility commands (Change 2); forward-looking presence=intent + the `fab config upgrade` managed fence marked as Change 3 |
