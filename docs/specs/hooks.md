...
|------------|--------------------------|-------------|
| `session-start` | SessionStart | Removed — agent-state production divested to `@rk_pane_agent_state` |
...
Agent active/idle/waiting state is no longer written by a fab hook. The `fab pane` commands READ a tmux **pane user option** `@rk_pane_agent_state` (value grammar, states, and the legacy-name fallback are owned by `src/kit/skills/_cli-fab-pane.md` § fab pane → § agent state) written by run-kit's `rk agent setup` global agent-harness hooks (which cover Claude Code, Codex, Copilot, Gemini, OpenCode — not just Claude). fab reads it with plain tmux commands (`tmux show-options -pv` / the `#{@rk_pane_agent_state}` list-panes format field), so there is no dependency on run-kit software being installed. (When rk *is* installed, `fab pane map` prefers rk's already-reconciled state via its delegated `rk mux panes --json` enumeration — cli-layering Part 8; the plain-tmux read is its fail-open fallback and remains `capture`'s path.) The deleted producer subsystem was: the three hook writes, `WriteAgent`/`ClearAgent`/`ClearAgentIdle`, the throttled GC sweep (`last_run_gc`), the grandparent PID walker (`internal/proc`), the runtime file (`internal/runtime` / `.fab-runtime.yaml`), and the `_agents` matching in `internal/pane`. See `src/kit/skills/_cli-fab-pane.md` § fab pane and `docs/memory/runtime/runtime-agents.md` for the read-side contract.
...
|-------|--------|-----------|
| **Stop** | Removed | Agent active/idle state divested to run-kit's `@rk_pane_agent_state` pane-option convention — fab is a consumer, not a producer |
