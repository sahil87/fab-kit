...
- `fab/project/config.yaml` — project configuration: identity (name/description), `source_paths`/`test_paths`, true-impact excludes, plan-acceptance extra categories, independent provider capabilities (`session_command`, `dispatch_command`, and `native`) plus per-role fills (`providers:`), the two agent depth knobs (`agent.session`/`agent.workers`) and sparse `agent.profiles` overrides, `dispatch.mode` (the `pane → native → headless` preference ceiling), pane placement/reaping settings, optional `stage_hooks`
