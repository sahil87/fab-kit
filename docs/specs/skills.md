...
- `fab/project/config.yaml` — project configuration: identity (name/description), `source_paths`/`test_paths`, true-impact excludes, plan-acceptance extra categories, provider session/dispatch commands and per-role fills (`providers:`), the two agent depth knobs (`agent.session`/`agent.workers`) and the sparse `agent.profiles` overrides, optional `stage_hooks`
