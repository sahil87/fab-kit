...
- `fab/project/config.yaml` — project configuration: identity (name/description), `source_paths`/`test_paths`, true-impact excludes, plan-acceptance extra categories, provider session/dispatch commands and default fill (`providers:`), agent role tiers (`agent.tiers`), optional `stage_hooks`
