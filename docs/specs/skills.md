...
Adding a skill to the kit touches nine integration points. Work through all of them — drift in any one is invisible until an agent hits it.
...
9. **Portability of citations** — a skill cites kit skills, `fab` commands, `fab/project/` files, `$(fab kit-path)/…` assets, or host convention paths; never fab-kit's `docs/specs/*`, `docs/memory/*`, `docs/site/*`, `src/go/*` (Constitution V; guarded by the fab-kit Go test).
