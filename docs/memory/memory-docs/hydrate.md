...
- **The malformed-frontmatter check (xu0k) is a SEPARATE blocking signal the guard does NOT key on.** A corrupted frontmatter block (unclosed fence, or a `description:` that starts with a quote but fails quote-stripping — e.g. the glued-fence `"…text…"---`) makes `fab memory-index --check` **fail (exit ≥ 1) independent of drift**, but it is **not** a tier-2 destructive loss — so the refuse-before-regen guard (which fires only on **exit == 2**) does not treat it as a reason to refuse, and malformed frontmatter routes to a **fix-the-file remediation, not the `→ /docs-reorg-memory` pointer** (a reorg cannot repair source corruption). The corruption still surfaces as a blocking `--check` failure at review-pr / CI (exit ≥ 1). The advisory over-500-char `description:` length warning is likewise **non-blocking** (it never affects `--check`). See [pipeline/schemas.md](/pipeline/schemas.md) § Malformed-Frontmatter Blocking for the exit-code/tier contract.
