...
**Decision**: The root landing's "New here?" navigation note is a per-root config key, `nav_note`, presence=intent — unset renders the built-in default on the legacy memory root, `""` omits the line, a non-empty value renders verbatim.
**Why**: Fits the configurable-roots direction with zero regression for standard layouts and a full escape for non-standard ones. Curated-block ownership cannot carry this: `curatedNavigation` deliberately never seeds from an already-generated index (`adoption.go`), so that route would require new seeding behavior — more invasive than a config key.
