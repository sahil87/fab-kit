...
4. **Index updates**: Run `fab memory-index` — it regenerates the root (domains-only), every domain index, and every sub-domain index deterministically from folder contents + frontmatter + git dates. Never hand-edit index rows or "Last Updated" cells; the command is the single writer
