...
2. **Existing file (merge as current truth)**: Compare spec requirements against current file. **Rewrite the section that already documents the topic to state current truth** — keyed on the topic/section, not the change; superseded statements are removed, not narrated (no "renamed X→Y in {id}", no "was `old.value`"), and provenance in the body stays citation-only (§3.3). Minimize edits to unchanged sections. Keep the `description:` frontmatter accurate and **free of change-ids** (and stamp `type: memory` if the pre-existing file lacks it). Memory↔memory cross-links written use the bundle-relative `/...` form (FKF §7)
3. **Design decisions**: Extract durable decisions from spec. Skip tactical details. Record provenance in citation form — the `*Introduced by*: {change-name}` field (or a trailing `(change-id)` citation), not free-text change-name narration (§3.3)
