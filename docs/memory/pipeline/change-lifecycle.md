...
5. Display the switched change's status summary (three-line format: `Stage: {display_stage} ({N}/6) — {state}` + `Confidence: {score} of 5.0` + `Next: {routing_stage} (via {default_command})`)
