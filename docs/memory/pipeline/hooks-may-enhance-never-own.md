...
The governing principle and the pull-based artifact-state model it produces: **no correctness-critical state may live behind a Claude Code hook, because a hook fires only in the Claude harness** (y022). Artifact-derived pipeline state (`change_type`, intake `confidence`, `plan` counts) is correctness-critical and pull-based. The principle's sharpest form: **a hook is justified only for state that is genuinely underivable and harness-local** — there are zero fab hooks of any kind and no `fab hook` command family (ioku); even push-by-nature session telemetry has a cross-harness owner (run-kit's `@rk_pane_agent_state` convention). This file records the principle, the seam set that operationalizes it, and the rationale for the seams chosen and excluded. The `.status.yaml` field-level mechanics live in [schemas.md](/pipeline/schemas.md); the skill-side contract in [planning-skills.md](/pipeline/planning-skills.md) § Change Type (Pull-Based) and [execution-skills.md](/pipeline/execution-skills.md) § Pull-based bookkeeping; the divested session hooks + the consumer-not-producer model in [runtime-agents.md](/runtime/runtime-agents.md).

> The principle is stated in spec prose and enforced in code: `docs/specs/harness-adapters.md` fixes "hooks may enhance, never own" as a **dispatch-protocol** rule (6sgj) — a harness hook MAY add value around dispatch (telemetry, notifications) but MUST NOT own any step of the protocol, which is complete and correct without any hook. Promotion to a **normative constitution MUST-rule** remains later work (aetz).
>
> **The `fab status refresh` epilogue is the protocol-owned step, written into the dispatch prompts — NOT a hook (3d (aetz)).** Under the dispatch-protocol rule above, the worker-side post-stage state recompute is a **protocol** step, so 3d wires it as a **terminal `fab status refresh <change>` epilogue in the dispatch prompt itself** (part of the dispatch-prompt obligations that bind ALL THREE adapters — native Agent-tool, headless CLI `fab dispatch`, and interactive-pane `fab dispatch --pane` — per `_preamble.md` § Dispatch-Prompt Obligations), never as a harness hook a non-Claude worker would not fire. This is the concrete application of "if a step matters, it lives in the protocol (the prompt epilogue, the result-file obligation), not in a hook a different harness won't run": the same pull-based `fab status refresh` this file's principle produced is now the dispatched worker's own closing action, keeping a cross-harness-dispatched stage's `.status.yaml` consistent with the artifacts it wrote regardless of which harness ran it. The block-contract carve-out reconciles it with "the orchestrator owns all transitions": the prompt prohibits `fab status` *transition* commands but requires this one *recompute* (refresh is not a transition). See [pipeline/execution-skills.md](/pipeline/execution-skills.md) § Status-transition ownership and [runtime/dispatch.md](/runtime/dispatch.md).

## Requirements

### Requirement: Correctness-Critical Artifact State Is Pull-Based, Not Hook-Owned

The four artifact-derived `.status.yaml` field groups — `change_type` (from `intake.md`), `confidence.*`/`confidence.score` (from `intake.md`), and `plan.generated`/`plan.task_count`/`plan.acceptance_count`/`plan.acceptance_completed` (from `plan.md`) — SHALL be recomputed from the on-disk artifacts by `fab status refresh [<change>]` (the change argument is optional: omitted, it resolves the active change via the `.fab-status.yaml` symlink, the same resolution bare `fab preflight` uses; backed by the shared `internal/refresh.Refresh`), never owned by a write-time hook. A hook fires only in the Claude harness, so a hook-owned writer goes silently stale on any edit that does not fire it — a `sed`-style edit, a direct file write, or a non-Claude agent such as `codex` writing the artifact (the blocking case for cross-harness dispatch); pull-based recompute closes every such window (y022).
...
- **THEN** it skips inference and keeps the explicit type (the sticky-explicit guard (jznd), preserved verbatim in the refresh path)

### Requirement: No Session Telemetry Hooks — Agent State Has a Cross-Harness Owner (ioku)

There SHALL be no fab session-telemetry hooks and no `fab hook` command family (ioku). Agent active/idle lifecycle state is owned cross-harness: run-kit's `rk agent setup` writes the `@rk_pane_agent_state` tmux pane-option convention, which any harness (Claude, codex, copilot, gemini, opencode) can read and fab consumes as a pure reader (see [runtime-agents.md](/runtime/runtime-agents.md)). fab carries no producer subsystem (no writes, GC, PID walker, flock, `internal/runtime`/`internal/proc`) and no shim commands — an un-migrated `.claude/settings.local.json` still invoking `fab hook <x>` errors (cobra unknown-command) until the settings entries are removed, by the `2.13.6-to-2.14.0` migration in the checkout it runs in and the `2.15.7-to-2.15.8` migration (weoh) across every worktree (see [migrations.md](/distribution/migrations.md) § `2.15.7-to-2.15.8`) — the honest, accepted consequence of removal without a shim.

This is the principle in its sharpest form: session telemetry is push-by-nature and once looked hook-justified, but state is underivable only while it has no other source — give it an owner and the hook has no reason to exist. **Zero** fab hooks remain of any kind. See [runtime-agents.md](/runtime/runtime-agents.md) § Consumer-not-producer and § The Deleted Producer Pipeline.

#### Scenario: A hook loses its justification the moment its state gets an owner
- **GIVEN** the three session hooks recording agent telemetry (legitimate (y022) — push-by-nature, underivable, harness-local)
- **WHEN** run-kit's `@rk_pane_agent_state` convention gives that telemetry a cross-harness owner (ioku)
- **THEN** the state stops being underivable-and-hook-only, so the hooks — and the whole `fab hook` command family — are removed outright and their producer pipeline is deleted; fab reads the convention as a pure consumer instead
...
**Decision**: The `artifact-write` PostToolUse hook is removed outright (y022) (drops its two `DefaultMappings` rows in both binaries, ships the `2.10.1-to-2.11.0` migration to strip the settings entry) rather than keeping it as a second, redundant writer alongside `fab status refresh`.
**Why**: A hook that "owns" correctness-critical state is precisely the anti-pattern the principle exists to eliminate. Keeping it would leave **two writers** for the same fields — a drift risk and a source-of-truth ambiguity — for no benefit, since refresh already covers every reachable-stale window. The `fab hook artifact-write` subcommand survived one release (y022) as a **silent no-op shim** (exit 0, nothing on stdout) purely so an un-migrated project's still-registered PostToolUse entry did not feed Claude Code ~505 bytes of cobra help as invalid `additionalContext`; it carried no bookkeeping logic. **that shim too is deleted (ioku)** — along with the whole `fab hook` command family — per Sahil's no-retention directive; the `2.10.1-to-2.11.0` migration had already stripped the settings entry, so the risk the shim guarded against was already past for migrated projects.
**Rejected**: Belt-and-braces (hook + refresh both write) — reintroduces the dual-writer drift the change exists to kill. (deleting the shim immediately was initially rejected (y022) to spare un-migrated projects noisy help text; that was revisited once the migration had propagated (ioku) and removed it outright.)
