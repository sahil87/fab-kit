...
- **THEN** the state is no longer underivable-and-hook-only, so the hooks are reduced to inert no-op shims and their producer pipeline is deleted — fab reads the convention as a pure consumer instead
