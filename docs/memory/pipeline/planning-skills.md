---
type: memory
description: "`/fab-new`, `/fab-draft`, `/fab-dedupe`, `/fab-clarify`, `/fab-ff`, `/fab-fff` — the intake-only planning stage: the shared `_generation` and `_intake` (Steps 0–9, interactive | promptless-defer) helpers with thin call-sites; re-run/ID-collision resume semantics; pull-based change_type with sticky explicit set; SRAD v2 demerit confidence scoring in the `_srad` helper; fab-ff/fab-fff as thin wrappers over the `_pipeline` bracket."
---
# Planning Skills
...
The partial contains four procedures — the two **forward** procedures (Intake Generation, Plan Generation) and, (t54n), two **diff-based** procedures (Intake-from-Diff, Plan-from-Diff) for `/fab-adopt`. The **Intake Generation Procedure** (consumers: `/fab-new`, `/fab-draft`, `/fab-dedupe` (4v91), and `/fab-continue`'s intake-`active` regeneration row — `/fab-draft` and `/fab-dedupe` reach it through `_intake.md` Step 5, and `/fab-dedupe` runs it once per accepted cluster group) and the **Plan Generation Procedure** below (consumers: `/fab-continue`, `/fab-ff`, `/fab-fff`, at apply entry). `/fab-continue` belongs to **both** forward consumer groups (d9rs):
...
- **`interactive`** — used by `/fab-new`, `/fab-draft`, and `/fab-dedupe`. Step 8 asks via SRAD (SRAD-driven question selection, no fixed cap, conversational mode when 5+ Unresolved). Byte-identical to the pre-3xaj inline fab-new Step 8.
...
**The four consumers are thin call-sites** (3xaj), the fourth being `/fab-dedupe` (4v91):
...
| `/fab-dedupe` | `_intake(interactive)`, stop at `ready` — **the fan-out call site (4v91)**: the procedure runs once per accepted cluster group, so one invocation drafts N intakes. Step 0 always takes natural-language input (`consolidate {shared behavior} into {canonical home}`), so no backlog/Linear collision check ever fires and each run creates a fresh change |
...
**Decision**: `/fab-new` Steps 0–9 (the "create an intake" procedure) are lifted into a single internal helper `_intake.md`, parameterized by one knob `{questioning-mode}` (`interactive` | `promptless-defer`) applied only at Step 8. The consumers become thin call-sites — three at extraction (`/fab-new`, `/fab-draft`, `/fab-proceed`), joined by a fourth, `/fab-dedupe` (4v91), the first to run the procedure more than once per invocation. The activate/branch tail (`/fab-new` Steps 10–11) and `/fab-proceed`'s state-detection stay at the call site (the EXTRACTION BOUNDARY — do NOT over-extract). This is a pure restructure: behavioral parity is the bar, zero Go code touched.
...
*Introduced by*: 260613-3xaj-extract-intake-helper; *Updated by*: 260728-4v91-add-fab-dedupe-skill (fourth consumer `/fab-dedupe`, the first fan-out call site — one invocation runs Steps 0–9 once per accepted cluster group)
