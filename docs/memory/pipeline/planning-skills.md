---
type: memory
description: "`/fab-new`, `/fab-draft`, `/fab-clarify`, `/fab-ff`, `/fab-fff` — the intake-only planning stage: the shared `_generation` and `_intake` (Steps 0–9, interactive | promptless-defer) helpers with thin call-sites; the fab-new-owned micro-change backstop (Step -1); re-run/ID-collision resume semantics; pull-based change_type with sticky explicit set; SRAD v2 demerit confidence scoring in the `_srad` helper; fab-ff/fab-fff as thin wrappers over the `_pipeline` bracket."
---
# Planning Skills
...
Command invocations are logged by the skill itself: after preflight, each skill calls `fab log command "<skill-name>" "<id>"` directly (`_preamble.md` §2 step 4) — best-effort, always exit 0, no shell guard. `fab preflight` logs nothing on the skill's behalf. All event commands (`start`, `advance`, `finish`, `reset`, `skip`, `fail`) accept an optional `driver` positional argument; skills always pass it to identify the invoking skill (e.g., `fab-continue`, `fab-ff`).
...
#### Micro-Change Backstop (Step -1)

Before creating anything, `/fab-new` evaluates the described change against the **micro criteria** — `fab-new.md` § Step -1 is the **canonical owner** of the criteria text; `fab-proceed.md`'s create-new-row backstop points at it rather than restating it (owner-or-pointer, code-quality.md) (slpa). A change is micro when ALL THREE hold: no memory/spec impact (empty Affected Memory), no behavior-contract change, and a single-spot ~1-task edit; the tie-breaker is default-closed — **when unsure, use fab** (memory drift is the expensive failure the backstop must not reintroduce). When all three hold, `/fab-new` — interactive by posture — confirms inline ("This looks like a direct fix — handle it without fab, or continue with a tracked change?"): a continue answer IS the explicit go-ahead and proceeds to Steps 0–9 unchanged; a decline STOPs with nothing created and no `Next:` line (mirroring the gap-analysis no-folder outcome) — the right move is a direct edit + commit outside fab. Any failed or uncertain criterion skips the confirm. The criteria deliberately separate a **follow-up tweak to a change still in flight** (belongs to that change — amend it, not `/fab-new`) from a **standalone micro fix** (the direct-edit case). The four routing skills' frontmatter descriptions (`fab-proceed`, `fab-fff`, `fab-ff`, `fab-new`) carry a byte-identical compressed anti-trigger form of the same criteria — descriptions are the one kit-controlled surface read at routing time, without loading files, so they carry the self-contained form rather than a pointer (slpa). `fab-draft` is deliberately ungated, and `_intake.md` carries no backstop (it stays at the call sites per the EXTRACTION BOUNDARY).

#### Slug Generation and Change Creation

The agent generates a 2-6 word slug (lowercase, hyphen-joined, no articles/prepositions) from the description. The slug SHALL NOT include a parsed Linear issue ID — it contains only the descriptive portion (e.g., `add-oauth`); the issue ID is recorded in `.status.yaml`'s `issues` array (via `fab status add-issue`), never in the folder name. An explicit Linear ID handed to a creation skill is the **pull** linking path (fetched via the Linear MCP at intake); the post-intake **push** path — find-or-create from a natural-language intake — is `/fab-issue`, covered in [issue-linking.md](/pipeline/issue-linking.md). The folder name format `{YYMMDD}-{XXXX}-{slug}` is constructed by the Go CLI's `fab change new`, which handles date generation, random/provided 4-char ID, collision detection, directory creation, `created_by` detection, `.status.yaml` initialization, and status integration (`start intake fab-new`; command logging via `fab log` when `--log-args` is passed). The skill calls `fab change new --slug <slug> [--change-id <4char>] [--log-args <description>]` as a single operation and captures the folder name from stdout.
...
- **Backlog ID** (4-char — embedded in the folder-name prefix): `fab resolve --id {id} --or-none`, then compare its stdout (the canonical 4-char ID of the matched change) for **equality** with `{id}` — only an exact ID match names an existing change for this ID (w7dp). The `--or-none` flag (dow0) prints `(none)` + exit `0` when no change resolves, so the probe reads a valid "no existing change" on the success channel instead of a red error frame; the exact-ID equality compare is unchanged and naturally rejects the sentinel — `(none)` can never equal a 4-char ID. Resolution is substring-based, so `{id}` occurring inside another change's *slug* also resolves — with a different canonical ID — and MUST NOT route to resume; on an exact match the folder name comes from `fab resolve --folder {id}`.
...
- **`promptless-defer`** — used by `/fab-proceed`'s dispatch. Step 8 records each would-be-asked Unresolved decision as a deferred Unresolved row (Rationale `Deferred — promptless dispatch`) instead of asking, pointing to `_srad.md` § Critical Rule's canonical promptless-dispatch carve-out. Preserves `/fab-proceed`'s defer-and-surface contract exactly (see § Promptless Dispatch above).
...
Auto mode (the `[AUTO-MODE]` prefix) operates on `intake.md` only and returns `{resolved: N, blocking: N, non_blocking: N}`. No orchestrator invokes it (j6cs) — `/fab-ff`/`/fab-fff` dropped their auto-clarify steps — but the mode is retained for future use:
...
### Micro-Change Backstop Criteria Owned by `fab-new.md`
**Decision**: The skill-body micro-change criteria + backstop text has a single owner — `fab-new.md` § Step -1 — with `fab-proceed.md`'s create-new-row backstop pointing at it and stating only its posture delta (gate-failure-style stop, "use fab anyway" conversation go-ahead, no new flag). Frontmatter descriptions carry a byte-identical compressed self-contained form of the criteria instead of a pointer. There is no third pipeline lane for micro changes: no-flow means fab is not invoked at all.
**Why**: The backstop is an intake-entry check ("should this be a tracked change at all?") and `/fab-new` is the intake-entry skill; its interactive posture is the natural home of the confirm flow. Descriptions are read at routing time without loading files, so a pointer cannot work there. A "nano-flow" lane would still create a change folder, branch, and status file — exactly the overhead micro fixes must not pay.
**Rejected**: `_intake.md` as owner (would leak the backstop to `fab-draft` or need a second knob against its single-fork design); `_preamble.md` as owner (expands the always-load surface for all skills with routing-only content); `fab-proceed.md` as owner (a lean dispatch table, far from the interactive flow that uses the criteria most); a third pipeline lane (rejected outright — the lane overhead is the complaint).
*Introduced by*: 260815-slpa-right-size-fab-routing

### SRAD Autonomy Framework
**Decision**: All planning skills use the SRAD framework (Signal Strength, Reversibility, Agent Competence, Disambiguation Type) to evaluate decision points and assign confidence grades (Certain, Confident, Tentative, Unresolved). All four grades are recorded in every Assumptions table with a required Scores column (`S:nn R:nn A:nn D:nn`). Unresolved rows include status context in Rationale. Each skill has a defined autonomy level and interruption budget. Dimensions are evaluated on a continuous 0–100 scale, aggregated into a composite via weighted mean — **`0.20·S + 0.30·R + 0.30·A + 0.20·D` (4yi8)** — and mapped to **indicative-only** grades via half-open bands: **composite ≥ 80 Certain, 50 ≤ c < 80 Confident, 20 ≤ c < 50 Tentative, else Unresolved (aligned with the demerit penalty curve's knees) (4yi8)**. The grade is **derived from the composite and never read by any formula** — it can never contradict its own dimensions. **There is no Critical-Rule override and no hard-fail short-circuit (4yi8)**: blocking is **emergent from the demerit curve** (a `composite < 20` row penalizes ≥ 2.0), and reversibility is carried by R's 0.30 composite weight rather than a separate rule. The autonomy tables cover all six declaring planning skills with four core-skill columns plus explicit rows for fab-draft (fab-new posture) and fab-clarify (the escape valve). Worked-example arithmetic is internally consistent: Example 1 (high-ambiguity) blocks via the curve, Example 2 (low-ambiguity) scores 5.0, and Example 3 surfaces a single risky decision through R's weight — none rely on a hard-fail. The framework **lives in the dedicated `_srad.md` helper** (`_preamble.md` keeps a ~3-line pointer) (zc9m), declared via frontmatter `helpers:` by the planning skills — `fab-new`, `fab-draft`, `fab-continue`, `fab-ff`, `fab-fff`, `fab-clarify` — plus the report-only analysis skills `code-reorg`/`code-dedupe` for report-item grading. Other skills do not load it. The Critical Rule carries a **promptless-dispatch carve-out** (w7dp) cross-referencing fab-proceed's defer-and-surface contract: under a promptless dispatch the MUST-ask is satisfied by recording the decision as a `Deferred — promptless dispatch` Unresolved row and surfacing it (the intake gate backstops — a genuine unknown scored with honestly-low dimensions lands at `composite < 20` and the demerit curve's ≥ 2.0 penalty blocks the gate) (4yi8); wherever a user is reachable, the MUST-ask applies unchanged — resolving the contradictory MUSTs a subagent loading both files would otherwise receive.
**Why**: Replaces ad-hoc question selection with a principled, consistent framework. Ensures high-blast-radius decisions are always surfaced while low-value prompts are eliminated. The four-dimension scoring prevents both over-asking and silent high-risk assumptions. The R-biased weighting (0.30, alongside A at 0.30) (4yi8) carries the Critical Rule's intent at the formula level — an irreversible decision lands in a worse composite band and is penalized harder automatically, so no separate override is needed. The extraction (zc9m) keeps the framework out of the always-load layer — only the skills that grade decisions pay for it.
...
*Introduced by*: doc/fab-spec/TEMPLATES.md, doc/fab-spec/README.md; *Updated by*: 260207-09sj-autonomy-framework
...
*Introduced by*: doc/fab-spec/SKILLS.md; *Updated by*: 260215-237b-DEV-1027-redefine-ff-fff-scope (moved from fab-ff to fab-fff); 260314-q5p9-redesign-ff-fff-scopes (dropped frontloaded questions entirely); 260601-j6cs-merge-spec-into-apply (single intake gate after spec stage removal)
...
*Introduced by*: doc/fab-spec/SKILLS.md
...
**Decision**: The first pipeline stage is named `intake` (not `brief`). The artifact is `intake.md`. The Intake Generation Procedure lives in `_generation.md` (which (j6cs) contains only this and the unified Plan Generation Procedure), with an explicit generation rule emphasizing the intake is a state transfer document.
...
### Shared Generation Partial
**Decision**: Artifact generation logic lives in a single shared `_generation.md` partial, which (j6cs) defines two procedures: the **Intake Generation Procedure** and the unified **Plan Generation Procedure** (invoked by apply at entry, emitting `## Requirements` + `## Tasks` + `## Acceptance` in one walk). The standalone Spec Generation Procedure was deleted — requirement generation folded into plan generation. Each consumer retains its own orchestration logic.
**Why**: Generation steps were nearly identical across skills, requiring every fix or behavior change to be applied in two places. Centralizing eliminates drift and makes generation behavior authoritative in one location. The Tasks + Checklist procedures are collapsed into one Plan Generation Procedure (qszh); the Spec Generation Procedure is folded into it too (j6cs), so a single pass over the intake-derived design emits requirements, tasks, and acceptance — the strongest possible alignment guarantee (single skill call, single context window).
...
*Introduced by*: 260613-3xaj-extract-intake-helper; *Updated by*: 260728-4v91-add-fab-dedupe-skill (added `/fab-dedupe` as a fourth, fan-out consumer), direct change 2026-08-24 (that consumer removed — the skill became the report-only `/code-dedupe`, back to the three extraction-time consumers)
...
**Why**: The `tasks` stage was a no-decision gate. `/fab-continue` advanced spec → tasks → apply back-to-back; users never stopped at tasks. Every change paid the wall-time, token, and `.status.yaml` cost of a transition that had no decision content. Folding generation into apply makes drift between Tasks and Acceptance mechanically impossible (single skill call, single context window, single LLM, single file). The first collapse (qszh) stopped at `tasks` and preserved the spec stage; the spec stage folded in later (j6cs) (see "Spec Stage Merged into Apply" below), so apply entry now co-generates `## Requirements` alongside `## Tasks` + `## Acceptance`.
**Rejected**: (a) Keep two files, add cross-check step — adds ceremony, not less. (b) Drop checklist, review reads tasks directly — loses the imperative-vs-declarative framing review depends on. (c) Merge artifacts only, keep `tasks` stage — fixes drift but keeps the no-decision gate; the per-type-skip workaround (xvaz) remains relevant. (d) Drop both spec AND tasks — loses `/fab-clarify spec`, breaks per-type spec gate, weakens review. (e) Rename `tasks` → `plan` stage — same gate, new name. (f) Rename `apply` → `execute`/`implement` — semantic gain doesn't justify migration churn across state table, `.status.yaml`, all skills, muscle memory.
*Introduced by*: 260423-qszh-merge-tasks-checklist; *Replaces*: the proposal in `260423-xvaz-skip-tasks-simple-types` (per-type skip policy for the tasks stage) — obsolete by construction with the stages merged: there is no separate stage to skip. Simple changes naturally produce a tiny `plan.md` and execute in seconds; no skip policy needed. The xvaz folder will be archived by a separate user-initiated `/fab-archive 260423-xvaz...` action.

### Spec Stage Merged into Apply; `plan.md` Carries `## Requirements` (j6cs)
**Decision**: The `spec` stage is removed from the pipeline (7 → 6 stages: intake → apply → review → hydrate → ship → review-pr). The requirement discipline that lived in a separate `spec.md` artifact (RFC-2119 + GIVEN/WHEN/THEN) is absorbed into `plan.md` as a `## Requirements` section, co-generated at apply entry alongside `## Tasks` + `## Acceptance`. The `spec.md` template is removed; `spec.md` does not exist as an artifact. The canonical artifact set becomes `intake.md → plan.md → code`. Any `fab status` event targeting `spec` hard-errors (mirroring the `tasks` branch). SRAD confidence scoring is frontloaded to intake as the **single** confidence gate (flat 3.0 for all types); the per-type spec gate and the `confidence.indicative` flag are retired.
...
**Decision**: `/fab-ff` and `/fab-fff` describe execution behavior in their own orchestration context, rather than literally invoking `/fab-continue` as a sub-skill. That description lives once in the shared `_pipeline.md` bracket (szxd) (parameterized by `{driver}`/`{terminal}`) rather than duplicated inline in each file — but the principle stands: the orchestrators dispatch `/fab-continue` *behavior sections* as subagents and own all status transitions; they never invoke `/fab-continue` as a skill.
**Why**: These skills have fundamentally different orchestration: bail behavior, autonomous rework, resumability across all stages. Literal sub-skill invocation would add complexity (nested preflight checks, status conflicts) without benefit. The extraction (szxd) removed the remaining cost of this stance — the two files shared ~88% of their content verbatim, and every bracket edit had to land twice (with documented drift: "Two gates" vs the single intake gate).
