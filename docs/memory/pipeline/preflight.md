...
Skills that operate without an active change do not run preflight: `/fab-setup`, `/fab-new`, `/fab-switch`, `/fab-discuss`, `/fab-incognito`, `/fab-help`, `/code-dedupe` and `/code-reorg` (which must not resolve or disturb a change), and `/fab-operator` (a listed exception to the always-load layer). Exempt skills that still want telemetry call `fab log command` directly.
