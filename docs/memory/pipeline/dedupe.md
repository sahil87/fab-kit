...
Step 2 reads `consolidate.detectors` from project config, defaulting to jscpd alone when absent ([_shared/configuration.md](/_shared/configuration.md) § `consolidate`). Each entry is a shell command template with `{paths}` (space-joined resolved scope paths) and `{out}` (a scratch dir created for the run) substituted before execution. Two rules bind:
