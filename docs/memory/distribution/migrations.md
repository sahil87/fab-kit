---
type: memory
description: "The migration system — dual-version model (engine VERSION vs fab/.kit-migration-version), the {FROM}-to-{TO}.md file format, range-based applicability, binary-owned discovery (fab migrations-status [--json] / DiscoverMigrations; exit 3 = not-a-fab-managed-repo, 1 = genuine error), the /fab-setup migrations runner (discovery delegated, application LLM-driven), version-drift detection, and the shipped migration catalog."
---
# Migrations
