...
Bare `fab setup` runs an interactive interview layered over the setup-check probe. All capability detection comes from one `setupcheck.Run(setupCheckInput())` call — the same call `fab setup check` makes; the wizard never asks a capability question, only preference questions over options pre-filtered to what the probe found. Flow: non-TTY guard → write-target resolution → existing-system-scaffold refresh → setup probe/read model → scope banner → 4-question default path → opt-in advanced section → diff-before-write summary → surgical writes.
...
- **System-scaffold warm-up** — before the banner and first question, an existing `~/.fab-kit/config.yaml` is reconciled through `configupgrade.Upgrade(SystemTarget(...))`. Clean or missing files are silent (a missing file stays missing, preserving the all-Enter zero-write invariant); a changed file prints exactly one `Refreshed … reference block` advisory; a reconciliation error warns on stderr and the interview continues. The warm-up never prompts.
- **Writes** — a diff summary (one `<key>: <old> → <new>` line per changed answer plus the target tier) and a `[Y/n]` confirmation precede any preference write. Zero changed answers (the all-Enter run) prints "nothing to change" and exits 0; when the system scaffold was already current or absent, repeated all-Enter runs touch no file (Constitution III). Confirmed writes go through the existing `fab config set` path in-process — `configupgrade.SetSystem` for the system tier, `configupgrade.Set` for the project tier, with the target path from `configMutationPath` — never a child-process exec, with `warnIfShadowed` firing per written key so a write shadowed by a higher tier says so.
- **`--defaults` / non-TTY** — `--defaults` runs the full flow non-interactively accepting every default (banner, resolved answers, and the zero-preference-write summary), composable with `--project`; the system-scaffold warm-up above may still rewrite an existing stale scaffold, while a clean or missing scaffold keeps the whole run zero-write. Non-TTY stdin without `--defaults` fails with a usage hint naming the flag (stdlib-only `os.ModeCharDevice` detection, the `batch_archive.go` pattern); EOF on stdin falls back to a question's default, so the interview can never hang on an exhausted reader.
...
- **User-level operator skill** (`user-skills`) — the `fab sync`-written `fab-operator` pointer skill under `$HOME` (see [kit-architecture.md](/distribution/kit-architecture.md) § Agent Skill Deployment): OK per present tier — `~/.agents/skills/fab-operator/SKILL.md` is always expected, `~/.claude/skills/fab-operator/SKILL.md` is expected only when `claude` is on PATH (absent `claude` yields an informational "tier not expected" finding); Warn when an expected tier is missing, with the fix hint to run `fab sync` in any fab project (the `rk operator -L` respawn window opens in `$HOME`); Warn ("stale — re-run `fab sync`") when a present file's `description:` line differs from the kit's `skills/fab-operator.md`. Never failure-severity, so the doctor's exit code is unaffected. The probe's extra reads stay read-only: the two user-level skill files and the kit's `fab-operator.md` frontmatter join the kit cache's `VERSION` as the fixed read set.
...
| Skill deployment (Claude Code, OpenCode, and the generic agents dir) | `fab-kit sync` | Deploys from `{kit-dir}/skills/` to three targets; `.agents/skills/` is always on; `.claude/skills/` and its settings scaffold are gated on `claude`, while `.opencode/commands/` is gated on `opencode` availability — see [kit-architecture.md](/distribution/kit-architecture.md) § Agent Skill Deployment. The same run also writes the user-level `fab-operator` pointer skill — `~/.agents/skills/fab-operator/SKILL.md` always, `~/.claude/skills/fab-operator/SKILL.md` under the same `claude` gate |
| `.envrc` entries | `fab-kit sync` | Line-ensuring merge from `{kit-dir}/scaffold/fragment-.envrc` |
| `.gitignore` entries | `fab-kit sync` | Line-ensuring merge from `{kit-dir}/scaffold/fragment-.gitignore` — non-directory lines only (`.fab-*`, `!fab/.fab-version`, `.status.yaml.lock`, `/.claude/settings.local.json`); deployed skills are ignored by the per-target generated manifest |
| Hook registration | *(none)* | `fab-kit sync` registers no Claude Code hook; its Claude-gated scaffold merges permissions into `.claude/settings.local.json` — there is no `fab hook` command family (ioku). Agent-state is read from run-kit's `@rk_pane_agent_state` convention; artifact bookkeeping is pull-based via `fab status refresh` (y022). Cleanup of any lingering hook entries in an existing project is the migrations' job — `2.13.6-to-2.14.0` (the checkout it runs in) and `2.15.7-to-2.15.8` (every worktree, main checkout included — see [migrations.md](/distribution/migrations.md) § `2.15.7-to-2.15.8`) |
...
**`.gitignore` dedup is gitignore-aware (mqiq).** The "already present?" check in `lineEnsureMerge` goes beyond literal string equality for a *directory-style* fragment entry merged into a `.gitignore` destination. The gitignore-aware path is **double-gated**: the destination basename must be `.gitignore` **and** the fragment entry must be a directory token (`gitignoreIsDirectoryToken` — anchored with a leading `/`, or in trailing-slash directory form, and carrying no `*` glob). Two helpers then add gitignore semantics: (1) **variant coverage** — a directory-style entry like `/.claude` counts as already present when any existing line normalizes to the same directory token, across the set `{/.claude, /.claude/, /.claude/*, .claude, .claude/, .claude/*}` (leading slash optional, trailing `/` or `/*` stripped); a *deeper* path such as `/.claude/commands/` does **not** reduce to the token and so does not count as covering; and (2) a **negation hard-stop** — if any `!.../.claude/...` line is present, the broader ignore is never appended (regardless of a preceding `/.claude/*` exclusion), so a user's re-inclusion block survives every sync. Everything else keeps strict literal equality: non-`.gitignore` destinations (notably `.envrc`, Guardrail A), **and** every non-directory pattern the fragment ships (`.fab-*`, `.status.yaml.lock`, `!fab/.fab-version` (8ken), and `/.claude/settings.local.json` (jjg0) — anchored but non-directory, so a pre-existing unanchored `settings.local.json` line does not count as covering it). The non-directory scoping (Guardrail C) is what stops an anchored `/.status.yaml.lock` (root-only) from being mistaken as covering the unanchored, at-any-depth fragment `.status.yaml.lock`, and stops a `!/.status.yaml.lock` negation from hard-stopping it — either would suppress the broader ignore and let nested `fab/changes/**/.status.yaml.lock` files be committed. Since 2.23.0 the shipped fragment carries **no directory token** (jjg0 — fab's deployed skill copies are ignored by the per-target generated `.gitignore` manifest instead, see [kit-architecture.md](/distribution/kit-architecture.md) § Agent Skill Deployment), so the directory-token dedup class currently has no shipped consumer; the code and its tests stay for the next directory line.
