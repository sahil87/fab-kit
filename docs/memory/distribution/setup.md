...
As of 260708-j0qm, `/fab-setup config` **create mode** no longer reads a scaffold `config.yaml` and substitutes `{PROJECT_NAME}`/`{SOURCE_PATHS}`/`{TEST_PATHS}` placeholders — the scaffold `config.yaml` was deleted. Instead it **shells out to `fab config init --project`** with the detected identity seed (`--name`, `--description`, `--source-path`, `--test-path`), which generates the file from the registry: the A-class identity fields live, the managed fence below (see [configuration.md](/_shared/configuration.md) § `fab config init --project`). Notes:
...
- **No `fab_version` step.** The former "Config Create Mode preserves `fab_version`" carry-forward is gone — `fab_version` left `config.yaml` for `fab/.fab-version` (stamped by `fab init`, j0qm), so create-mode neither preserves nor stamps it.
