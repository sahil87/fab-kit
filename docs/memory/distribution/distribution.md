...
The standalone `wt` and `idea` formulas in `sahil87/homebrew-tap` SHALL declare `link_overwrite "bin/wt"` and `link_overwrite "bin/idea"` respectively. This allows them to take ownership of pre-existing symlinks (e.g., from a `fab-kit 1.6.2` install, which bundled `wt`/`idea` directly) silently during `brew upgrade fab-kit`. The directives are also carried in the templates of `sahil87/wt` and `sahil87/idea`, so subsequent regenerations preserve them.
...
`fab update` (fab-kit self-update via Homebrew) exits non-zero when fab-kit was not installed via Homebrew: `Update()` returns the sentinel `var ErrNotBrewInstalled` (`errors.Is`-matchable) instead of the former nil, while preserving the user guidance prints ("Update manually, or reinstall with: brew install sahil87/tap/fab-kit"). A nil return on that path would exit 0 having updated nothing, and `versionGuard` would treat the nil as "updated" — silently defeating the guard for go-install/manual/CI installs; hence the sentinel. Brew subprocess failures (`brew update`, `brew upgrade`, bounded at 30s/120s) also exit non-zero, as before. The `dev` version sentinel does not shelter local builds — the justfile injects real semver via `-X main.version`.
...
- **Decouple wt and idea via `depends_on`, not Go module pin or CI-time external builds (260506-4rtx)**: After `wt` and `idea` were extracted into `github.com/sahil87/wt` and `github.com/sahil87/idea` with their own release pipelines, fab-kit's vendored `src/go/wt/` and `src/go/idea/` were removed and replaced with Homebrew dependency declarations. Each repo versions and releases independently; fab-kit's CI does not cross-compile `wt`/`idea`. Rejected: vendor via Go module dep `require github.com/sahil87/wt` (still ties fab-kit's release to a `wt` version pin and produces a duplicate `wt` binary built from fab-kit's CI); bundle binaries in fab-kit's brew tarball but build them from external repos at CI time (coupling moves from source to CI; fab-kit releases are blocked when external CI is broken); keep vendored sources, accept drift (defeats the purpose of the extraction).
