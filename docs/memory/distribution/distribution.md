...
4. Stamp `fab/.fab-version` to `{latest}` (j0qm — the plain-text sibling; `fab_version` no longer lives in `config.yaml`)
...
5. Stamp `fab/.fab-version` (j0qm — was `fab_version` in config.yaml) only AFTER the sync succeeds
...
`fab upgrade-repo` MUST NOT modify project content. Its write surface (as of j0qm): `fab/.fab-version` (version stamp, only after a successful sync), `fab/project/config.yaml` (rewritten ONLY by the post-sync `fab config upgrade` auto-run — the single writer, which preserves live fields verbatim), `fab/.kit-migration-version` (silent self-stamp only when no migrations apply), and the sync-managed workspace files (agent skill deployments, scaffolding). Preserved: `fab/project/constitution.md`, `docs/memory/`, `docs/specs/`, `fab/changes/`, `.fab-status.yaml`.

**The version stamp no longer touches `config.yaml` (j0qm).** `fab_version` moved out of `config.yaml` into the plain-text sibling `fab/.fab-version`, so both `fab init` and `fab upgrade-repo` stamp the version via `stampFabVersion` (a `stampMigrationVersion`-shaped plain-text write) rather than editing `config.yaml`. The yogn `setFabVersion` line-splice stopgap — a targeted `fab_version:` value-token splice introduced to stop the whole-file marshal from stripping comments on every upgrade — is **deleted**; its comment-preservation job is subsumed by the general comment-preserving writer `fab config upgrade`, now `config.yaml`'s only writer (single-writer invariant). Any config.yaml comment/order preservation is now `fab config upgrade`'s A-field-verbatim contract, not a bespoke splice. See [kit-architecture.md](/distribution/kit-architecture.md) § Version Tracking and [configuration.md](/_shared/configuration.md) § `fab config upgrade`.
...
fab-kit is part of the **sahil87 toolkit** and MUST conform to the toolkit's published standards, per the Constitution's `### Toolkit Standards` article (v1.4.0, added in `260717-y8it`). The standards are enumerated at runtime by `shll standards` — the constitution deliberately names none of them, so the binding set is whatever `shll standards` returns for the installed shll version (canonical source: the `sahil87/shll` repo's `docs/site/standards/` tree, rendered on `https://shll.ai`). Before changing the CLI surface, help output, `README.md`, or `docs/site/`, the change MUST be checked against the standards governing that surface.

A full conformance audit against the four runtime-enumerated standards was performed at **shll v0.0.23** (change `260717-ptwh`). The audit report — one section per standard, each PASS or listing gaps dispositioned as fixed-here or deferred — lives at `fab/changes/260717-ptwh-toolkit-standards-conformance/conformance-report.md` (it became that change's PR body). Current posture:
