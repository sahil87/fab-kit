...
- **`principles`** (foundation, ten CLI principles) — **6 PASS + 4 deferred.** P1/P5/P6/P7/P8/P10 pass (non-interactive-by-default, visible mutation boundaries, stateless/retry-safe, compose-don't-reinvent, graceful degradation, agent-discoverable docs). Four gaps are restructuring-sized (multi-command / cross-command) and deferred to `fab/backlog.md`: P2 (`--json` on the `fab status` query surface) → `[jx4w]`; P3 (cobra `Example:` blocks) → `[b91h]`; P4 (route usage errors to exit `2`, reconciled with existing domain-specific exit-2 uses) → `[swon]`; P9 (`--quiet` on `batch archive`/`batch switch`) → `[o5f9]`.
