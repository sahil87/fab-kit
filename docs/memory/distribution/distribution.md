...
6. **Generate `fab/project/config.yaml` from the registry** (j0qm) by shelling out to the pinned fab-go `fab config init --project` with a mechanically-detected identity seed (repo-folder name, `src/`, on-disk test markers) — a pinned fab-go that predates the subcommand fails closed with a non-zero error instructing a fab-go upgrade, and no config.yaml is written in that state (see [kit-architecture.md](/distribution/kit-architecture.md) § fab-kit `Init` config generation). Runs before the scaffold walk, preserving the new-vs-existing classification
...
- Sync fails during deployment (read-only/full filesystem under `.claude/skills/` when the `claude` gate fires) — each failed skill reported on stderr, tally shows `failed N`, `fab sync` exits non-zero (`1`)
...
Three version sources can disagree, and a version-string match is not proof against skew: the running binary's version, the kit cache's `<kit-dir>/VERSION`, and the project pin `fab/.fab-version`. The `fab setup check` doctor (see [setup.md](/distribution/setup.md) § Setup-State Doctor) reports the triplet and warns on any mismatch (`v`-prefixed tags normalize to bare semver for comparison; a `dev` binary version is reported but never compared; an empty pin outside a fab repo and an unresolvable kit-cache dir each drop their side of the comparison, the latter with an informational finding).

It also runs the **override-masking check** — the behavioral half of skew detection: for each capability-bearing override (`interactive_command` / `headless_command` / `native`, with the deprecated `session_command`/`dispatch_command` spellings aliased) under `providers.<name>` in the system or project config tier, it introspects the running binary's **own embedded defaults** (`internal/agent.BuiltinProvider` over the `go:embed`'d `defaults.yaml` — never the kit cache, so no reference artifact is needed) and flags any override on a provider+key the embedded defaults do not define as **load-bearing against the installed binary** — unsetting it silently changes behavior. This is the bottle-skew incident shape: a Homebrew bottle built without a source-tree defaults change, where every version indicator agrees while behavior diverges, and the standing rule is "verify the binary's *behavior*, not the version string". Providers absent from the embedded table entirely are skipped — a user-defined provider is a definition, not a mask. All skew findings are warnings, never failures: a triplet mismatch or a load-bearing override is a skew signal, not proof of breakage.
...
The `wt` binary provides a `shell-init` subcommand that outputs a shell wrapper function to stdout, suitable for `eval` in the user's shell profile. This follows the direnv/rbenv/mise pattern (mirroring `fab shell-init`).
...
- **`help-dump`** (binary scope) — **PASS after fix.** The envelope is exactly `{tool, version, schema_version, root}` with `schema_version` = `1` and carries no `captured_at` (the standard forbids the tool emitting it — the capture timestamp is owned and stamped by shll.ai's puller). See [kit-architecture.md](/distribution/kit-architecture.md) § fab-go Subcommands for the emitter, and § shll.ai Public Docs Site above for the pull surface.
- **`readme-extraction`** (repo scope) — **PASS after fix.** Every README slice-region link that leaves the published set (README slice + `docs/site/**`) is an absolute `https://github.com/sahil87/fab-kit/blob/main/<path>` URL; README→`docs/site/` links stay repo-relative (the sanctioned auto-rewritten form). The command-reference URL is `https://shll.ai/tools/fab-kit/commands/` (the standard's `/<tool>/commands/` template renders under `tools/fab-kit/` for this repo). See § Producer-Side README Conformance Obligation above for the standing structural obligations.
- **`principles`** (foundation, ten CLI principles) — **6 PASS + 4 PASS after fix (all audit gaps closed).** P1/P5/P6/P7/P8/P10 pass (non-interactive-by-default, visible mutation boundaries, stateless/retry-safe, compose-don't-reinvent, graceful degradation, agent-discoverable docs). The four audit-time gaps were restructuring-sized (multi-command / cross-command), deferred to `fab/backlog.md`, and have since each shipped as its own change:
  ...
  - **P4 (route usage errors to exit `2`) — PASS after fix (swon)**: binary-wide `0` success / `1` operational / `2` usage-error convention via the testable `run(args, errW) int` + `markRunReached` phase classifier, coexisting unchanged with the in-handler pane 2/3 and docs-index 0/1/2 schemes — see [kit-architecture.md](/distribution/kit-architecture.md) § Binary-Wide Exit-Code Convention.
...
- **`skill`** (binary + repo scope) — **PASS, adopted (fskl).** Deferred at the audit (ptwh) — a tool without a `skill` subcommand was "not yet in violation" per the standard's phased per-repo Adoption section — then adopted per its backlog entry against the same shll v0.0.23 contract. fab ships a **visible `fab skill` subcommand** (`src/go/fab/cmd/fab/skill.go`, registered in `newRootCmd()`) that prints the canonical bundle `docs/site/skill.md` (124 lines, static-only, ≤150-line budget) as **raw markdown to stdout, byte-identical**, with **stderr empty and exit 0**. Its single accepted positional is the standard's **reserved topic `topics`** (psgm): `fab skill topics` enumerates content-topic names one per line, raw to stdout, stderr empty, exit 0 — fab ships zero topic pages, so the output is empty (zero bytes), the standard's scriptable "zero topics" answer; the verb is a machine affordance implemented as a custom `Args` validator (deliberately not a cobra child command, keeping `fab skill --help` and the `help-dump` tree free of it). Any other positional is a usage error (exit `2` via the binary-wide `run()`/`markRunReached` classifier — no new exit code). The bundle is an agent-first usage briefing (when-to-use / capabilities map / composition with `wt`/`rk`/`gh`/the `/fab-*` skills / stdout+exit-code contracts / gotchas) and explicitly disambiguates `fab skill` (this toolkit-standard bundle) from fab's own kit-skills (the `/fab-*` markdown deployed to `.agents/skills/` unconditionally and `.claude/skills/` when `claude` is available by `fab sync`). It renders at `shll.ai/tools/fab-kit/skill` for free since `docs/site/**` is already the pulled+rendered site surface. Embedding is fab's **first `go:embed`** usage, via the sync + drift-guard pattern the `shll standards` mechanism established (committed copy `src/go/fab/cmd/fab/skill.md` + `scripts/sync-skill.sh` `//go:generate` + drift-guard test `TestSkillEmbedMatchesCanonical`) — see [kit-architecture.md](/distribution/kit-architecture.md) § fab-go Subcommands. The deployed `_cli-fab.md` § fab skill carries only the consumer contract (raw markdown, byte-stable per release, stderr empty, exit 0, the reserved `topics` topic); this build provenance is documented here, not in the deployed skill (d5tk). The consumer-side version-skew pairing remains tracked in the backlog (clix); the cross-repo aggregation effort (shll `[agst]`) is unblocked on the fab row.

The `update` and `version` standards were re-audited clause-by-clause against the live `shll standards` output on **2026-07-20 (1e4m)** — all clauses of both are conformant:

- **`update`** (binary scope) — **PASS after fix.** The brew-handling safety MUSTs (no `SIGKILL` to a package-manager subprocess mid-transaction; no short hard timeout on `brew upgrade`) are met by running brew unbounded — no timeout wrapper exists (see § `fab update` Exit Semantics and the Design Decisions below). The non-brew install degrades with a clear message and exits 0 (SHOULD "degrade instead of erroring" + MUST "exit non-zero only on genuine failure"); the `--skip-brew-update` flag, brew-only self-update gate, `v{semver}` tags, and one-name-four-places requirements already passed unchanged.
...
- **Override-masking via embedded-defaults introspection**: Bottle skew is detected by comparing user-tier capability overrides against the running binary's OWN embedded defaults — an override on a provider+key the embedded defaults lack is load-bearing (see § Version Skew Detection above). **Why**: needs no reference artifact and reproduces the incident shape exactly; env-var overrides cannot test it (the env layer is the top cascade tier — it can only shadow lower tiers, never reveal or remove them, and empty leaves fall through). **Rejected**: artifact-based defaults comparison (shipping a comparable `defaults.yaml` counterpart into the kit tarball — the kit cache carries no counterpart today; deferred); env-var-based detection (structurally incapable). *Introduced by*: 260811-pgbq-setup-check-doctor
