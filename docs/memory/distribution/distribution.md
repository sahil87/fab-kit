---
type: memory
description: "How src/kit/ is distributed — Homebrew formula (fab router + fab-kit direct, wt + idea via depends_on), always-route policy, fab init bootstrap, fab upgrade-repo (offline-first, --latest opt-in), the release workflow, hardened auto-download (timeouts, flock, atomic rename, digest verify), fail-loud exit contracts (exit 3 = not-a-fab-managed-repo), the update brew-safety contract (unbounded brew, non-brew exits 0), wt shell-init, the shll.ai README pull surface, and toolkit-standards conformance."
---
# Distribution
