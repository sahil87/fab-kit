---
type: memory
description: "How src/kit/ is distributed — Homebrew formula (fab router + fab-kit direct, wt + idea via depends_on), the fab router's always-route policy, fab init bootstrap, fab upgrade-repo (offline-first default, --latest opt-in), the release workflow, hardened auto-download (timeouts, version-keyed flock, atomic rename, digest verification), fail-loud lifecycle exit contracts (exit 3 = not-a-fab-managed-repo), wt shell-init, the shll.ai README-slice pull surface, and toolkit-standards conformance."
---
# Distribution
