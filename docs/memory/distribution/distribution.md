---
type: memory
description: "How src/kit/ is distributed — Homebrew formula (fab router + fab-kit direct; wt/idea are standalone sibling formulas, no depends_on), always-route policy, fab init bootstrap, fab upgrade-repo (offline-first, --latest opt-in), the release workflow, hardened auto-download (timeouts, flock, atomic rename, digest verify), fail-loud exit contracts (exit 3 = not-a-fab-managed-repo), the update brew-safety contract, wt shell-init, the shll.ai README pull surface, toolkit-standards conformance."
---
# Distribution
