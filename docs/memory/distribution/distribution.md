...
- **`principles`** (foundation, ten CLI principles) — **6 PASS + 4 deferred at audit time.** P1/P5/P6/P7/P8/P10 pass (non-interactive-by-default, visible mutation boundaries, stateless/retry-safe, compose-don't-reinvent, graceful degradation, agent-discoverable docs). Four gaps were restructuring-sized (multi-command / cross-command) and deferred to `fab/backlog.md`: P2 (`--json` on the `fab status` query surface) → `[jx4w]`; P3 (cobra `Example:` blocks) → `[b91h]`; P4 (route usage errors to exit `2`, reconciled with existing domain-specific exit-2 uses) → `[swon]`; P9 (`--quiet` on `batch archive`/`batch switch`) → `[o5f9]`. **P9 is now resolved (o5f9)**: both `fab batch archive` and `fab batch switch` gained a `--quiet`/`-q` flag suppressing per-change progress while retaining the data + all stderr (archive's `Archived N, …` footer; switch stdout-silent on success) — see [kit-architecture.md](/distribution/kit-architecture.md) § Batch Scripts and [pipeline/change-lifecycle.md](/pipeline/change-lifecycle.md).
- **`skill`** (binary + repo scope) — **deferred, not yet adopted.** `fab skill` and `docs/site/skill.md` do not exist. Per the standard's own phased per-repo Adoption section, a tool without a `skill` subcommand is not yet in violation, so this is not an in-scope fix; the consumer-side pairing is tracked at backlog `[clix]`.
