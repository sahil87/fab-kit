...
**`/loop` lifecycle (adaptive heartbeat — mmmt)**: Start when first change enrolled (no loop running). The cadence is **not fixed** — it adapts to whether any monitored agent is sitting on an interactive menu (input-waiting per §5 Question Detection): **normal `3m`** when none is, **tightened `90s`** the moment a tick detects any monitored agent menu-waiting, relaxing back to `3m` when none is. Adapting cadence means **re-establishing the single loop at the new interval** (e.g. restart `/loop 90s "operator tick"`) — never a second concurrent loop, preserving the one-loop invariant (`_cli-external.md` § /loop, "one loop at a time"). The tick's loop-lifecycle step (Tick Behavior step 7) performs the adapt. Autopilot's own cadence (default `2m`) composes unchanged — when a queue is driving, autopilot's cadence governs and the menu-tightening applies to the monitoring loop's `3m`/`90s` band only. The §4 Idle Message reflects the currently-active interval via its existing `fab operator time --interval {interval}`, so a tightened cadence shows the nearer next-tick time. Stop when monitored set empty (and no autopilot, no watches).
...
| Menu-detected heartbeat (mmmt) | 90s | "tighten to {N}s when an agent is on a menu" |
