---
description: "Operator coordination skill (`/fab-operator`, superseding the historical operator4) — multi-repo / multi-session coordination on one tmux server via the `(session, repo, pane)` addressing tuple, the server-keyed operator state file (term defined once in skill §4), multi-agent monitoring, auto-answer model with strategic escalation, repo-targeted spawning, two-tier dependency resolution, repo-spanning autopilot, and tmux tab-naming. Historical operator4 context plus the operator design-decision lineage."
---
# Operator
