---
type: memory
description: "`fab pane {map,capture,process,window-name,open,ready,deliver,kill}` reference: map delegates enumeration to `rk mux panes` (fail-open to internal tmux), enriches with change/stage, and declares the row identity-key contract (`pane`+`server`/`window_id` identity; `session`/`window_index` display-only); `open` spawn + `ready`/`deliver` primitives, `--json`, `--server`/`-L`, exit codes 2/3. capture/process/kill are dispatch-internal; skill-facing use rides the `rk mux` twins."
---
# Pane Commands
