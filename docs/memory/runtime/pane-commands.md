...
**JSON output**: `pane`, `lines`, `content`, `worktree`, `change`, `stage`, `agent_state`, `agent_idle_duration` — `agent_state` ∈ `{active, waiting, idle, null}` (same structured-source semantics as `map --json`; `waiting` added in ioku), `agent_idle_duration` only for `idle`. The `worktree`/`change`/`stage` fields are `null` when the pane is not in a fab worktree or has no active change.
