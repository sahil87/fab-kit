---
type: memory
description: "`fab pane {map,capture,process,window-name,open,ready,deliver,kill}` reference: map delegates enumeration to `rk mux panes` when rk is present (fail-open to internal tmux) and enriches with change/stage; `open` spawn + `ready`/`deliver` primitives, `--json`, `--server`/`-L`, pane-family exit codes (2 = pane missing / 3 = other tmux failure), shared `internal/pane` helpers. capture/process/kill are dispatch-internal (rk-less pane arm); skill-facing use rides the `rk mux` twins."
---
# Pane Commands
