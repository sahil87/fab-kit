...
**Consequence — the registry row is the STATIC default; the read model composes a live one**: `configref.Fields()`'s `agent.profiles` default is built from `DefaultProfile`, which is knob-blind by construction. So `fab config show` — every form, `--origin` or not — recomposes that one row through `configref.DefaultsMapFor`, resolving each role against the **live** config (`agent.ResolveRole`) with the user's own `agent.profiles`/`agent.tiers` entries stripped first — the defaults tier reports the built-in a user override *shadows*, never an echo of that override. The drill-down and `fab resolve-agent` therefore agree on which provider a role would dispatch to, both reading the knob. See [_shared/configuration.md](/_shared/configuration.md) § Six-Verb Surface.
*Introduced by*: 260806-j9nh-agent-profiles-session-workers
