...
**fab-kit ships FOUR built-in providers — `claude`, `codex`, `agy`, and `kimi`** (`defaultProviders` in `internal/agent`, parsed from the embedded `defaults.yaml` — § The built-in defaults are an embedded `defaults.yaml`; each command string is additionally exposed as a package var, `DefaultInteractiveCommand` / `DefaultCodex*` / `DefaultAgy*` / `DefaultKimiHeadlessCommand`, that `internal/configref` interpolates, so no literal is duplicated):
...
| `agy` | `agy --dangerously-skip-permissions --model {model} -i {prompt}` (§ agy's pane capability rides `{prompt}`) | absent | `sh -c 'agy --dangerously-skip-permissions --print-timeout 120m --model {model} -p "$(cat)"'` | sparse — `default`, `fast` (model only) |
| `kimi` | **none** → dispatch-only (§ kimi is the sole dispatch-only built-in) | absent | `sh -c 'kimi -m {model} -p "$(cat)"'` | **none** — deliberate |
...
**agy's pane capability rides `{prompt}`.** The agy built-in ships **both** command fields, so `fab agent --provider agy` and `fab operator` open a session and `dispatch.mode: pane` inside tmux selects the pane rung — with no user config. One field serves both kinds of launch because its trailing `-i {prompt}` is a placeholder pair: `-i`/`--prompt-interactive` is a **value-taking** flag (a bare `-i` exits 2 with `flag needs an argument`; a bare positional prompt is silently discarded), so every prompt-carrying launch substitutes its shell-quoted prompt there while the one promptless launch (bare `fab agent`) drops the pair and opens a plain session. The two delivery shapes are one shared grammar — see [configuration.md](/_shared/configuration.md) § `providers`.

**Trust wall — documented, not seeded.** agy gates a **fresh workspace** behind an interactive trust dialog even under `--dangerously-skip-permissions`, so a pane-dispatched agy worker in a not-yet-trusted worktree parks there **once per worktree** until a human answers it (the pipeline's wait-timeout peek classifies that as "waiting on genuine human input" and escalates without killing — [dispatch.md](/runtime/dispatch.md)). The trust store is `~/.gemini/antigravity-cli/settings.json` `trustedWorkspaces`: **EXACT-match** absolute paths, so a trusted parent does not cover a child directory, and there is no CLI verb to add an entry. fab does not write it. The dialog **defers** the `-i` prompt rather than eating it — answering it opens the session with the prompt already delivered as its first user message.

**kimi is the sole dispatch-only built-in.** kimi deliberately carries **no `interactive_command`**, which is the mirror image of claude carrying no `headless_command`. A provider with an `interactive_command` becomes eligible for **pane-mode dispatch**, which delivers a one-line pointer to the stage prompt file through that command — substituted at a `{prompt}` placeholder, or appended as a **positional argument** when the command declares none — and kimi can receive a prompt **neither way**: it parses a bare positional as a **subcommand** and exits non-zero, and it has no interactive-initial-prompt flag for a `{prompt}` placeholder to target. With no `interactive_command`, automatic selection skips the pane rung and descends to headless (`descended: pane unavailable: no interactive_command` — [dispatch.md](/runtime/dispatch.md)) and an explicit `--pane` hard-errors with the `providers.<name>.interactive_command` hint. A user who adds `providers.kimi.interactive_command` in their own config opts into pane workers that never see their pointer prompt.
...
#### Scenario: a dispatch-only built-in inside tmux descends to headless

- **GIVEN** `agent.workers: kimi` and a reachable tmux server
...
- **AND** an explicit `--pane` instead exits non-zero naming `providers.kimi.interactive_command`, persisting no dispatch record

#### Scenario: one agy grammar serves both kinds of launch

- **GIVEN** the built-in agy provider and no `providers:` config
- **WHEN** `fab agent --provider agy --model gemini-3.1-pro-high --print` runs
- **THEN** it prints `agy --dangerously-skip-permissions --model gemini-3.1-pro-high` — the `-i {prompt}` pair dropped, no placeholder surviving
- **AND** a pane dispatch of the same provider composes `agy … --model <id> -i '<shell-quoted pointer>'`, so the worker's first user message is the pointer
...
- `dispatch=<command>` — absent exactly when the shared mode selector lands on native; present with the substituted `interactive_command` for pane or `headless_command` for headless. Selection starts at `dispatch.mode`, descends pane → native → headless without ascending, and fails only when no reachable capability exists. The command always embeds the full model ID even under `--alias`. A `{prompt}` placeholder is left **verbatim** on this line: the pointer does not exist at resolve time (`fab dispatch start` writes the prompt file and derives it), and dispatch sites never execute the value.
...
**`fab agent` is the one promptless launch.** It is the only seam that composes an `interactive_command` with **no** initial content, so after `WithProfile` it resolves `{prompt}` to empty and the same token-drop rule removes the placeholder's token plus a preceding `-`-flag — agy's `-i {prompt}` pair vanishes and a plain session opens. Every other seam that composes an `interactive_command` carries a prompt (`fab operator`, `fab batch new`/`switch`, pane dispatch) and delivers it through the shared two-shape rule instead ([configuration.md](/_shared/configuration.md) § `providers`). The load-bearing axis is **promptless vs prompt-carrying**, not session vs pane: `fab operator` and `fab batch` are session launches that do carry a prompt.
...
### kimi Is the Sole Dispatch-Only Built-in
**Decision**: The kimi built-in carries a `headless_command` and deliberately no `interactive_command`, the mirror image of claude carrying no `headless_command`.
**Why**: An `interactive_command` is what makes a provider **pane-eligible**, and pane dispatch delivers the worker a one-line pointer to the prompt file — substituted at a `{prompt}` placeholder, or appended as a positional argument when the command declares none. kimi can act on neither shape: it parses a bare positional as a subcommand and exits non-zero, and it has no interactive-initial-prompt flag for a placeholder to target (that is precisely the difference from agy, whose value-taking `-i` the placeholder does target). Shipping an `interactive_command` would therefore make a pane-preferring dispatch inside tmux select a mode that cannot work and orphan every stage. Withholding it makes automatic resolution skip the pane rung and descend to headless (`descended: pane unavailable: no interactive_command`) and makes an explicit `--pane` fail loudly with the config-key hint.
...
**Known consequence**: `fab agent --provider kimi` and `fab operator` on kimi need an `interactive_command` the user adds themselves — which re-enables pane eligibility, so those stages must then be dispatched `--headless` or pointed at another provider.
*Introduced by*: 260808-rpsr-remove-gemini-add-agy-kimi; *Updated by*: 260809-agik-agy-interactive-command

### agy's `-i {prompt}` Pair Is Load-Bearing, Not Cosmetic
**Decision**: agy's built-in `interactive_command` ends on the `-i {prompt}` pair — `agy --dangerously-skip-permissions --model {model} -i {prompt}` — and no `{effort}` placeholder appears anywhere in it.
**Why**: `-i`/`--prompt-interactive` is a **value-taking** flag (probed live on agy 1.1.11: a bare `-i` exits 2 with `flag needs an argument`), and a bare positional prompt is silently discarded, so neither a plain command nor a static trailing `-i` serves both kinds of launch. The placeholder does: a prompt-carrying launch substitutes its shell-quoted prompt as the flag's value, so the prompt lands as the session's first user message, and the promptless launch substitutes empty so the empty-value token-drop removes the pair, leaving a plain session. One field, both seams. Effort has no placeholder because agy's model IDs embed the reasoning level as a suffix (`gemini-3.1-pro-high`), which a `--effort` flag would fight.
**Rejected**: A grammar without `-i` (nominally pane-capable while orphaning every tmux-dispatched stage — the failure the pane rung exists to avoid); a trailing bare `-i` (hard-errors every promptless launch); two separate command fields for the two launch kinds (a third capability field for one CLI's flag shape).
*Introduced by*: 260809-agik-agy-interactive-command

### `fab resolve-agent`'s `dispatch=` Line Leaves `{prompt}` Unsubstituted
**Decision**: A `dispatch=` line for the pane rung prints the profile-resolved command with `{prompt}` still visible.
**Why**: The pointer does not exist at resolve time — `fab dispatch start` writes the stage prompt file and derives the pointer from it — and dispatch sites branch on the line's presence without ever executing its value. A visible `{prompt}` honestly marks where the pointer goes.
**Rejected**: Token-dropping it at resolve time (prints a session-shaped command that is not what `fab dispatch` runs — actively misleading); substituting a placeholder pointer path (invents a path no worker will read).
*Introduced by*: 260809-agik-agy-interactive-command

### agy's Trust Wall Is Documented, Not Seeded
**Decision**: agy's once-per-worktree interactive trust dialog is accepted and caveated wherever agy's pane capability is described; fab never writes `~/.gemini/antigravity-cli/settings.json`.
**Why**: Seeding a third-party CLI's settings file at dispatch time is delivery choreography, not provider data, and the pipeline already handles the parked worker correctly — its wait-timeout peek classifies a trust dialog as "waiting on genuine human input" and escalates without killing. The wall is also bounded: one dialog per worktree, and it **defers** rather than eats the `-i` prompt, so the prompt is still the session's first user message after a human answers.
**Rejected**: Read-modify-write of agy's trust store at dispatch time (belongs with the pane-delivery redesign, where the readiness gate is being built anyway); withholding agy's `interactive_command` because of the wall (a human-answerable one-time dialog is not a reason to withhold a working capability).
*Introduced by*: 260809-agik-agy-interactive-command
...
- **The operator launcher** (`fab operator`) resolves the **operator** role in-process and composes its session command from that role's provider `interactive_command` + profile, then delivers `/fab-operator` through the shared two-shape rule. See [operator.md](/runtime/operator.md).
- **Batch worker spawns** (`fab batch new`/`switch`) compose from the **default**-role provider `interactive_command` + profile — so workers spawn WITH a profile — and deliver their `/fab-new …` / `/fab-switch …` prompt through the same rule. The operator's own repo-targeted worker spawns hand-assemble a `tmux new-window` from `fab agent --print` output instead, which is why they are **unsupported for a `{prompt}`-carrying session provider** ([agent-primitives.md](/runtime/agent-primitives.md) § Spawn composition). See [operator.md](/runtime/operator.md) and [distribution/kit-architecture.md](/distribution/kit-architecture.md).
