...
- `fab batch new --workers <provider>`, `fab batch switch --workers <provider>`, and `fab operator --workers <provider>` (on its rk-absent fallback path) prefix a shell-quoted assignment (`FAB_AGENT_WORKERS='<provider>'`) to a newly launched tmux shell command. Embedded quotes and shell metacharacters remain data. When `fab operator` delegates to a capable `rk operator` (see [operator.md](/runtime/operator.md) § Launcher), the value instead rides argv (`rk operator --workers <v>`) and rk validates it (letters/digits/`_`/`-`).
...
| `codex` | `codex --dangerously-bypass-approvals-and-sandbox -m {model} -c model_reasoning_effort={effort}` | absent | `codex exec --dangerously-bypass-approvals-and-sandbox -m {model} -c model_reasoning_effort={effort}` | all six roles — dense; `doing` and `review` on different models |
...
**claude, codex and agy ship per-role fills**, so naming one of them on a depth knob is a complete configuration: `agent.workers: codex` resolves a model suited to each role, and **role differentiation survives the provider swap** (apply runs on codex's top catalog model, review on a different model at `xhigh`, ship on the cheaper `fast` model at `low`). claude's and codex's maps are **dense** — every role pins its own model and effort, so no shipped row leans on the cross-role fallback. agy's map is **sparse** — a role absent from it resolves agy's `default` entry. The merge is per **field**, so a user override carrying `effort` only still takes its model from the provider's `default`; no shipped codex row relies on that.
...
**A model bump edits `defaults.yaml`** and runs the tests. The file carries the bump procedure inline and names the drift guards that will name *themselves* if a mirror falls behind — `TestDefaultsFileIsWellFormed` (the file itself), `TestDefaultRoleProfilesArePinned` (the deliberate-change pin for claude) and `TestNonClaudeProviderFillsArePinned` (the same pin for codex/agy, plus kimi's deliberate empty map), `TestDefaultsFileProviders` (codex's map is dense — every role names a model and no row ships `ultra`; agy's stays model-only; kimi's stays empty), `TestDocTablesMatchAgentMaps` (`docs/specs/stage-models.md`'s four-column claude table — the one doc mirror that spells a shipped ID), and `TestConfigReferenceDocumentsProviderFill` (the fill lines in the rendered `fab config explain`, derived from `ResolveProvider`). The spec's inline `providers:` sample is shape-only (IDs elided) and deliberately unguarded. Every other consumer derives from `DefaultProfile()` or `ResolveProvider`, so a bump touches nothing else.
...
- **THEN** `provider=codex` with codex's own `doing` fill — its shipped `doing` model and effort — and `dispatch=codex exec --dangerously-bypass-approvals-and-sandbox …` with both placeholders substituted
...
- `-o, --output yaml` emits the full resolution as structured YAML. `selector`, `kind`, `role`, `provider`, `model`, `effort`, and `command` appear first in that order, followed by `model_alias`, raw `template`, `fill_mode`, per-field `source`, optional labelled `dispatch`, and a trailing always-present `skill_prefix`. `model_alias` is the Agent-tool short alias for Claude IDs and empty otherwise; `source` names the actual provider/model/effort precedence rung (empty means inherit); `dispatch` is absent exactly for native and otherwise contains `rung: pane|headless` plus its substituted `command`; `skill_prefix` is the receiving harness's explicit skill-invocation prefix (`$` for `codex`, `/` for every other provider — `agent.SkillPrefix` owns the rule, see [agent-primitives.md](/runtime/agent-primitives.md) § Skill Prompt Rendering) and is the last key whether or not `dispatch` is present; the deprecated `fab resolve-agent` line protocol never carries it. YAML alone derives dispatch. It implies print, accepts exactly `yaml` (anything else is a usage error), and is mutually exclusive with `--print` and with `-t` — one output sink per invocation.
...
**Decision**: `providers.codex.profiles` ships concrete model slugs for all six roles. `providers.agy.profiles` ships two model-only rows (`default`, `fast`) and no `effort` anywhere. `providers.kimi` ships no `profiles` map at all.
**Why**: Each CLI's own model-addressing vocabulary decides the shape. The codex CLI exposes no alias mechanism (`-m` takes a slug), so concrete IDs are the only option; the shipped ones are read from the installed CLI's own cached model catalog (`~/.codex/models_cache.json`: slug, priority, description, supported reasoning levels) rather than from documentation, and the supported reasoning levels are checked there too. agy's IDs *embed* the reasoning level as a suffix (`gemini-3.1-pro-high`), so effort has no separate field to occupy and a `--effort` flag would fight the suffix. kimi's `-m` takes a **user-config alias** rather than a catalog ID, so there is no value that is correct for every install — the empty model drops the flag and the user's own `default_model` applies. For agy, sparseness buys role differentiation cheaply: rows omitted from its map inherit `default` through the per-field merge.
**Rejected**: Pinning kimi's managed-install alias `k3` (breaks custom-provider installs) or giving it a lone `default` fill (same problem, one row smaller); a separate `--effort` flag on agy alongside its suffixed IDs; codex slugs quoted from docs or memory rather than the installed catalog (placeholders carried in from an intake were absent from it entirely, and the next tier down carried deprecation notices).
*Introduced by*: 260806-ywkx-ship-codex-gemini-fills; *Updated by*: 260808-rpsr-remove-gemini-add-agy-kimi, 260908-wcib-refresh-codex-builtin-fills

### The Codex Fill Map Is Dense by Policy; Author and Critic Run Different Models
**Decision**: `providers.codex.profiles` ships all six roles with both `model` and `effort` set. `default`/`doing` take the catalog's top model at `high`; `review` takes a different model at `xhigh`; `hydrate` the same second model at `high`; `operator`/`fast` the cheap model. `TestDefaultsFileProviders` rejects any codex row without a model and any row shipping `ultra`; `max` is an explicit-override level only.
**Why**: Effort-only rows made a `default` bump silently repoint `doing`/`review` through the per-field merge, and a reader could not see what a role resolved to without knowing that rule. Putting the critic on a different model than the author keeps the reviewer from sharing the author's blind spots (the 2026-08-10 four-provider comparison in `docs/findings/` recorded reviewer strictness tracking the worker model on every arm). Codex's `ultra` level means automatic task delegation, and a worker spawning its own sub-agents breaks the single-worker dispatch contract and the `{stage}-result.yaml` protocol. Per-model codex pricing was not available when the table was set; the tier ladder is inferred from codex's own model descriptions and priority order, not measured. One consequence rides the density: the deprecated flat `providers.codex.model` alias reaches only the `default` role (§ Deprecated spellings) — an upgrade note in `docs/specs/stage-models.md`, not a migration, since nothing on disk restructures.
**Rejected**: Keeping effort-only rows (the inheritance is the defect); the same model for author and critic (shared blind spots); `gpt-5.6-terra` as a default (no evidence in this repo; A/B candidate only); a migration for the flat-alias reach change (no config key changed).
...
- **The operator launcher** (`fab operator`, on its rk-absent fallback path — a capable rk owns the launch and resolves the same role via `fab agent operator --print`; see [operator.md](/runtime/operator.md) § Launcher) resolves the **operator** role in-process and composes its session command from that role's provider `interactive_command` + profile. Its interactive spawn carries the shell fallback (composed command + `; exec "$SHELL"` — see [agent-primitives.md](/runtime/agent-primitives.md) § Spawn composition). See [operator.md](/runtime/operator.md).
