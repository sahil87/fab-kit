...
| `gemini` | `gemini --approval-mode=yolo -m {model}` | absent | `gemini --approval-mode=yolo -m {model}` | sparse — `default`, `fast` (model only) |
...
**Naming a built-in is sufficient — no `providers:` block required.** A depth knob, role override, or invocation flag resolves the provider and then applies the configured dispatch preference to its capabilities. Adding or overriding a command never changes mode policy by itself.
...
The non-claude built-ins carry explicit full-auto grammar on both command forms: codex uses `--dangerously-bypass-approvals-and-sandbox`, and gemini uses `--approval-mode=yolo`. Gemini has no `{effort}` placeholder and no `-p` on its headless command; codex and gemini consume the prompt from stdin. Claude's headless command also consumes stdin through `-p`. `fab config explain` derives all capability values from the embedded defaults.
