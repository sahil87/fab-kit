...
**Pane capability is the default expectation.** Every agent CLI has an interactive mode — that is what a TUI is — so shipping an `interactive_command` is a fab decision about launch grammar, not a discovery about the CLI, and **all four built-ins ship one**. Pane-mode dispatch rides that grammar plus the provider-neutral readiness gate: fab appends nothing to the command and types the stage prompt in afterwards ([dispatch.md](/runtime/dispatch.md) § Prompt delivery), so no initial-prompt flag is involved — kimi has none (its `-p` is the non-interactive form and a bare positional parses as a subcommand) and agy's `-i` goes unused. **Headless** grammar is the half that genuinely varies per CLI — stdin vs a `-p` argument, nested shells, per-form approval flags, timeout defaults — and is what a new provider has to be probed for.

**First-run behavior is a gate input, not a capability.** agy and kimi both wall a fresh workspace behind a trust dialog even under their full-auto flags, and worktree-per-change makes every dispatch a fresh workspace, so each costs the readiness gate one **judgment round** — the pane reads `parked`, one answer clears it, and the answer is remembered per workspace, so it amortizes across every later pane worker in that checkout ([dispatch.md](/runtime/dispatch.md) § `fab dispatch ready`). It needs no code. The mechanism is verified live against kimi 0.34.0 (2026-08-10); agy rides that same gate, with its own end-to-end pane run pending its quota reset (~2026-08-16). agy's trust store is additionally user-seedable (`~/.gemini/antigravity-cli/settings.json` `trustedWorkspaces`, EXACT-match paths) — a user-side optimization fab deliberately does not ship, since the gate is provider-neutral and needs no knowledge of any provider's file format.

kimi's other delivery-relevant quirk is its **input echo**: it draws vertical rules down both sides of its input box, so a hard-wrapped pointer arrives with `││` interleaved between the halves. `deliver`'s echo verification counts occurrences with box-drawing runes dropped alongside whitespace, so the pointer verifies ([dispatch.md](/runtime/dispatch.md) § `fab dispatch deliver`).

An **absent** `interactive_command` is a valid shape — a provider a user defines with headless grammar alone — and it is a user-config one, not a shipped state. Automatic selection then skips the pane rung and descends to headless (`descended: pane unavailable: no interactive_command` — [dispatch.md](/runtime/dispatch.md)) while `fab dispatch open` hard-errors with the `providers.<name>.interactive_command` hint.
...
#### Scenario: `agy` is eligible for a pane worker and a session

- **GIVEN** `agent.workers: agy`, `dispatch.mode: pane`, a reachable tmux server, and no `providers.agy` block
- **WHEN** `fab dispatch open` runs for a stage
- **THEN** the pane opens on `agy --dangerously-skip-permissions --model <agy's fill for that role>`, and the readiness gate answers the first-run trust wall before `deliver` types the pointer
- **AND** `fab agent --provider agy --print` prints `agy --dangerously-skip-permissions` — the provider-addressed path bypasses the fills, so the empty `{model}` drops the `--model` pair

#### Scenario: a provider carrying headless grammar alone descends to headless

- **GIVEN** `agent.workers: mine`, a project-defined `providers.mine` with a `headless_command` and no `interactive_command`, and a reachable tmux server
...
- **AND** `fab dispatch open` instead exits non-zero naming `providers.mine.interactive_command`, persisting no dispatch record
...
### Pane Capability Is the Default Expectation; Headless Grammar Is What Varies
**Decision**: All four built-ins ship an `interactive_command` — agy's being `agy --dangerously-skip-permissions --model {model}`, carrying its own headless posture flag, no `{effort}` and no initial-prompt argument. The capability model states that every agent CLI has an interactive mode, so **pane capability is the default expectation for any provider**: it rides launch grammar plus the provider-neutral readiness gate, while **headless** grammar is the per-CLI variable that needs probing. A first-run wall is a **gate input**, not a capability gap.
**Why**: "The CLI has an interactive mode" (universally true of a TUI) and "fab ships an `interactive_command` for it" (a fab decision) are different claims, and conflating them judges the next provider by the wrong axis. Prompt delivery is post-launch — `fab dispatch deliver` types a pointer into the running TUI and verifies it landed — so pane capability depends on launch grammar alone and never on whether a CLI accepts a positional initial prompt. The four built-ins' interactive forms are uniform while their headless forms genuinely diverge (stdin vs a `-p` argument, nested shells, per-form approval flags, timeout defaults), which is what makes headless the half worth probing. agy's trust prompt is the same wall kimi's gate clears in one judgment round, so it costs a round rather than the rung.
**Rejected**: `agy … -i {prompt}` spawn-time delivery (superseded by post-launch delivery, and a silently-dropped positional is unverifiable — [dispatch.md](/runtime/dispatch.md) § Prompt delivery); `--effort {effort}` on agy (fights the model-ID suffix); a per-provider "pane-ineligible" config flag (an absent `interactive_command` already encodes it); a provider-specific trust-store pre-seed in Go (undocumented-format provider machinery inside a provider-neutral binary — see [dispatch.md](/runtime/dispatch.md) § First-run walls); keeping a per-provider "is it pane-capable?" question (answered yes four times over).
**Known consequence**: agy's own end-to-end pane run is pending its quota reset (~2026-08-16); the shipped grammar is verified against the installed CLI's own flag surface and the gate mechanism against kimi's identical wall.
