...
**Pane capability is the default expectation.** Every agent CLI has an interactive mode — that is what a TUI is — so every built-in ships an `interactive_command`, the mirror image of headless grammar being the capability that genuinely varies per CLI (stdin vs. argument prompt, subcommand vs. flag) and needs probing. What an `interactive_command` confers is eligibility for **pane-mode dispatch**, where fab types a one-line pointer to the stage prompt file into the pane and verifies that it landed — so what decides the field is not the CLI's argument grammar but its **first-run behavior and input echo**. agy's first-run shape is a fresh-workspace trust prompt that fires even under `--dangerously-skip-permissions` (and worktree-per-change makes every dispatch a fresh workspace); that wall is an ordinary readiness-gate **judgment round** — one answer, remembered per folder, the same shape as kimi's — so it needs no agy-specific code, and agy's trust store (`~/.gemini/antigravity-cli/settings.json`, exact-match paths) is additionally user-seedable. An absent `interactive_command` in a user's own `providers:` block remains a valid configuration: automatic selection skips the pane rung and descends to headless (`descended: pane unavailable: no interactive_command` — [dispatch.md](/runtime/dispatch.md)) and `fab dispatch open` hard-errors with the `providers.<name>.interactive_command` hint. It is just no longer a shipped state.
...
- **GIVEN** a `providers:` entry carrying only a `headless_command`, and a reachable tmux server
...
- **AND** `fab dispatch open` instead exits non-zero naming `providers.<name>.interactive_command`, persisting no dispatch record
...
### Pane Capability Is the Default Expectation — agy Ships Its Interactive Grammar
**Decision**: The agy built-in ships `interactive_command: 'agy --dangerously-skip-permissions --model {model}'` alongside its headless command, so all four built-ins are pane-capable. The shipped value is pinned by a test, and its first-run trust wall is handled by the generic readiness gate — no agy-specific code anywhere.
**Why**: Every agent CLI has an interactive mode, so pane capability rides launch grammar plus the generic readiness gate; headless grammar is the capability that varies per CLI and needs probing. agy's fresh-workspace trust prompt (it fires even under `--dangerously-skip-permissions`, and worktree-per-change makes every dispatch a fresh workspace) is an ordinary readiness-gate judgment round — one answer, remembered per folder, the same shape kimi's identical wall already clears in production — so withholding the field protected against a hazard the gate already handles. The full-auto flag matches agy's own headless posture, and no `{effort}` placeholder exists because its model IDs embed the reasoning level.
**Rejected**: Withholding the field until a live agy pane probe (the gate mechanics are generic and kimi-proven; agy's quota exhaustion only defers *live* confirmation, which is recorded as a follow-up); shipping trust-store seeding in Go or config (provider-specific file-format machinery in a provider-neutral binary — the trust store is documented as user-seedable instead); an initial-prompt placeholder in the command (delivery is post-launch via `fab dispatch deliver`, so pane capability rides launch grammar alone).
