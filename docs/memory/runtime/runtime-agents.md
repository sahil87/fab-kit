...
- The **pid segment is optional and ignored.** Current run-kit hooks append the agent process pid as a third segment; fab validates it (positive integer) and discards it — PID-liveness reconciliation is rk's. Two-segment values remain valid. A consequence: agent state cannot detect an agent *exit* — nothing rewrites the option when the agent process dies, so after an exited interactive agent's `; exec "$SHELL"` shell fallback takes the pane over (see [agent-primitives.md](/runtime/agent-primitives.md) § Spawn composition), the option still reads the agent's last value (typically `idle`). The operator's `agent_exited` delta therefore keys on the pane's foreground `command`, never on agent state (see [operator.md](/runtime/operator.md)).
...
**Why**: Current run-kit hooks write `state:epoch:pid` for rk's PID-liveness reconciler; a last-colon split would read `waiting:1751790000` as the state token and resolve every current value to unknown. Validating rather than ignoring the segment honours run-kit's "malformed ⇒ wholly unknown" contract, and fab has no consumer for the pid itself. The discard has one load-bearing consequence: nothing rewrites the option when the agent process exits, so the value is **stale after exit** — the reason the operator's `agent_exited` detection keys on `pane_current_command` (the foreground process) instead of agent state or a pid-liveness heuristic (see [operator.md](/runtime/operator.md)).
**Rejected**: Splitting on the first colon and ignoring the tail (accepts garbage the writer contract calls malformed). Surfacing the pid in `fab pane` output (no consumer; rk owns liveness). Reading the pid segment for fab-side liveness checks (would still leave the stale-state problem — the option is not rewritten on exit — and duplicates rk's reconciler).
*Introduced by*: 260828-xdmh-rk-pane-agent-state-option; *Updated by*: 260829-1xqx-agent-exit-shell-fallback (the stale-after-exit consequence named, as the `agent_exited` predicate's motivation)
