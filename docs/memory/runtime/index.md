...
| [operator](operator.md) | Operator coordination across repositories and tmux sessions: one generic `tracked`-item model (agent panes — change/stage observed — GitHub PRs, Linear/Slack queries, shell probes, notes) probed by the binary at tick-start, a derived rk-cron cadence, prompt answering via rk mux capture/send, repo/session-targeted spawning, dependency-aware queues, review-pr recovery (one bounded action per stuck reason), and the rk-delegated/built-in launcher. run-kit is a hard dependency (the §2 rk Gate). |
