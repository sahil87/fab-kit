...
**Pipeline consumer — the interactive-pane dispatch adapter.** `fab dispatch start <change> <stage> --pane` (see [dispatch.md](/runtime/dispatch.md)) is the pipeline-side consumer of these procedures: it composes the provider's `session_command` and opens it in a tmux **pane** per § Spawn Composition — including the "embed the initial prompt at spawn as one shell-escaped quoted argument" form, which is *why* the pane adapter delivers a one-line pointer to a prompt file rather than sending the prompt afterwards (embedding at spawn sidesteps the printed-prompt trap entirely: there is no pre-existing buffer to probe). It applies the same composition form to `tmux split-window` (its default shape — splitting the dispatching agent's own window) as to `tmux new-window` (its fallback shape); the § Spawn Composition rules are verb-independent. Its completion detection likewise follows § Await's preference for an **artifact** over a screen pattern — the dispatch contract's `{stage}-result.yaml`. A consumer steering that pane afterwards uses § Pre-send validation and § Peek. The helper carries the primitives; the adapter's own contract (state derivation, record shape, pane placement and identity) is dispatch's.
