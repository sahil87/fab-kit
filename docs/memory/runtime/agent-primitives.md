---
type: memory
description: "The `_cli-agents` helper — agent-CLI interaction primitives any skill opts into via `helpers:`: spawn composition from `fab agent --print` (tier- or provider-addressed; the provider form bypasses tiers AND the provider fill), the pane-exists/agent-idle pre-send gate, the printed-prompt probe, peek (capture + `@rk_agent_state`; uninstrumented panes read unknown), await (poll or announce), and a grammar + discovery-recipe dictionary for the three built-ins plus the `codex mcp-server` bridge."
---
...
**Pipeline consumer — the interactive-pane dispatch adapter.** `fab dispatch start <change> <stage> --pane` (see [dispatch.md](/runtime/dispatch.md)) is the pipeline-side consumer of these procedures: it composes the provider's `session_command` and opens it in a tmux window per § Spawn Composition — including the "embed the initial prompt at spawn as one shell-escaped quoted argument" form, which is *why* the pane adapter delivers a one-line pointer to a prompt file rather than sending the prompt afterwards (embedding at spawn sidesteps the printed-prompt trap entirely: there is no pre-existing buffer to probe). Its completion detection likewise follows § Await's preference for an **artifact** over a screen pattern — the dispatch contract's `{stage}-result.yaml`. A consumer steering that window afterwards uses § Pre-send validation and § Peek. The helper carries the primitives; the adapter's own contract (state derivation, record shape, window naming) is dispatch's.
...
**`fab agent --provider` bypasses the provider default fill too.** Its bypass of tier resolution is total: `providers.<name>.model`/`.effort` are deliberately not consulted on this path, so an omitted flag means empty, not the configured fill. The fill applies on the resolution surface (`fab resolve-agent`, tier resolution) — see [providers-and-tiers.md](/runtime/providers-and-tiers.md) § `fab agent`. Pass `--model` explicitly when a specific model is wanted from this launcher.
...
All three dictionary providers are **built-in providers** resolvable by name — no `providers:` block and no uncommenting is needed to spawn one (`fab agent --provider codex`, or a tier naming it). Only the *volatile* half needs config: pin a current model ID via `providers.<name>.model` (or pass `--model`), since fab ships grammar and no model ID for the non-claude built-ins. The dictionary's command strings stay consistent with that built-in grammar; being markdown, it updates with the kit and carries no behavior risk.
...
**Decision**: Each provider entry records stable invocation grammar plus a recipe for discovering models on the *installed* CLI, and records interactive quirks only once confirmed by a real encounter. No model-ID catalog appears anywhere in it. The same grammar-vs-fill split governs the binary: `internal/agent`'s built-in provider table carries the three providers' invocation *grammar* and no model IDs, so the dictionary and the resolvable provider set describe the same three names, and the volatile half is pinned in config (`providers.<name>.model`).
**Why**: Model catalogs rot within weeks while grammar changes on major-version boundaries, so a recipe stays correct where a list silently misleads; this matches fab's document-don't-validate posture, which never infers a provider from a model string. Keeping the dictionary in markdown means it updates with the kit at zero behavior risk, and asserting unverified behavior about an uninstalled CLI is worse than silence because it reads as verified. Grammar-only built-ins are safe to ship in the binary for the same reason the dictionary is safe to ship in markdown — neither carries a value that rots — and a built-in provider is inert until named, so the dictionary can point at `--provider codex` as a working invocation rather than at a config-editing prerequisite.
**Rejected**: Baked model IDs per provider, in the dictionary or the Go table (rot monthly); speculative quirk entries for CLIs never actually driven (unverifiable claims presented as fact); describing a config-editing prerequisite the resolvable provider set does not impose (the dictionary's whole value is that its recipes work as written).
*Introduced by*: 260805-nvad-cli-agents-helper-provider-spawn; *Updated by*: 260805-j3cm-builtin-provider-templates-and-fill
