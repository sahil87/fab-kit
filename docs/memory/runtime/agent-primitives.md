...
2. **Pre-send validation** — a two-step gate before sending keys into an existing pane: the pane exists (refreshed pane map — a dead pane swallows keys silently, so a stale pane ID is a silent-failure hazard, not a visible error), and the agent's state fits the send intent per the three-state `@rk_agent_state` read. `fab pane send` enforces the same gate with a mode per intent (plain send for command routing — a parseable `active`/`waiting` refuses; `--answer` for delivering a detected prompt's answer to a `waiting` agent — still refuses `active`; `--force` as the skip-everything override; an **unknown** state — a foreign-agent or uninstrumented pane — warns `agent state unknown — sending anyway` and sends in both non-force modes), so consumers prefer it over raw `tmux send-keys` and let the binary hold the gate — key-name input (bare Enter, arrows, `C-c`) is the one case that stays raw, since the literal-text send cannot express it. Everything beyond the two mechanics — whether to ask the user, how many times to retry, whether the pane is on the right change or branch — is consumer policy.
