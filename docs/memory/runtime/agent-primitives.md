...
Peeking SHALL read output and agent state as separate axes: output via `fab pane capture <pane> [-l N] [--json]` (or `--raw` for the bare captured text; `-l N` returns the last N lines — blank screen-padding stripped, tailed internally, see [pane-commands.md](/runtime/pane-commands.md)), state via the pane's `@rk_agent_state` option (surfaced by `fab pane map`'s Agent column and `fab pane capture --json`'s `agent_state`/`agent_idle_duration`). A wide capture window (`-l 50`+) compensates for line wrapping when scanning for a prompt.
