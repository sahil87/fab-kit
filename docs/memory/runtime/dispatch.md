...
### Start-Probe Discovery Deliberately Ignores the Labelled Rung
**Decision**: Stage-dispatch consumers branch on `dispatch:` key presence but deliberately do not read `dispatch.rung`; inside the CLI-adapter arm they attempt `fab dispatch start` first and let its pre-write refusal redirect pane landings to `open`.
**Why**: The start probe keeps the existing choreography explained and preserves one discriminator across the pane and headless entries without coupling skill prose to the labelled rung.
**Rejected**: Branching on `dispatch.rung` in the skills, which duplicates adapter-selection semantics at every consumer and leaves the start redirect boundary unexplained.
