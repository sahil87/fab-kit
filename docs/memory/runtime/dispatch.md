---
type: memory
description: "`fab dispatch {start,status,logs,kill,clean}` — the tmux-independent headless process manager for CLI-dispatched pipeline stages: the `SysProcAttr{Setsid:true}` detached `sh -c` wrapper, the `.fab-dispatch/{id}/` state layout, the five byte-stable status states (incl. `failed (no-result)`), refuse-if-running concurrency, POSIX timeout, two cleanup paths with no auto-GC, POSIX-only v1, and the tier → provider → `dispatch_command` resolution with NO fallback to `session_command`."
---
# fab dispatch
