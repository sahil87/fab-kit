...
`fab dispatch` is a **tmux-independent, headless process manager** for launching a pipeline stage's resolved dispatch command (the provider `dispatch_command`, persisted in the record as `spawn_cmd`) as a detached CLI worker, tracking it via a repo-root state dir, and exposing poll/logs/kill/clean surfaces. It is the headless pipeline path — the parallel, independent complement to the tmux-bound interactive `fab pane` / `fab operator` family (see [pane-commands.md](/runtime/pane-commands.md) and [operator.md](/runtime/operator.md)), which stays exactly as-is. Dispatch is the runtime for **cross-harness stage dispatch** ("a codex orchestrator runs `apply` on claude"): a fundamentally launch-and-poll problem, not a pane-observation one. It is change 3c of the cross-harness dispatch series; it *runs* the provider `dispatch_command` that `fab resolve-agent` emits (a tier names a provider; the provider carries the command — since tykw's agent config v3; before tykw this was a per-tier `spawn_command` emitted on a `spawn=` line — see [_shared/configuration.md](/_shared/configuration.md) § `providers` and [runtime/providers-and-tiers.md](/runtime/providers-and-tiers.md)), and its cross-adapter protocol is fixed by the human-curated spec `docs/specs/harness-adapters.md`. The **3d skill wiring** (aetz) consumes it: the dispatch-seam skills branch on the resolved `dispatch=` line (renamed from `spawn=` in tykw) and, when present, drive this command family — `fab dispatch start` (block prompt on stdin) → `sleep 30` polling of `fab dispatch status` → the five states below → read `{stage}-result.yaml` on `done`. The wiring lives in `_preamble.md` § CLI-Adapter Dispatch + § Dispatch-Prompt Obligations (see [pipeline/execution-skills.md](/pipeline/execution-skills.md) § Status-transition ownership and [_shared/context-loading.md](/_shared/context-loading.md) § Per-Stage Model Resolution).
...
### Requirement: No-dispatch_command error (no fallback)

If the resolved tier's provider has no `dispatch_command`, `start` SHALL error clearly — naming the stage, the resolved tier, and the `providers.<name>.dispatch_command` config key (tykw — was `agent.tiers.<tier>.spawn_command`) — and MUST NOT fall back to the provider's `session_command`. This no-cross-fallback rule is the load-bearing dispatch-mode semantic (a fallback would silently flip a session-command-only provider into CLI dispatch); see [_shared/configuration.md](/_shared/configuration.md) § `providers` and [runtime/providers-and-tiers.md](/runtime/providers-and-tiers.md).
