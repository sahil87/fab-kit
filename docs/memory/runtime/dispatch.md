...
*Introduced by*: 260809-3oz7-pane-readiness-gate-sendkeys-delivery; *Updated by*: 260829-57mp-pane-readiness-agent-takeover (the foreground-command precondition joins the classification — shell foreground ⇒ `booting`, zero keystrokes)

### The takeover precondition sits in front of DeriveReadiness, not inside it
**Decision**: `Probe` checks `IsShellCommand(CurrentCommand(pane))` before typing the sentinel; `DeriveReadiness` keeps its pure `(echoed, first, second)` signature and table.
