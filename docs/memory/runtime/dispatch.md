...
| `{stage}-result.yaml` | the **dispatched agent** (contract) | the stage result; dispatch defines only the path + consumes its presence. Presence is required for `done` in both modes and is the **sole** completion signal in pane mode. Its **content schema** is a minimal YAML envelope mirroring each native block's return — common `stage`/`status`/`summary`; apply adds `failed_task`/`reason` on failure; review adds `verdict` (pass\|fail) + `findings{must_fix,should_fix,nice_to_have}`; hydrate carries only the envelope; ship adds `pr_url` (+ `reason` on failure); review-pr adds a four-class `outcome` (`success`\|`failure`\|`no-reviews`\|`timeout` — git-pr-review's own Step 6 outcome classes) + `reason` on `outcome: failure`. The **`status` (worker/infra outcome) vs `verdict`/`outcome` (stage outcome) split is load-bearing** — a completed review with `verdict: fail` is dispatch-state `done` (result present), and the orchestrator then takes the normal review-fail path; likewise review-pr's `outcome: failure` and `outcome: timeout` are dispatch-state `done`, never `failed` — a first timeout maps to the orchestrator's existing leave-`active` + pending-message path, not the restart budget; the second consecutive timeout on the same PR exits as `no-reviews` (reason `review-gate-unavailable` — see [execution-skills.md](/pipeline/execution-skills.md) § Review-PR Always Terminalizes), whose `summary` the orchestrator reports. Dispatch-state `failed` is reserved for worker/infrastructure failure. Schema documented in `_preamble.md` § Dispatch-Prompt Obligations. |
...
`dispatch.SelectMode` is pure: it receives explicit signals, preferred mode, provider capabilities, and a tmux-availability value, and performs no environment read or tmux probe. `fab agent <stage> -o yaml` supplies environment-derived availability; start/restart supply a real reachability result before any state write.
...
### Requirement: `fab dispatch ready <change> <stage>` is a mechanical probe gated on agent takeover

`ready` SHALL answer one question about an opened pane — *can it accept typed input right now?* — and report exactly one of `ready`, `booting`, or `parked`, derived from the pane's foreground command plus a mechanical classification: never from `@rk_pane_agent_state` directly, never from a pattern table of known dialogs. Before any keystroke the probe reads the pane's foreground command (`tmux display-message -p -t <pane> '#{pane_current_command}'`): while its basename is a known shell — for the supported unwrapped pane shape, the provider binary has not taken the tty — it reports `booting` and types **nothing**, because a shell holds the pty in cooked mode, where the tty echoes typed characters by itself, so the sentinel would echo for a reason that has nothing to do with an agent being ready. This takeover precondition runs fab-side ahead of BOTH classification arms — rk's await deliberately classifies a cooked-shell echo as ready, the exact false-ready this guard closes — and rk is never invoked while it holds. Past the precondition, the classification runs in two arms. The **rk arm** (preferred): when a sentinel-capable run-kit is on PATH — capability probed from the binary, `rk mux await --help` mentioning `parked`, cached at most once per process, never a version compare — the probe runs `rk mux await --ready <pane> [-L <server>] --timeout <bounded internal constant>` and maps the report word: `ready` (state-present or sentinel echo) → `ready`; `parked` → `parked`; timeout `running` → `booting`; `gone` → the dead-pane error, not a classification. Any unexpected rk outcome (non-zero exit, unparsable report) fails OPEN to the raw arm for the same probe with at most one stderr warning per process; a classified rk outcome is an answer and is never re-classified. The **raw-tmux arm** (rk absent, not sentinel-capable, or failed unexpectedly) is fab's own classifier: it sends the sentinel with `send-keys -l` (never submitted), takes the captures, clears the sentinel with `C-u` whether or not it echoed, and presses no other key.

The direct foreground-command precondition is scoped to the unwrapped panes created by `fab dispatch open` and `fab pane open`. On Linux, a wrapped interactive spawn keeps its wrapper shell as the foreground process-group leader while its live agent child runs, so `fab pane ready` reports `booting` for that shape. Operator sends do not use this gate: they ride run-kit's agent-state gate, and dispatch pane workers are unwrapped. Wrapped-pane readiness is a backlog candidate outside the current gate contract. (260902-ssyf-operator-liveness-process-tree)

| Report | Condition |
|--------|-----------|
| `ready` | the pane accepts typed input — rk's state-present fast path or sentinel echo on the delegated arm, or the echoed sentinel on the raw arm |
| `booting` | the pane's foreground command is still a shell (the provider has not taken the tty — no keystroke is sent on this path, and rk is never invoked), rk's bounded await timed out (`running`), or no echo and the screen is blank or **changed** between two captures spaced by an internal stability delay — a TUI still painting itself |
| `parked` | no echo on a **stable, non-blank** screen — a dialog, survey, login wall, or wedged process is holding the input |

On the raw-tmux arm the post-takeover classification is the pure `DeriveReadiness(echoed, first, second)`, matching the package's `SelectMode`/`DerivePaneState` precedent — the foreground-command precondition sits in front of it rather than inside its table; the blank-screen case precedes the difference check because two identical *empty* captures are stable by the letter of the rule while meaning the opposite of parked. **Both stability captures are taken BEFORE the sentinel is cleared**: `C-u` is itself a keystroke, and a TUI that repaints its input line in response would make every straddling capture pair differ, so a genuinely parked pane would read `booting` forever.

Every non-`ready` report SHALL carry the pane ID, the record's socket when non-empty, and a trailing capture snippet — fab's OWN capture on both arms (rk's stderr formatting is not contract) — everything a judgment round needs to answer the wall with `tmux [-L <server>] send-keys` without a second lookup. The snippet is the last 20 lines counted **after** the pane's trailing blank padding is dropped, so a dialog drawn near the top of a tall pane is what a reader sees; an empty snippet prints no header at all.
...
- **THEN** stdout is exactly `ready` (rk's state-present or sentinel-echo report on the delegated arm; the echoed sentinel, cleared with `C-u`, on the raw arm)
...
- **AND** GIVEN a pane whose screen is still changing between the two captures (or rk's bounded await timing out), the report is `booting`
- **AND** GIVEN a pane whose foreground command is still a shell, the report is `booting` with the snippet and no keystroke was sent — regardless of what an echo would have shown

### Requirement: `fab dispatch deliver <change> <stage> [--prompt-file <path>]` is the sole, verified delivery

`deliver` SHALL be the **only** mechanism that hands a pane worker its prompt — for both initial dispatch and rework-cycle continuation — and SHALL verify every step that could silently do nothing. Per attempt: an internal readiness probe → `C-u` → capture the cleared baseline → type the one-line pointer literally → **capture-verify that the pointer newly echoed** → `Enter` → **confirm the screen advanced**. Every keystroke rides `internal/pane`'s shared send seam, whose pre-send pane-mode guard cancels copy-mode (or any tmux mode) a human left the pane in before typing — see [pane-commands.md](/runtime/pane-commands.md) § Shared Pane Package. A failed check costs one attempt; there is exactly **one retry** (reported as a `warning: delivery attempt 1 failed (…); retrying` on stderr even when the retry succeeds), and a second failure exits non-zero with the pane's last lines on stderr. A pane that failed verification twice needs eyes, not a loop.
...
- **WHEN** the echo is counted — for `deliver`'s pointer or for the raw arm of `ready`'s sentinel, which share `countWrapped`
...
A pane below **either** floor (`dispatch.min_cols` / `dispatch.min_rows`, defaults **50 cols × 20 rows**, scope `both`; a dimension exactly at the floor passes) demotes the launch to a **manually-sized detached window** — `pane.OpenManualWindow`, which runs `new-window -d` printing both ids, then `set-option -w window-size manual` and `resize-window -x 200 -y 50` (the **ManualWindowCols/ManualWindowRows constants** — config would be speculative surface; the floor pair is the policy knob). The demotion warns on stderr naming the measured window, the requested placement, the failing dimension, and the action: `warning: window 100x16 too narrow for a 45% column (45 cols < 50): opening worker in its own window` (height-limited variants read `too short … (N rows < M)`; the stacked variants say `a stacked split`). Sizing follow-up failures after a successful `new-window` are non-fatal warnings, and the report keeps the window form (`pane %N, window fab-{id}-{stage}`) — downstream is shape-blind. Both probe failure and unparseable geometry are **fail-open**: the launch proceeds with today's split byte-for-byte, warning-free, and never demotes on unmeasured evidence. When the floor passes, the split argv sequence — sibling stack, sized carve, unsized retry — is byte-for-byte unchanged.
...
- **GIVEN** `dispatch.column_width` resolving to 45
- **WHEN** the first pane worker of a window is dispatched
- **THEN** the split argv carries `-h -l 45%` and the dispatcher keeps ~55% of the window width
...
- **GIVEN** a dispatcher in a viewer-shrunk 100x16 window, `dispatch.column_width` 45, floor 50x20
- **WHEN** `fab dispatch open <change> <stage>` launches a pane worker
- **THEN** no split occurs: the worker opens in its own detached window with `window-size manual` resized to 200x50, stderr carries `warning: window 100x16 too narrow for a 45% column (45 cols < 50): opening worker in its own window`, and the record carries the new pane's ID exactly as the window shape always has
...
A user MAY converse with a running pane worker mid-stage. This changes **no** contract: the worker still owes `{stage}-result.yaml`, still ends with the terminal `fab status refresh <change>` epilogue, and still runs no `fab status` **transition** command — **the orchestrator owns every transition** (ship/review-pr workers excepted: they self-manage their own stage's transitions per the `_preamble.md` § Dispatch-Prompt Obligations carve-out; see [pipeline/execution-skills.md](/pipeline/execution-skills.md) § Status-transition ownership). Steering is human input into the worker's context, exactly like answering a native sub-agent's question. A worker steered away from producing a result needs no new state: it never reaches `done` and surfaces through the never-`done` escalation the orchestrator already owns. This is **documentation only** — no code detects, gates, or reports a steered worker.
...
### The geometry defaults admit a 120-column window by value, not by formula

**Decision**: The shipped `dispatch.column_width` / `dispatch.min_cols` pair is `45` / `50`, chosen so an ordinary 120-column tmux window carves a 54-col worker column (≥ the 50-col floor) and splits; the smallest window that still splits is 112 cols (112 × 45 / 100 = 50, exactly at the floor), and 111 cols and below demote to the manual window. The dispatcher keeps 55% of the width.

**Why**: 120 columns is a very common terminal width, and the two existing policy knobs already govern the outcome — so admitting it is a defaults choice, not a mechanism change. 50 cols is the narrowest worker column judged usable for the shipped agent TUIs. Both values live only in `defaults.yaml` (see [_shared/configuration.md](/_shared/configuration.md) § Design Decisions → "Dispatch Defaults Are Init-Injected from `defaults.yaml`"), so the bump touches no Go logic and needs no migration: the advert is commented reference text and a user's explicit override keeps winning.

**Rejected**: An absolute-column fallback — carving `-l max(pct%, min_cols)` and demoting only when `window < 2 × min_cols` — would reach the same 120-col threshold but changes the carve formula, the split argv, the warning grammar, and every doc and test that describes them, for a marginal gain over better defaults.
...
### Consumers Branch on the Labelled Rung; Start's Refusal Is Defense-in-Depth
**Decision**: Stage-dispatch consumers branch on `dispatch:` key presence and, inside the CLI-adapter arm, on the labelled `dispatch.rung` — `rung: headless` goes straight to `fab dispatch start`, `rung: pane` straight to `fab dispatch open`. `start`'s pre-write pane-refusal remains as defense-in-depth for a mislabelled or stale-environment landing, never as the discovery mechanism.
**Why**: The label is computed by the same `SelectMode` ladder the launch verbs re-resolve, so reading it loses no correctness while saving a refused-`start` invocation per pane dispatch and removing probe-and-branch indirection; the retained refusal still catches resolution-to-launch environment skew, and branching on the label is a read of the surfaced resolution, not a duplication of adapter-selection semantics.
**Rejected**: Keeping the start-probe as the primary discovery mechanism — an interim posture 260901-u6es held deliberately (so the mechanical YAML migration and this semantic simplification stayed independently reviewable) that pays a wasted probe per pane landing once the label ships.
*Introduced by*: 260901-u6es-fab-agent-yaml-skill-migration; *Updated by*: 260902-0i4x-labelled-rung-dispatch-choreography
...
**Decision**: `countWrapped`'s `squeeze` drops runes in the Unicode box-drawing block (U+2500–U+257F) alongside every whitespace rune, on both the capture and the needle — one predicate covering both shapes a wrap can insert, at both call sites (the raw-tmux arm of `ready`'s sentinel echo and `deliver`'s pointer echo — the delegated rk arm runs rk's own probe and does not count an echo fab-side).
...
*Introduced by*: 260810-ki9v-kimi-pane-enablement; *Updated by*: 260904-b4j7-gate-delegation-rk-await-ready (the `ready` call site scopes to the raw-tmux arm — the rk arm delegates the probe)

### First-run walls are classified mechanically and answered by agent judgment
**Decision**: `fab dispatch ready` classifies a pane by its foreground command first — a shell foreground is `booting` with zero keystrokes sent, since a cooked-mode shell echoes the sentinel by itself — and, once a non-shell process owns the pane, by a two-arm mechanical classification — delegated to a sentinel-capable `rk mux await --ready` when one is on PATH, fail-open to fab's raw-tmux sentinel echo + screen stability — `ready` / `booting` / `parked` — and reports a capture snippet with every non-`ready` answer. It never presses Enter, never answers anything, and carries no table of known dialogs; deciding what a parked screen wants is the orchestrator's judgment, bounded by the wiring's 2-round budget with login walls escalating immediately.
**Why**: Dialog text is a version treadmill and provider-specific, and a half-matched pattern pressing Enter into an unknown screen is worse than stalling. An agent already reads screens for a living, and the snippet gives it everything it needs in the same call. Keeping the binary's half purely mechanical is also what keeps it provider-neutral: nothing in Go knows what a trust prompt looks like. The foreground-command half exists because the echo is only trustworthy post-takeover: a tmux pane starts with a shell holding the pty in cooked mode, so in the window before the provider TUI takes the tty the sentinel echoes with no agent ready, and every downstream verification built on the echo false-verifies with it.
**Rejected**: A Go pattern table of known dialogs (version treadmill, blind keypresses). Provider-specific trust-store pre-seeding — probed and working for agy, but undocumented-format provider machinery inside a provider-neutral binary. Consulting `@rk_pane_agent_state` (couples dispatch to the operator's agent-state reader for an answer the foreground command already gives).
*Introduced by*: 260809-3oz7-pane-readiness-gate-sendkeys-delivery; *Updated by*: 260829-57mp-pane-readiness-agent-takeover (the foreground-command precondition joins the classification — shell foreground ⇒ `booting`, zero keystrokes), 260904-b4j7-gate-delegation-rk-await-ready (the post-takeover classification is two-arm — rk-delegated, raw-tmux fallback)

### The takeover precondition sits in front of both classification arms, not inside either
**Decision**: `Probe` checks `IsShellCommand(CurrentCommand(pane))` ahead of BOTH arms — rk is never invoked while a shell owns the pane; `DeriveReadiness` keeps its pure `(echoed, first, second)` signature and table as the raw-tmux arm's classifier.
**Why**: Two unrelated axes — who owns the pane vs. what the screen shows — should not share one classifier table; the package's pure-classifier precedent is one decision per function. rk's await deliberately classifies a cooked-shell echo as `ready (echo)` (terminals-are-one-standard — correct for rk's send use case), which for a dispatch pane is exactly the false-ready this guard closes; the precondition is also not part of the classification the agent-messaging spec delegates (state-present / sentinel echo / parked).
**Rejected**: A fourth `DeriveReadiness` parameter — spreads the ownership question across every existing table row. Pushing the guard upstream into rk (a run-kit change — rk's terminals-too standard is deliberate; noted as a run-kit follow-up idea).
*Introduced by*: 260829-57mp-pane-readiness-agent-takeover; *Updated by*: 260904-b4j7-gate-delegation-rk-await-ready (the precondition now fronts the rk arm too — rk is never invoked while it holds)

### Readiness classification delegates to a sentinel-capable `rk mux await --ready`, fail-open
**Decision**: Past the takeover precondition, `Probe` prefers the rk arm — `rk mux await --ready <pane> [-L <server>] --timeout <bounded constant>` — branching on the first token of rk's stdout report line and mapping onto the frozen contract: `ready` (state-present or sentinel echo) → `ready`, `parked` → `parked`, timeout `running` → `booting`, `gone` (exit 1) → the dead-pane error (never a classification, never fallback), `narrow` (rk declining to classify a pane under its own geometry floor) → a silent hand-off to the raw-tmux arm with no warning. Any other rk outcome — non-zero exit, empty or unparseable stdout — fails open to the byte-identical raw-tmux arm for the same probe, with at most one stderr warning per process; a classified rk outcome is an answer and is never re-classified.
**Why**: The agent-messaging spec assigns mechanical classification to run-kit ("fab consumes, never reimplements"), and rk's classifier is strictly stronger on rk-equipped machines — a state-present fast path that types nothing into an instrumented agent, and one bounded call absorbing boot churn. Two live copies of one classifier drift; delegating inside `Gate.Probe` upgrades all three consumers (`fab pane ready`, `fab dispatch ready`, `Gate.Deliver`'s precondition) at once with every report-word, exit-code, and JSON contract frozen.
**Rejected**: A skill-side rewrite (the gate's callers are Go verbs whose frozen report contract the skills branch on — a rewrite would fork it); requiring rk (fab must work rk-less — the raw arm is byte-identical); re-running the raw arm after rk answered (double-types sentinels into the pane).
*Introduced by*: 260904-b4j7-gate-delegation-rk-await-ready

### Sentinel capability is probed from rk's help text, never a version compare
**Decision**: The rk arm's availability is decided by `exec.LookPath("rk")` plus `rk mux await --help` mentioning `parked` (present in the sentinel classifier's `--ready` flag help, absent from the earlier capture-settle one), computed at most once per process; the runner/probe seams are injectable package-level vars (the `rkPanesRunner` precedent) for tmux-free, rk-free tests.
**Why**: `--ready` predates the sentinel classifier with weaker capture-settle semantics (the `ready (settled)` false-fire hazard), so flag presence is not capability; and a version string can lie — a bottle can predate a same-version source change. Verify the binary, not the version.
**Rejected**: An `rk --version` compare (version-skew hazard); probing by invoking `--ready` against a scratch pane (side-effectful, slow).
*Introduced by*: 260904-b4j7-gate-delegation-rk-await-ready

### Snippets stay fab-captured on both arms
**Decision**: Non-`ready` snippets come from fab's own `Capture`+`Snippet` path on the rk arm too (one extra capture per non-ready probe); rk's stderr surfaces only in the fail-open warning.
...
### Bounded rk timeout; `running` maps to `booting`
**Decision**: The rk call rides an unexported 20-second constant (`rkReadyTimeout` — mechanics, not policy; no flag, no config field), and its expiry (rk's `running`) maps to `booting`.
**Why**: rk's await blocks through boot churn, so one probe call absorbs most boots; mapping the bound's expiry to `booting` preserves every documented report word and the skill-side gate loop's re-probe cadence and 5-consecutive-booting allowance unchanged.
**Rejected**: Blocking indefinitely inside `ready` (changes the verb's latency contract and stalls `deliver`'s precondition); exposing the timeout as a flag or config field (the gate-timings precedent — mechanics stay unexported constants).
*Introduced by*: 260904-b4j7-gate-delegation-rk-await-ready
...
### Ship/review-pr result schemas mirror their skills' native return contracts
**Decision**: The dispatched ship worker's result file carries `status` + `pr_url` (+ `reason` on failure); the review-pr worker's carries `status` + a four-class `outcome` (`success|failure|no-reviews|timeout` — git-pr-review's own Step 6 classes) + `summary` (+ `reason` on `outcome: failure`). `outcome: failure` and `outcome: timeout` are dispatch-state `done` (result present), never dispatch-state `failed`.
**Why**: The Copilot-poll timeout is a healthy, expected stage outcome that maps to the orchestrator's existing leave-`active` + pending-message path; routing it through `status: failure` or dispatch-state `failed` would spend the one-restart recovery budget on a non-failure. Reusing git-pr-review's own Step 6 class set adds no new taxonomy.
**Rejected**: Overloading `status: failure` for timeout (routes a healthy outcome into the restart budget); a new dispatch state for timeout (the five-state machine is byte-stable — the outcome rides inside a `done` result).
*Introduced by*: 260903-0y4c-ship-reviewpr-cli-dispatch; *Updated by*: 260916-vb4l-review-pr-recovery (the leave-`active` mapping binds the FIRST timeout in the activation only — the second consecutive timeout on the same PR arrives as `outcome: no-reviews` with reason `review-gate-unavailable`, which the orchestrators already treat as a successful no-op whose `summary` is reported)
