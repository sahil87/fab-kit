...
1. `fab/project/config.yaml` — project configuration: identity (name/description), `source_paths`/`test_paths`, true-impact excludes, plan-acceptance extra categories, the `providers:` command table, the `agent.tiers` five-role-tier override, optional `stage_hooks` (the former `review_tools` toggles moved to `code-review.md` § Review Tools and `agent.spawn_command` to `providers.claude.session_command`, both in tykw; descriptor rewritten in c5tr — it formerly advertised "naming conventions, model tiers", both removed/dead surfaces)
...
**Nested dispatch**: When a subagent dispatches its own sub-subagent, the inner prompt MUST also include the standard subagent context instruction. The same 5 files are loaded at every nesting level. (The rule is general; it no longer cites the review stage as its canonical example — since the pag2 single-agent collapse the review block runs inline with no nested reviewer sub-agents.)
