...
- **Both dispatch commands nest a shell: `sh -c '<cli> … -p "$(cat)"'`.** `fab dispatch` pipes the stage prompt to the command's **stdin**, but both CLIs take the prompt as the **argument** to `-p` and never read stdin. POSIX expands `$(cat)` before the redirect applies, so the un-nested form would read the *outer* stdin; nesting makes the inner shell's stdin the redirected prompt file. (codex needs no nesting — it reads its prompt from stdin via the `exec` subcommand.)

**Presentation guarantees.** The rendered claude block restates its built-in `native: true`, `session_command`, and `dispatch_command` live; codex/agy/kimi examples remain commented and parse as absent project overrides (`GetProvider` returns `!ok` for all three). All strings derive from the embedded defaults and render as single-quoted scalars via the exported `configref.YAMLSingleQuoted` — the one owner of the quote-doubling rule the nested-shell agy/kimi dispatch commands depend on — so whole-block uncommenting remains valid YAML, and byte-stability/coverage guards pin the four-provider capability and fill surface.
...
**Why (built-in default templated) (gvxd)**: (1) **explicitness** — a reader of the scaffold *sees* where the role profile lands instead of it being appended invisibly by `WithProfile`; (2) **internal consistency** — every command string in the scaffold is templated (the live claude `session_command` matches its own commented `dispatch_command` and the non-claude starter blocks); (3) **reference sync by construction** — `fab config explain` renders the `session_command` from `agent.DefaultSessionCommand` (`configref.go`), so the reference cannot contradict the binary. Template-mode substitution of the placed-last placeholders yields output byte-identical to the append path (verified empirically against `fab agent default/operator --print`), and append mode stays load-bearing for plain-form user configs (some pin `--model`/`--effort` and rely on append-last/last-wins) — both forms resolve identically, so no migration ships.
