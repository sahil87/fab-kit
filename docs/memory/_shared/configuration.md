...
4. **built-in defaults** — applied at the existing **point-of-use** seams (`internal/agent`'s role/provider merge and nil-safe accessors), and projected as a materialized tier for the read-model surfaces by `configref.DefaultsMap`/`DefaultsMapFor` (§ Six-Verb Surface). Their physical source is `internal/agent`'s embedded `defaults.yaml`, shaped as a config-file fragment so it can serve as tier 0 (see [runtime/providers-and-profiles.md](/runtime/providers-and-profiles.md) § The built-in defaults are an embedded `defaults.yaml`)
...
- **`interactive_command`** declares how to open an interactive session and is the pane-dispatch prerequisite. It MAY additionally carry the `{prompt}` placeholder marking where a launch's initial prompt is delivered (§ `{prompt}` — the initial-prompt delivery point, below).
...
Fab ships `claude`, `codex`, `agy`, and `kimi` as built-ins. claude has all three capabilities (`native: true` plus both commands); codex and agy have interactive and headless commands; kimi has a headless command only (no `interactive_command`, so no pane capability). None of the three non-claude built-ins declares native capability, and kimi deliberately ships no fills. Claude's headless grammar is exactly `claude -p --dangerously-skip-permissions --model {model} --effort {effort}`. User entries extend or override the built-ins without changing the mode policy merely by being present.
...
**`{prompt}` — the initial-prompt delivery point (`interactive_command` only).** An `interactive_command` MAY carry a third placeholder, `{prompt}`, marking where a launch's **initial prompt** lands. It follows the same substitute-or-drop contract as `{model}`/`{effort}` and shares their one token-drop implementation, but sits on a different axis and a different clock:

- **It does not select template mode.** `isTemplate` keys on `{model}`/`{effort}` alone, so a command whose only placeholder is `{prompt}` still takes append mode and receives its ` --model`/` --effort` at the end. Folding `{prompt}` in would silently suppress that append.
- **It resolves on a separate, later pass.** `spawn.WithPrompt` (with `spawn.HasPrompt` as its predicate) runs **after** `spawn.WithProfile`, never as a third `WithProfile` parameter — model and effort are known at profile resolution, while a prompt exists only at launch, and running last means a prompt whose text happens to contain `{model}` is never re-substituted.
- **Two delivery shapes, one implementation.** Every seam that launches an agent **with** initial content — pane dispatch (`internal/dispatch.WindowCommand`), the operator launcher, and both `fab batch` worker spawns — goes through `spawn.DeliverPrompt`: substitute the **shell-quoted** prompt at `{prompt}` when the command declares one, **append** it as a trailing shell-quoted positional when it does not. Quoting is inside the helper, so both shapes are provably consistent, and a command with no placeholder composes byte-for-byte as it always has (claude, codex, and any user command written before the placeholder existed).
- **The empty case belongs to the promptless seam.** Bare `fab agent` is the only launch composing an `interactive_command` with no initial content; it calls `WithPrompt(cmd, "")` directly so the pair token-drops. `DeliverPrompt` takes real prompts only — one entry point meaning both "deliver" and "drop" is the delivery bug this grammar exists to prevent.

Use the placeholder when a CLI's initial-prompt flag **takes a value**: agy's `-i`/`--prompt-interactive` does, so a positional is discarded and a bare trailing `-i` hard-errors — the placeholder is the only shape serving both kinds of launch from one field. `headless_command` has no `{prompt}` support: headless delivery is stdin, not argv.

**Four built-in providers rendered as commented reference defaults.** `fab config explain` (`internal/configref`) presents the `providers:` block as claude/codex/agy/kimi, each command string interpolated from its canonical `internal/agent` symbol — which reads the embedded `defaults.yaml` — so the reference holds no literal copy. Presentation is the ordinary commented-reference style **every non-overridden default uses**, because all four are Go built-ins: the blocks merely *restate* what the binary already resolves, rather than being uncomment-to-opt-in additions. The registry row's `Description` and `Segment` document the `profiles` fills, including the precedence position, the `profiles.default` cross-role fallback, and the `scope: both` machine-wide-fill affordance. The block is `advertise: false`, so it is documented **here in the reference and not scaffolded** into each project's managed fence.

- **Claude's `native`, `interactive_command`, and `headless_command`** are live restatements of its three built-in capabilities. The command's presence does not select headless mode; `dispatch.mode` does.
- The **codex, agy and kimi blocks** ship commented, each showing the command fields its built-in actually defines plus its **real shipped `profiles:` fills** — interpolated from `agent.ResolveProvider(nil, name).Profiles` in `agent.RoleNames()` order (no map range-iteration, so the rendering is byte-stable), with the renderer's omitempty shaping so agy's model-only rows and codex's effort-only rows each render as written, and a note to override one to pin a newer model. kimi has no fills, so `profilesLines` returns "" and its block renders no `profiles:` key at all rather than a stray empty one. Commented is deliberate even though the values are real: a block restating a built-in registers no project override.
...
Three facts on the agy and kimi strings:

- **agy's block opens on an `interactive_command` ending in `-i {prompt}`; kimi's block has none.** agy's `-i`/`--prompt-interactive` takes a value, so the placeholder is what lets one field serve both a prompt-carrying and a promptless launch (§ `{prompt}`). Rendering it carries agy's trust-dialog caveat with it — a pane worker in a not-yet-trusted worktree parks once per worktree until a human answers, and fab does not seed the trust store ([runtime/providers-and-profiles.md](/runtime/providers-and-profiles.md) § agy's pane capability rides `{prompt}`).
...
**Presentation guarantees.** The rendered claude block restates its built-in `native: true`, `interactive_command`, and `headless_command` live; codex/agy/kimi examples remain commented and parse as absent project overrides (`GetProvider` returns `!ok` for all three). All strings derive from the embedded defaults and render as single-quoted scalars via the exported `configref.YAMLSingleQuoted` — the one owner of the quote-doubling rule the nested-shell agy/kimi headless commands depend on — so whole-block uncommenting remains valid YAML, and byte-stability/coverage guards pin the four-provider capability and fill surface.
...
### `{prompt}` Is a Third Placeholder on Its Own Pass, With One Delivery Helper
**Decision**: `{prompt}` is recognized by `internal/spawn` alongside `{model}`/`{effort}` and shares their single token-drop implementation, but it is resolved by its own exported pass (`spawn.WithPrompt`, predicate `spawn.HasPrompt`) running **after** `WithProfile` rather than by extending `WithProfile`'s signature — `isTemplate` keeps keying on `{model}`/`{effort}` alone. The two delivery shapes (substitute-at-placeholder / append-as-positional, both shell-quoted) live in exactly one helper, `spawn.DeliverPrompt`, consumed by `internal/dispatch.WindowCommand`, the operator launcher, and both `fab batch` worker spawns. The empty-value case stays on `WithPrompt`, reached only by the one promptless seam (bare `fab agent`).
**Why**: The placeholders have different lifetimes — model and effort are known at profile resolution, a prompt only at launch — and the mode decision must not key on the placeholder that must not influence it: a command carrying only `{prompt}` still has to receive its appended `--model`/`--effort`. Running the prompt pass last also means a prompt whose text contains `{model}` is never re-substituted. Putting the shell-quoting **inside** `DeliverPrompt` is what makes the two paths provably consistent — a caller that quoted only one path is exactly the bug class the single helper removes, and it is what keeps the placeholder-free composition byte-for-byte identical for claude, codex, and pre-placeholder user commands.
**Rejected**: A third parameter on `WithProfile` (forces every call site to name a prompt value most of them do not have, and couples the mode decision to `{prompt}`); folding `{prompt}` into `isTemplate` (silently suppresses the `--model`/`--effort` append for a `{prompt}`-only command); one entry point where `DeliverPrompt(cmd, "")` means "drop" (one keystroke from silently delivering an empty argument, the delivery bug this design prevents); `{prompt}` support in `headless_command` (headless delivery is stdin, not argv).
...
**Decision**: `configupgrade.CommentOutSegment` — the single comment-out helper shared by the managed fence and `fab config init --system` — tests the raw line for a `#` at column 0 and skips only those; every other non-blank line gets the `# ` prefix. A rendered segment therefore carries fence-level prose and content-level commented YAML such as the codex/agy/kimi provider examples; content-level lines are prefixed like live lines.
