...
Fab ships `claude`, `codex`, `agy`, and `kimi` as built-ins. claude has all three capabilities (`native: true` plus both commands); codex, agy and kimi each have both command fields and no native capability — kimi's interactive grammar being `kimi --auto -m {model}` and agy's `agy --dangerously-skip-permissions --model {model}` — so all four are pane-capable, and kimi deliberately ships no fills. Claude's headless grammar is exactly `claude -p --dangerously-skip-permissions --model {model} --effort {effort}`. User entries extend or override the built-ins without changing the mode policy merely by being present.
