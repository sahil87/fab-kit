---
type: memory
description: "`config.yaml` schema + the 5 Cs of Quality. Four-layer env > project > system > built-in resolution with scope-gated `FAB_*` overrides and `fab config show --origin`; provider capability grammar (`session_command`, `dispatch_command`, `native`) with per-role fills; the `agent.session`/`agent.workers` depth knobs and sparse profiles; `fab config` reference/init/upgrade; `dispatch.{mode,column_width,reap_done}` with the pane → native → headless descent; impact/test/hook/branch settings."
---
# Configuration
...
**`fab config reference --json` (ff2v).** The flag emits a deterministic flat JSON array in registry order. Non-null defaults include the provider capability/fill table, resolved agent profiles, both depth knobs, `dispatch.mode: native`, `dispatch.column_width: 35`, and `dispatch.reap_done: true`. The providers object contains only capabilities and fills that actually exist; claude includes `native: true` and both commands, while codex/gemini omit native. YAML and JSON key-parity guards keep the two projections aligned.
...
- **`fab config init --system`** — writes a non-overwriting `~/.fab-kit/config.yaml` scaffold generated from all `scope: both|system` registry rows: agent machinery plus `dispatch.mode`, `dispatch.column_width`, and `dispatch.reap_done`, all commented. Bare init remains a usage error; project init is documented below.
...
#### `providers`
Top-level map holding each agent's **capability grammar** and per-role fills, keyed by opaque provider names. `ProviderConfig` carries `session_command`, `dispatch_command`, `native`, and `profiles`. `agent.ResolveProvider` merges user fields over the embedded built-ins per field; explicit `native: false` disables a built-in native capability, so YAML-presence tracking distinguishes it from an omitted value.

- **`session_command`** declares how to open an interactive session and is the pane-dispatch prerequisite.
- **`native`** declares that the provider can run through the native Agent tool. Fab never infers this from the provider name.
- **`dispatch_command`** declares how to run one headless stage task. The prompt is supplied on stdin.
...
Fab ships `claude`, `codex`, and `gemini` as built-ins. All three have session and headless commands and per-role fills; claude additionally has `native: true`. Claude's headless grammar is exactly `claude -p --dangerously-skip-permissions --model {model} --effort {effort}`. Codex and Gemini do not declare native capability. User entries extend or override the built-ins without changing the mode policy merely by being present.

Provider names and model/effort values are opaque, unvalidated pass-through strings. `{model}`/`{effort}` substitution continues through `spawn.WithProfile`; a role's provider is always resolved before its fills, preventing cross-provider leakage. The pre-2.17.0 flat provider `model`/`effort` spellings remain read-time aliases for `profiles.default`; dispatch capabilities have no read-time aliases.
...
**Presentation guarantees.** The rendered claude block restates its built-in `native: true`, `session_command`, and `dispatch_command` live; codex/gemini examples remain commented and parse as absent project overrides. All strings derive from the embedded defaults, whole-block uncommenting remains valid YAML, and byte-stability/coverage guards pin the three-provider capability and fill surface.
...
- `profiles` — the **sparse per-role escape hatch** beneath the knobs, and `advertise: false` (machinery: documented in `fab config reference`, not scaffolded into each project's fence). Keys are the six **role** names — `default`, `operator`, `doing`, `review`, `hydrate`, `fast`; each value is a `{provider, model, effort}` object with **every field optional**. A set `provider` beats the role's depth knob; a set `model`/`effort` beats the resolved provider's fill. Modeled on `AgentConfig.Profiles map[string]RoleProfile`, read via the nil-safe `GetAgentProfile(role) (RoleProfile, bool)` — the bool distinguishes "no override" from "override present with empty fields", which the resolver needs.
  ...
  **There is no cross-role inheritance on the agent side.** `agent.profiles.default` is the `default` **role's** own override, never a fallback source for the other five. The system's one cross-role fallback is `providers.<p>.profiles.default` (§ `providers`), which is what makes cross-provider leakage structurally impossible and leaves no cutoff rule to document.
  ...
  > **For the current built-in values, run `fab config reference`** — it renders live from the embedded defaults, so it cannot go stale. This doc deliberately does not restate the per-role model IDs; the one human-readable mirror is the drift-guarded table in [`docs/specs/stage-models.md`](../../specs/stage-models.md) § Default role profiles.
...
The **roles** above are stable; the **profiles** are not restated here (`fab config reference`, or stage-models.md § Default role profiles).
...
**Consequence worth knowing**: an `agent.profiles.doing` override governs apply and review-pr only — it does NOT govern hydrate. A project that dials `doing` but sets no `hydrate:` entry resolves hydrate at the kit default (run `fab config reference` for the shipped profile); to run hydrate on the same profile it adds a matching `hydrate:` entry.
...
**Decision**: `agent.profiles` and `providers` carry `advertise: false`, so `fab config upgrade` scaffolds neither into a project's managed fence, while both keep their registry rows, their `--json` defaults, and a full rendered segment in `fab config reference`. The fence's whole agent surface is one ~20-line `agent:` block: the two knobs live, the role→depth partition compacted to one line per depth, and a commented `profiles:` example. That block is emitted by a **single** segment (owned by the `agent.session` row), and its partition lines derive from `agent.IsSessionRole`.
...
**Rejected**: Dropping the segments entirely (regresses `fab config reference` and `fab config init --system`, which render from them); keeping `advertise: true` (leaves the fence bloated, which is the size requirement this decision answers); a second `agent:` segment for the profiles example (duplicate YAML key); hardcoding the session/workers role lists in the renderer (a second copy of a fab-owned partition).
...
**Why (built-in default templated) (gvxd)**: (1) **explicitness** — a reader of the scaffold *sees* where the role profile lands instead of it being appended invisibly by `WithProfile`; (2) **internal consistency** — every command string in the scaffold is templated (the live claude `session_command` matches its own commented `dispatch_command` and the codex/gemini starter blocks); (3) **reference sync by construction** — `fab config reference` renders the `session_command` from `agent.DefaultSessionCommand` (`configref.go`), so the reference cannot contradict the binary. Template-mode substitution of the placed-last placeholders yields output byte-identical to the append path (verified empirically against `fab agent default/operator --print`), and append mode stays load-bearing for plain-form user configs (some pin `--model`/`--effort` and rely on append-last/last-wins) — both forms resolve identically, so no migration ships.
...
**Decision**: `dispatch.mode` (`pane | native | headless`, default `native`, scope `both`) selects the highest adapter fab may try. The shared resolver descends pane → native → headless without ascending and chooses the first possible rung. Provider `session_command`, `native`, and `dispatch_command` fields independently declare pane, native, and headless capability.
...
**Decision**: `configupgrade.CommentOutSegment` — the single comment-out helper shared by the managed fence and `fab config init --system` — tests the raw line for a `#` at column 0 and skips only those; every other non-blank line gets the `# ` prefix. A rendered segment therefore carries fence-level prose and content-level commented YAML such as the codex/gemini provider examples; content-level lines are prefixed like live lines.
**Why**: The column-0 rule keeps the fence visually uniform and exactly reversible: stripping the leading `# ` restores each segment byte-for-byte, including provider examples that intentionally remain commented at their original indent.
