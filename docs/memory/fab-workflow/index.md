...
| [execution-skills](execution-skills.md) | Apply, review, hydrate, archive, and operator behavior — `/fab-continue` for pipeline stages, `/fab-archive` for housekeeping, `/fab-operator1`, `/fab-operator2`, and `/fab-operator3` for cross-agent coordination, `/git-rebase` for branch rebasing | 2026-03-15 |
...
| [kit-architecture](kit-architecture.md) | `.kit/` structure, scripts, agent integration, distribution, versioning, monorepos | 2026-03-12 |
...
| [pipeline-orchestrator](pipeline-orchestrator.md) | Pipeline orchestrator — YAML manifest format, serial dispatch loop, worktree isolation, batch-pipeline.sh entry point, batch-pipeline-series.sh chain shorthand, finite-exit default, watch mode, local branch refs, changeman resolve, statusman detection, stacked PRs, SIGINT handling | 2026-02-27 |
