...
| [pane-commands](pane-commands.md) | `fab pane {map,capture,send,process}` subcommand reference, persistent `--server`/`-L` flag, `WithServer` argv helper, pane-ID-per-server semantics, motivating multi-socket use case | 2026-04-17 |
