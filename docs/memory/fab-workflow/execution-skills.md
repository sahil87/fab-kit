...
- **Inward sub-agent**: validates implementation against spec/tasks/checklist with fresh perspective — no shared context with the applying agent beyond explicitly provided artifacts. Performs eight validation checks: tasks complete, quality checklist, run affected tests, spot-check spec, memory drift check, code quality check, parsimony pass, and deletion-candidate prompt.
