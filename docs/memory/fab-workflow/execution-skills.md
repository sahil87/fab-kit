...
**Status mutations**: All `.status.yaml` progress transitions, checklist updates, confidence writes, and stage metric updates use `lib/stageman.sh` CLI commands (`transition`, `set-state`, `set-checklist`, `set-confidence`, `set-stage-metric`) via the Bash tool, rather than direct file editing. This centralizes validation and ensures atomic writes with `last_updated` refresh.

**Execution telemetry**: Execution orchestrators append `.history.jsonl` events through stageman logging helpers (`log-command`, `log-confidence`, `log-review`). Driver-tagged transitions populate `stage_metrics` (`started_at`, `completed_at`, `driver`, `iterations`) as stages activate and complete.
...
Loads: config, constitution, `specs/index.md`, `spec.md`, `brief.md`, target memory file(s) from `docs/memory/`, `docs/memory/index.md` and relevant domain indexes.
...
| Change | Date | Summary |
|--------|------|---------|
| 260214-r7k3-stageman-yq-metrics | 2026-02-14 | Added execution telemetry coverage for `.history.jsonl` (`log-command`, `log-confidence`, `log-review`) and driver-populated `stage_metrics` side-effects on transitions |
| 260214-q7f2-reorganize-src | 2026-02-14 | Renamed `_stageman.sh` → `lib/stageman.sh` in status mutations overview |
| 260214-w3r8-stageman-write-api | 2026-02-14 | All execution-stage `.status.yaml` transitions now use `lib/stageman.sh` CLI commands instead of direct file edits |
