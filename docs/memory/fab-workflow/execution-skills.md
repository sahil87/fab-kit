...
5. **Regenerate indexes** — run `fab memory-index` (tciy). It deterministically rewrites the root `docs/memory/index.md` (domains-only) and every domain `index.md` (file rows with git-stamped "Last Updated") from folder contents + frontmatter, byte-stable and idempotent. The hydrate skill never hand-edits index rows. Heed (but do not block on) any non-fatal shape-bound warnings it prints — they signal an over-wide / over-deep folder that `docs-reorg-memory` can later rebalance
