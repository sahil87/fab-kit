...
- **Inward sub-agent**: validates implementation against spec/tasks/checklist with fresh perspective — no shared context with the applying agent beyond explicitly provided artifacts. Performs six validation checks: tasks complete, quality checklist, run affected tests, spot-check spec, memory drift check, and code quality check.
...
**Enrollment also applies the `»` window-name prefix**: after writing the monitored entry to `.fab-operator.yaml`, the operator reads `tmux display-message -p -t <pane> '#W'` and runs `tmux rename-window -t <pane> "»<current-name>"` unless the name already starts with literal `»` (U+00BB). The guard makes the step idempotent across all enrollment sources — operator-spawned windows (already `»<wt>` from the spawn step), `/clear`-restored entries, and re-enrollment after transient removal all no-op through the guard. Rename failure (pane vanished between refresh and rename, or any other tmux error) is logged as `"{change}: window rename skipped ({error})."` and does not roll back the enrollment.

**Removal triggers**: change reaches a terminal stage (hydrate, ship, review-pr), pane dies, user explicitly stops monitoring. On removal the window name is **not** restored — the `»` prefix persists until the user renames manually. Restoring would require storing the original name somewhere, and the user's mental model is "once operator-touched, stays marked."
...
**Decision**: The `»` convention applies to **every** monitored window, not just operator-spawned ones. On enrollment the operator renames the target window via `tmux rename-window -t <pane> "»<current-name>"`, guarded by a literal `»` (U+00BB) prefix check so spawned windows and `/clear`-restored entries no-op. The rename runs after the monitored entry is durably written to `.fab-operator.yaml`; rename failure logs one line and leaves the enrollment intact. Removal does not restore the original name — the prefix persists.
**Why**: With the original decision, the `»` prefix was only half-enforced — windows enrolled via direct command dispatch, user request ("watch this pane"), autopilot spawns, and watch spawns kept their original names, so the monitored set split visually into two indistinguishable populations (prefixed vs unprefixed). Extending the convention to every enrollment path makes the tab bar an accurate at-a-glance map of what the operator is currently tracking. Rename-after-YAML-write ordering guarantees that a partial failure leaves a tracked entry without the cosmetic prefix, never a prefix without a tracked entry. No-restore-on-removal matches the user's mental model ("once operator-touched, stays marked") and avoids persisting an `original_name` field that would need maintenance through `/clear` recovery.
**Rejected**: (1) Inventing a second signal (status-bar marker, pane title) — violates parity with the existing convention, adds surface area. (2) Restoring the original name on removal — requires storing state for no user-requested benefit, and the user may have renamed the window mid-monitoring, making "original" ambiguous. (3) A generic "already-marked" regex (`^[»⚡…]`) — would silently absorb legacy or user-chosen markers; the literal `»` check keeps naming sovereignty with the user.
*Introduced by*: 260422-jyyg-operator-prefix-enrolled-windows
