...
**Decision**: `/git-pr-review` (renamed from `/git-review` to distinguish from the internal `review` stage) is a standalone skill that handles all PR review types (human or bot), replacing `/git-pr-fix` and `/git-pr` Step 6 (inline auto-fix). It checks for existing reviews with comments — if found, processes them; if none, runs a review request cascade (Copilot → Codex → Claude) to obtain an automated review. It drives the `review-pr` pipeline stage with statusman integration.
**Why**: The old architecture had fragmented review handling across multiple skills. The new skill consolidates all review handling into a single flow: check for existing reviews, process them or cascade to request one.
