...
**Copilot review fix**: `/git-pr-fix` is a standalone autonomous skill that waits for GitHub Copilot review comments on the current branch's PR, triages them (actionable vs informational), and auto-fixes actionable ones. Detection uses a 3-phase approach: (1) check `GET /reviews` for an existing `copilot-pull-request-reviewer[bot]` review — if found, skip to triage; (2) `POST /requested_reviewers` with `reviewers[]=copilot-pull-request-reviewer[bot]` — any non-zero exit from `gh api` means Copilot is not available, print message and stop (no polling); (3) poll for completion (mode-specific). Two modes: **standalone** (Phase 3 does a single check — if no review yet, prints "re-run later" and exits) and **wait mode** (Phase 3 polls every 30s, max 12 attempts / 6 minutes — used when auto-invoked from `/git-pr` Step 6). Note: the Copilot bot uses different login names across API endpoints — `"Copilot"` in `GET /requested_reviewers`, `"copilot-pull-request-reviewer[bot]"` in `GET /reviews` and `POST /requested_reviewers` body (empirically confirmed). Fixes are committed in a single `fix: address copilot review feedback` commit. The skill is idempotent — re-running after fixes finds no new modifications and exits cleanly. `/git-pr` auto-invokes this behavior as a best-effort Step 6 after printing "Shipped." — failures never block the shipping workflow.
...
### Companion Skill for Copilot Review Fixes
**Decision**: `/git-pr-fix` is a standalone skill auto-invoked from `/git-pr` Step 6, rather than embedding the logic directly in `/git-pr`.
**Why**: Keeps `/git-pr` focused on shipping (commit → push → PR). The standalone skill is independently re-runnable for manual invocation after seeing Copilot comments. Two detection modes (first-poll bail for standalone, wait mode for inline) match the distinct invocation contexts.
**Rejected**: Embedding all logic in `/git-pr` — bloats a clean shipping skill with polling and fix logic. Always polling in both modes — wastes time when review is already present or repo doesn't use Copilot.
*Introduced by*: 260303-4ojc-git-pr-copilot-fix
