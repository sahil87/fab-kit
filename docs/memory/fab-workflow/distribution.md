...
5. `Help-dump → shll.ai` — dump + validate `help/fab-kit.json` (fatal), then open a non-fatal auto-merging PR into `sahil87/shll.ai` (see below)
6. Run `just package-kit` (creates 5 archives with `fab-go` per platform — `fab` router and `fab-kit` are Homebrew-distributed via `package-brew`; `wt` and `idea` are external Homebrew dependencies)
7. Create GitHub Release via `gh release create` with all 5 archives and commit-level changelog (minor releases cumulate all commits since the previous minor; patch releases show commits since the previous release)

The workflow sets `permissions: contents: write` for release creation. `GITHUB_TOKEN` is used implicitly by `gh`.

After `Build all targets`, the workflow SHALL run a `Help-dump → shll.ai` step that produces the CLI command-reference artifact and delivers it cross-repo. The step runs `./dist/bin/fab-go-linux-amd64 help-dump > help/fab-kit.json` and validates the output with `jq -e '.tool=="fab" and .schema_version==1 and (.version|length>0) and (.root|type=="object")'`. **The dump + validate portion MUST be fatal to the release** (a malformed dump is a real fab-kit bug). The step then opens an **auto-merging PR** into the external repo `sahil87/shll.ai` (writing `help/fab-kit.json`) — clone-with-token + branch `help-dump/fab-kit-<version>` + `gh pr create` + `gh pr merge --auto --squash`, authenticated with the `SHLLAI_TOKEN` repo secret (contents + pull-requests write). **The shll.ai PR portion MUST be non-fatal** (wrapped in `|| echo "::warning::..."`) so a missing token or a shll.ai outage never blocks fab-kit's own release. The step is idempotent: it skips PR creation when the staged file is byte-identical to shll.ai's copy (`git diff --cached --quiet`). The dump uses the linux/amd64 `fab-go` binary because only that target carries both the rich command tree and the `main.version` ldflags. `help/fab-kit.json` is a transient artifact owned by shll.ai — fab-kit does not retain it (`/help/` is gitignored). This is fab-kit's slice of a 7-tool rollout; the shll.ai site-side consumer (Astro loader + reference UI) is a separate concern in the shll.ai repo.

**Scenarios**:
- Successful release — `help/fab-kit.json` is produced from `dist/bin/fab-go-linux-amd64`, validated with `jq -e`, and an auto-merge-squash PR is opened into `sahil87/shll.ai` when the content differs
- Byte-identical content — the delivery step skips PR creation (idempotent)
- Malformed dump — `jq -e` validation fails and the release job fails (fatal)
- Missing `SHLLAI_TOKEN` / shll.ai outage — the PR step warns and the release otherwise succeeds (non-fatal)
...
- **Help-dump delivered to shll.ai via auto-merging PR, not direct push (260602-xob7)**: The release workflow delivers `help/fab-kit.json` to `sahil87/shll.ai` by opening an auto-merge-squash PR rather than pushing to `main` directly. — *Why*: fab-kit is one of 7 tools that deliver their command reference into the single shll.ai repo around the same release window; concurrent direct pushes would hit non-fast-forward rejections. A per-tool PR with `--auto --squash` lets GitHub serialize the merges. Rejected: direct `git push` to `main` (push race across 7 tools). The dump + validate portion is **fatal** (a malformed dump is a genuine fab-kit bug), while the shll.ai PR portion is **non-fatal** (a downstream site-reference delivery failure — bad token, shll.ai outage — must not block fab-kit's own release/tap update). The dump runs the rich `fab-go` binary (`dist/bin/fab-go-linux-amd64`), not the `fab`/`fab-kit` shim, because only that target carries both the full command tree and the `-X main.version` ldflags from `src/kit/VERSION` — guaranteeing a byte-accurate, correctly-versioned reference. Rejected: dumping the shim (thin command surface, no real version).
