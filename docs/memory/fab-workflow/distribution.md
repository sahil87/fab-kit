...
3. **Absolute links in the slice region** — every repo-relative doc link in the slice region (head → `## Development` boundary) MUST be an absolute `https://github.com/sahil87/fab-kit/blob/main/<path>` GitHub-blob URL. The README's relative base differs by surface (`/` on GitHub vs `/tools/fab-kit/readme/` on shll.ai) and the renderer does no base rewrite, so no single relative string resolves in both. In-page `#anchor` links survive when their target heading stays in the slice; anchors pointing at moved/non-existent sections are dropped or re-pointed.
...
| `docs/site/` | Users | **RESERVED** — only once shll.ai ships the §9 pull path |
| README tail (below `## Development`), `CONTRIBUTING.md` | GitHub-native readers | No — fenced off by the tail boundary |
| `docs/internal/` | Maintainers | **Never** |
| `docs/img/` | (asset store) | Indirectly — via absolute raw URLs in the README |

- **`docs/internal/`** — free-form maintainer/design notes that must never reach the site. Distinct from `docs/specs/` (Constitution VI pre-implementation design intent) and `docs/memory/` (post-implementation "what shipped"); `docs/internal/` is the catch-all for everything else maintainer-only.
- **`docs/site/`** is **structure + explainer README only** today — **no content is migrated into it**. shll.ai's §9 `docs/site/` pull path is marked RESERVED / NOT YET IMPLEMENTED (the puller fetches only `README.md`), so nothing reads `docs/site/` yet. Migrating load-bearing README prose here would strand it on **both** surfaces (gone from the README, not pulled by the site). The safe floor is forward structure ahead of its consumer; real prose migrates only when §9 ships.
