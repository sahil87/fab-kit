...
Events are auto-logged as side effects: `preflight.sh --driver` logs command invocations, `statusman.sh finish/fail review` logs review outcomes, and `calc-score.sh` logs confidence changes via `logman.sh`. Skills never call `logman.sh` directly. The file is informational — no skill reads it for logic. Useful for debugging workflow progression and understanding change history.
