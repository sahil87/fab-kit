...
**Traceability annotations are REQUIRED** (j6cs, cross-linking changed from OPTIONAL): each `## Tasks` item carries a `<!-- R# -->` trace annotation naming the requirement it implements; each `## Acceptance` item names its requirement (e.g., `A-001 R2: {outcome}`). This `R# → T# → test → A#` chain lets the autonomous apply↔review loop localize a failing acceptance item back to its requirement.
