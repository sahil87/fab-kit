...
- **Auto-logging**: `finish review` auto-calls `logman.sh review "passed"`, `fail review` auto-calls `logman.sh review "failed" [rework]`. History logging (command, confidence, review) is handled by `logman.sh` — statusman does not log directly
