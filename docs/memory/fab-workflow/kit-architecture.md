...
`fab/.kit/` is the portable engine directory that contains all workflow logic: skill definitions, artifact templates, shell scripts, a Go binary (`bin/fab`), and version tracking. It is the only piece a new project needs from outside — everything else is generated by `/fab-setup`. This doc covers the `.kit/` directory structure, shell scripts, Go binary with shim delegation, agent integration via symlinks, distribution, updating, and monorepo guidance.
...
├── bin/                    # Go binary (platform-specific, from per-platform archives)
│   ├── fab                 # Single Go binary — all lib/ script functionality as subcommands
...
`.kit/` is a standalone directory — no package manager, no system install. The Go binary is an optional performance enhancement included in platform-specific archives, not a required dependency. This preserves the "pure prompt play" principle — shell scripts remain the fallback and work identically without the binary.
...
#### Shim Delegation Layer

All 7 `lib/` shell scripts contain a shim at the top that delegates to the Go binary when it is present and executable. The shim pattern:

```bash
_fab_bin="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)/../../bin/fab"
if [ -x "$_fab_bin" ]; then
  exec "$_fab_bin" <command> "$@"
fi
```

When `fab/.kit/bin/fab` exists (platform archive was used), every shell script call transparently `exec`s the Go binary — the bash code below the shim never runs. When the binary is absent (generic archive or manual copy), the shim falls through and the full bash implementation executes. This provides a seamless upgrade path: no skill or script changes needed, same calling convention, automatic performance improvement.

The shim mapping (shell script → Go command):
| Shell script | Go binary command |
|---|---|
| `resolve.sh` | `fab resolve` |
| `statusman.sh` | `fab status` |
| `logman.sh` | `fab log` |
| `preflight.sh` | `fab preflight` |
| `changeman.sh` | `fab change` |
| `calc-score.sh` | `fab score` |
| `archiveman.sh` | `fab archive` |

#### Skill Invocation Convention (`_scripts.md`)

The `_scripts.md` partial (loaded by every skill via `_preamble.md`) defines the calling convention for all kit operations. Skills invoke operations via `fab/.kit/bin/fab <command> <subcommand> [args...]` as the primary method. Shell script paths (`fab/.kit/scripts/lib/<script>.sh`) are documented as legacy fallback — they still work identically because the shim delegates to the same Go binary. The `_scripts.md` partial includes the full command mapping table, argument formats, stage transition sequences, and error patterns.

## Design Decisions

### All Logic in Markdown and Shell (with Optional Go Binary)
**Decision**: Workflow logic lives in markdown skill files and shell scripts. The Go binary at `fab/.kit/bin/fab` is an optional performance enhancement — shell scripts contain shims that delegate to the binary when present, falling back to their full bash implementations when it is not. No runtime dependencies for end users; the Go toolchain is only needed for building from source.
**Why**: Constitution I (Pure Prompt Play). Any AI agent that can read markdown and execute shell commands can drive the workflow. The Go binary is a pre-compiled static binary (no runtime dependencies via `CGO_ENABLED=0`), distributed inside the platform-specific archive — it is not a system install.
**Rejected**: CLI tool, npm package, or Python script — all introduce system dependencies. Also rejected: making the Go binary mandatory — preserves graceful degradation for unsupported platforms.
