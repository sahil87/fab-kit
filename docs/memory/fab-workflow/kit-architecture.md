...
- `fab-kit update [--skip-brew-update]` — self-update the installed `fab-kit`/`fab` binaries via Homebrew. Flow: guard (no-op with hint if not brew-installed) → `brew update --quiet` (tap-metadata refresh) → `brew info --json=v2 fab-kit` (latest stable) → short-circuit "Already up to date" if latest equals current → `brew upgrade fab-kit`. `--skip-brew-update` (bool, default false) skips **only** the `brew update --quiet` refresh; the version check, up-to-date short-circuit, and `brew upgrade` still run. The flag name is a cross-toolkit contract shared with sibling tools, so it is fixed exactly. The flag passes through the `fab` router unchanged (`fab update --skip-brew-update`).
...
| Change | Date | Summary |
|--------|------|---------|
| 260531-skbu-skip-brew-update | 2026-05-31 | Added `--skip-brew-update` boolean flag (default false) to `fab-kit update`, wired in `src/go/fab-kit/cmd/fab-kit/main.go` (`updateCmd`) and threaded as `skipBrewUpdate bool` through `internal.Update(currentVersion, skipBrewUpdate)` (`src/go/fab-kit/internal/update.go`). When set, skips **only** the internal `brew update --quiet` tap-metadata refresh; the `brew info` version check, the "Already up to date" short-circuit, and `brew upgrade fab-kit` all still run. Default behavior unchanged. The version-guard auto-update call in `sync.go` passes `false` (preserves prior behavior). Flag name is a cross-toolkit contract shared with 5 sibling tools — fixed exactly. Added the `update` subcommand to the `fab-kit` command list (previously omitted) documenting the full brew self-update flow. Tests: `internal/update_test.go` (contract tests via a fake `brew` on PATH plus `isBrewInstalledFn`/`brewLatestVersionFn` seams) asserting `brew update` is skipped while `brew upgrade` and the version check still run; `cmd/fab-kit/main_test.go` asserts flag registration (bool, default false). |
