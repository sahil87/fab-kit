...
`fab/.kit/` is the portable engine directory that contains all workflow logic: skill definitions, artifact templates, a shell dispatcher (`bin/fab`) with the Go backend (`bin/fab-go`), utility shell scripts, and version tracking. It is the only piece a new project needs from outside — everything else is generated by `/fab-setup`. This doc covers the `.kit/` directory structure, the dispatcher and Go backend, agent integration via symlinks, distribution, updating, and monorepo guidance.
...
#### `release.sh` (dev-only, at `src/scripts/release.sh`)
...
`.kit/` is a standalone directory — no package manager, no system install. The dispatcher script (`bin/fab`) is always included. The Go backend (`bin/fab-go`) is included in platform-specific archives. A compiled backend is required — there is no shell script fallback.
...
A POSIX-compatible shell script that serves as the sole entry point for all fab CLI operations. The dispatcher supports a backend override mechanism, then delegates to the Go backend. The Go backend is required — there is no shell script fallback.

**Backend override** (checked before default):
1. `FAB_BACKEND` environment variable — per-command override (e.g., `FAB_BACKEND=go fab resolve`)
2. `.fab-backend` file at repo root — persistent project-level override (three levels up from `fab/.kit/bin/`, gitignored, contains `go`, whitespace trimmed)
Invalid or unavailable override values silently fall through to the default.

**Default**:
1. `fab/.kit/bin/fab-go` — Go binary (from per-platform release archives)
2. Error — exits 1 with message: `Error: no fab backend found (expected fab-go in <dir>)`

**Version handling**: `fab --version` is handled by the dispatcher itself (not delegated). Reads `fab/.kit/VERSION` and reports the active backend: `fab {version} ({backend} backend)` where `{backend}` is `go` or `none`.

The dispatcher script is always shipped and always tracked in git. The Go backend (`fab-go`) is gitignored.
...
### All Logic in Markdown and Shell (with Go Backend)
**Decision**: Workflow logic lives in markdown skill files and shell scripts. A shell dispatcher at `fab/.kit/bin/fab` serves as the sole entry point, delegating to the Go backend (`fab-go`) with a backend override mechanism (`FAB_BACKEND` env var, `.fab-backend` file). No runtime dependencies for end users; the Go toolchain is only needed for building from source.
**Why**: Constitution I (Pure Prompt Play). Any AI agent that can read markdown and execute shell commands can drive the workflow. The Go backend is a pre-compiled static binary (no runtime dependencies via `CGO_ENABLED=0`), distributed inside platform-specific archives — it is not a system install. The dispatcher pattern centralizes backend selection in one place instead of duplicating shim logic in every shell script.
**Rejected**: CLI tool, npm package, or Python script — all introduce system dependencies. Also rejected: making compiled backends mandatory — preserves graceful degradation for unsupported platforms. Also rejected: per-script shim delegation (previous approach) — duplicated 3-line shim in all 7 scripts. Also rejected: CLI flag for backend selection — would require dispatcher to parse args before delegating.
