...
**Deferred follow-up (`sx7a`)**: The *file-moving* rebalancer — splitting an over-wide domain into sub-domains, with relative-link rewriting and the Internal-vs-External addressing decision — is NOT in this change. It lands as an enhancement to the existing `docs-reorg-memory` skill (an agent-driven propose-then-apply path, per Pure Prompt Play — not a Go file-mover). `tciy` ships the detect/diagnose + index-regen foundation; `sx7a` exercises and hardens the apply path, made cheap and conflict-free *because* the generated index already exists.
