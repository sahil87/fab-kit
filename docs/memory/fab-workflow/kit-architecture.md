...
**Binary location**: `~/.fab-kit/versions/{version}/fab-go` — cached per-version by the system shim. Included in per-platform release archives (`kit-{os}-{arch}.tar.gz`). No longer stored in `fab/.kit/bin/` — the repo holds content only.
...
**Binary location**: System PATH via Homebrew (`brew install fab-kit`). No longer stored in `fab/.kit/bin/` — distributed exclusively through the Homebrew formula as a version-independent standalone utility.
...
| `_cli-external.md` | Operator-only (loaded by `fab-operator.md` startup) | External CLI tools: `wt` (worktree manager), `tmux`, `/loop` |
