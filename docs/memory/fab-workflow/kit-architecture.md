...
Version selection for the fab-go exec is inline in `execFabGo` (and mirrored in `printHelp`): walk up from CWD to find `fab/project/config.yaml`; if `cfg != nil` use `cfg.FabVersion` (project-pinned), else use the router's build-time `version` constant (router-bundled). The resolved `fab-go` binary lives at `~/.fab-kit/versions/{version}/fab-go`; missing binaries are auto-fetched from GitHub releases and cached. If `config.yaml` exists but cannot be parsed, the router still hard-errors with the parse error from `internal.ResolveConfig` — only the missing-config case becomes a soft fall-through.
