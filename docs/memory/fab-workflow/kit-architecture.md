---
description: "`src/kit/` structure (binary-free; `spec.md` template removed in j6cs), three-binary architecture (fab router + fab-kit + fab-go), router always-route policy, `fab-kit sync`, agent integration, versioning, monorepos, underscore file ecosystem, `fab pane` command group, `fab shell-init`, hidden `fab help-dump`"
---
# Kit Architecture
...
- **`slugify(string) string`** — the deterministic, collision-free, filesystem-safe rule: strip the leading path separator, then replace remaining separators with `-` (so `/tmp/tmux-1000/default` → `tmp-tmux-1000-default`). Empty input slugifies to `"default"`.
...
The `fab` binary (installed via `brew install fab-kit`) is the user-facing entry point. It uses negative-match routing: a static allowlist of fab-kit commands (`init`, `upgrade-repo`, `sync`, `update`, `doctor`) is dispatched to `fab-kit` via `syscall.Exec`; the inline arguments `--version`/`-v` and `--help`/`-h`/`help` are handled by the router itself (printing version or composed help); all other commands are dispatched to the version-resolved `fab-go` via `syscall.Exec`.
...
- `fab pane` — parent command grouping five pane-related subcommands (`map`, `capture`, `send`, `process`, `window-name`). Available from any directory, including outside a fab repo — config-independent by virtue of the router's always-route policy plus the absence of any per-command `resolve.FabRoot()` guard (pane subcommands resolve state from pane IDs, not the invoker's CWD). Detailed subcommand behavior and the `--server`/`-L` flag live in [pane-commands.md](pane-commands.md).
