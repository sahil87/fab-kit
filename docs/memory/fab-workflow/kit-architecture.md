...
`fab/.kit/` is the portable engine directory that contains all workflow logic: skill definitions, artifact templates, shell scripts, a shell dispatcher (`bin/fab`) with optional compiled backends (`bin/fab-go`, `bin/fab-rust`), and version tracking. It is the only piece a new project needs from outside — everything else is generated by `/fab-setup`. This doc covers the `.kit/` directory structure, the dispatcher and backend priority chain, shell scripts, agent integration via symlinks, distribution, updating, and monorepo guidance.
...
│   ├── fab                 # Shell dispatcher — sole entry point, checks fab-rust → fab-go → shell
...
A POSIX-compatible shell script that serves as the sole entry point for all fab CLI operations. The dispatcher checks for compiled backends in priority order and delegates accordingly, with shell scripts as the always-available fallback.

**Backend priority chain** (first available wins):
1. `fab/.kit/bin/fab-rust` — Rust binary (future, not yet implemented)
2. `fab/.kit/bin/fab-go` — Go binary (optional, from per-platform archives)
3. Shell scripts — dispatcher routes to `fab/.kit/scripts/lib/` via a `case` statement

**Version handling**: `fab --version` is handled by the dispatcher itself (not delegated). Reads `fab/.kit/VERSION` and reports the active backend: `fab {version} ({backend} backend)` where `{backend}` is `rust`, `go`, or `shell`.

**Shell fallback diagnostic**: When no compiled backend is available, the dispatcher prints `[fab] using shell backend` to stderr before routing the command. This diagnostic is NOT printed for `--version` (handled before backend resolution).

**Routing table** (shell fallback):
...
A single Go binary at `fab/.kit/bin/fab-go` (source: `src/fab-go/`) that ports all 7 `lib/` shell scripts into subcommands. The primary motivation is performance: collapsing the preflight → statusman → resolve → logman call chain from ~25ms of subprocess spawning to <1ms of in-process function calls. Also eliminates the `yq` v4 dependency by internalizing YAML parsing.

**Module**: `github.com/wvrdz/fab-kit/src/fab-go` (Go 1.22+, dependencies: cobra, gopkg.in/yaml.v3, no CGo)

**Binary location**: `fab/.kit/bin/fab-go` — included in per-platform release archives (`kit-{os}-{arch}.tar.gz`), absent from the generic `kit.tar.gz`. When present and executable, the dispatcher delegates all commands to it.

**Subcommands** (1:1 mapping to shell scripts):
- `fab-go resolve [--id|--folder|--dir|--status] [<change>]` → resolve.sh
- `fab-go log command|confidence|review|transition ...` → logman.sh
- `fab-go status start|advance|finish|reset|skip|fail|...` → statusman.sh (all 20+ subcommands)
- `fab-go preflight [<change>]` → preflight.sh
- `fab-go change new|rename|switch|list|resolve ...` → changeman.sh
- `fab-go score [--check-gate] [--stage <stage>] <change>` → calc-score.sh
- `fab-go archive <change> --description "..." | restore | list` → archiveman.sh
